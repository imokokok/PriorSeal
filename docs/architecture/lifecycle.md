...
| Authorization | complete intent, principal, authorizer, delegate, time window, nonce, audience, policy hash and signature | `ACCEPTED → BOUND`; a pending correlated transaction is only a candidate, while a confirmed or reverted transaction on the requested chain with the delegated executor and nonce consumes the single use; complete action correlation is reported separately so a mismatch cannot reopen permission |
