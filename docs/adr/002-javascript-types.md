...
Frozen examples, standalone verifiers, vector generators, simple build tooling and adversarial tests that intentionally construct malformed or partial runtime values remain reviewed JavaScript. Those tests obtain more assurance from runtime validation than from pervasive escape casts. Runtime parsing and validation remain authoritative for every untrusted input.
