...
## Completion (2026-09-24): remaining owned examples

The remaining owned example entry points now have TypeScript sources. Current examples such as the receipt and RWA walkthroughs keep generated `.mjs` entry points for Node compatibility. Historical partner verification files have checked `.reference.mts` sources instead: their original `.mjs` bytes are pinned by SHA-256 in `scripts/build-core.mts` and remain immutable because existing fixture manifests and distributed packages identify those exact bytes. To change a historical verifier, copy its TypeScript reference into a new fixture version and publish a new manifest. The 402Signal standalone verifier and WAK v1.0.1 bundle are built from existing TypeScript sources. The WAK v1 bundle keeps its frozen JavaScript build source and now has a checked TypeScript reference. The 402Signal route-guard file remains reviewed third-party vendor code.
