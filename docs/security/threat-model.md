...
| Faulty or malicious RPC | explicit endpoint configuration, chain-ID check, response validation, per-endpoint retry/fallback; uncertainty remains undetermined |
...
| Replay | bounded Idempotency-Key record with request hash and TTL |
...
Operational limitation: independent verification proves the issuer signed the included bytes. It does not prove economic safety, RPC correctness, token semantics, internal transfers, or user intent outside the signed fields.
