...
The [`Web3 Agent Kit Base swap fixture`](examples/web3-agent-kit-base-swap-v1/README.md) applies a single-authority composition model to one synthetic ETH → USDC call: Insight is signed advisory input, the Web3 Agent Kit governor is the final agent-side policy decision point, and PriorSeal records the subsequent principal authorization and exact execution. Its self-contained bundle verifies after modeled ordinary key rotation and includes fail-closed mutations for the governor decision, raw calldata, Insight revocation and observed execution. Run it with `npm run example:web3-agent-kit-base-swap`.
