...
Cloudflare D1 is the production persistence backend. The Worker uses its `DB` binding and the schema in `d1/migrations/`; the previous Neon database is retained as a rollback snapshot, not a live dependency. Local Node operation still supports PostgreSQL through `DATABASE_URL`, with `DATABASE_URL_UNPOOLED` used by `npm run db:migrate`. Read [database operations](docs/runbooks/database.md) before applying migrations or restoring data.
