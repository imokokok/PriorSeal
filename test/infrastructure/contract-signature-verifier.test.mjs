// Generated from contract-signature-verifier.test.mts by npm run core:build. Do not edit directly.
...
const authorization = buildAuthorization({
  intent: { intentId: "erc1271-test", chainId: 8453, action: "TRANSFER", asset: "eip155:8453/native", amount: "1", sender: `0x${"a".repeat(40)}`, recipient: `0x${"b".repeat(40)}`, validUntil: 2e3, nonce: "1" },
...
test("ERC-1271 verifier distinguishes invalid signatures from unavailable chain state", async () => {
  const invalid = createContractSignatureVerifier({ rpcClient: { async call(_url, method) {
    return method === "eth_chainId" ? "0x2105" : "0x";
  } }, rpcUrls: () => ["base"] });
  assert.equal(await invalid(input), false);
  const unavailable = createContractSignatureVerifier({ rpcClient: { async call() {
    throw new Error("offline");
  } }, rpcUrls: () => ["base"] });
  await assert.rejects(() => unavailable(input), (error) => hasCode(error, "AUTHORIZATION_VERIFIER_UNAVAILABLE"));
  const wrongChain = createContractSignatureVerifier({ rpcClient: { async call() {
    return "0x1";
  } }, rpcUrls: () => ["wrong"] });
