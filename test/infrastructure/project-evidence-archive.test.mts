...
import { archiveEntry, archiveQuery, type ArchiveAccess } from '../../src/application/archive/evidence-archive.mjs';
...
test('PostgreSQL archive migration enforces immutable, scoped evidence and stable pagination', async () => {
  ...
  try {
    ...
    const store = createPostgresStore(testPostgresPool({ query: async (statement, parameters) => { queries.push(statement); return db.query(statement, parameters); } }));
    const a: ArchiveAccess = { projectId: 'alpha', environment: 'test', role: 'writer' }, b: ArchiveAccess = { ...a, projectId: 'beta' };
    const artifact = (n: number) => ({ schema: 'priorseal.verification-bundle.v1', receipt: { receiptId: `psr-${n}`, execution: { txHash: `0x${String(n).repeat(64)}`, status: 'CONFIRMED' } } });
    const query = (url: string, access: ArchiveAccess = a) => archiveQuery(new URL(url, 'https://local.invalid'), access);
    const first = await store.saveArchiveEntry(archiveEntry(artifact(1), a, { now: 1000 }));
    const largeArtifact = { ...artifact(2), payload: 'x'.repeat(64_000) };
    const second = await store.saveArchiveEntry(archiveEntry(largeArtifact, a, { now: 2000, supersedesId: first.id }));
    ...
    await store.saveArchiveEntry(archiveEntry(artifact(3), a, { now: 3000 }));
...
test('rotation diagnostics include expired unresolved authorizations and flag truncation without writes', async () => {
  ...
  try {
    ...
    assert.equal((await db.query<{ count: number }>('SELECT COUNT(*)::int AS count FROM authorizations')).rows[0]?.count, 1003);
