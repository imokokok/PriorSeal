...
test('Postgres migration backfill gives the same compact proof as the memory log', async () => {
  ...
  try {
    ...
    for (let sequence = 1; sequence <= 250; sequence += 1) {
      ...
      if (sequence === 1) acceptance = { ...entry, issuer: 'test', keyId: 'key' };
    ...
    await backfillAuthorizationMerkleIndex(db);
    const expected = await memory.getAuthorizationMerkleSnapshot(acceptance);
    const actual = await createPostgresStore(db).getAuthorizationMerkleSnapshot(acceptance);
    assert.deepEqual(actual, expected);
    assert.equal(actual.proof.length, 8);
    assert.equal((await db.query('SELECT COUNT(*)::integer AS count FROM authorization_log_merkle_nodes')).rows[0].count, 494);
    const pool = { query: db.query.bind(db), connect: async () => ({ query: db.query.bind(db), release() {} }) };
    const postgres = createPostgresStore(pool);
