...
test('PostgreSQL archive migration enforces immutable, scoped evidence and stable pagination', async () => {
  ...
  try {
    ...
    await db.exec(sql);
    const queries = [];
    const store = createPostgresStore({ query: (...args) => { queries.push(args[0]); return db.query(...args); } });
    ...
    const expectedListEntry = { ...second };
    delete expectedListEntry.artifact;
    delete expectedListEntry.sequence;
    ...
    assert.match(queries.at(-1), /^SELECT entry_json,sequence/);
    ...
    await assert.rejects(store.saveArchiveEntry(archiveEntry(artifact(2), b, { supersedesId: first.id })), error => error.code === '23503');
    await assert.rejects(store.saveArchiveEntry(archiveEntry(artifact(1), a, { supersedesId: second.id })), error => error.code === 'ARCHIVE_CONFLICT');
...
test('rotation diagnostics include expired unresolved authorizations and flag truncation without writes', async () => {
  ...
  try {
    ...
    const report = await rotationImpact(db, 'next', 500);
    ...
    await assert.rejects(rotationImpact(db, "key'; DELETE FROM authorizations; --"), /Usage/);
  } finally { await db.close(); }
});
