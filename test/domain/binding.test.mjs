// Generated from binding.test.mts by npm run core:build. Do not edit directly.
import test from "node:test";
import assert from "node:assert/strict";
import { bindIntentExecution } from "../../src/domain/binding.mjs";
import { buildIntent } from "../../src/domain/intent.mjs";
const intent = { chainId: 8453, action: "TRANSFER", sender: "0xaa", recipient: "0xbb", asset: "native", amount: "10", nonce: "2", validUntil: 1e3, constraints: { minConfirmations: 3, maxGasUsed: "100000" } };
...
test("binding returns explainable reason codes", () => {
  assert.deepEqual(bindIntentExecution(intent, execution).reasonCodes, []);
  const result = bindIntentExecution(intent, { ...execution, recipient: "0xcc", confirmations: 1, gasUsed: "200000" });
  assert.equal(result.bound, false);
  assert.deepEqual(result.reasonCodes, ["RECIPIENT_MISMATCH", "INSUFFICIENT_FINALITY", "GAS_LIMIT_EXCEEDED"]);
});
test("multiple transfers are undetermined unless uniquely selected", () => {
  const result = bindIntentExecution(intent, { ...execution, transfers: [{}, {}] });
  assert.equal(result.reasonCodes.includes("AMBIGUOUS_TRANSFER"), true);
});
test("action and block time participate in binding", () => {
  assert.equal(bindIntentExecution(intent, { ...execution, action: "CONTRACT_CALL" }).reasonCodes.includes("ACTION_MISMATCH"), true);
  assert.equal(bindIntentExecution(intent, { ...execution, executedAt: 1001 }).reasonCodes.includes("OUTSIDE_TIME_WINDOW"), true);
  assert.equal(bindIntentExecution(intent, execution).reasonCodes.includes("OUTSIDE_TIME_WINDOW"), false);
});
test("binding fails closed when nonce or constrained gas evidence is missing", () => {
  ...
  const { gasUsed: _gasUsed, ...missingGas } = execution;
...
test("exact-call profile binds transaction bytes instead of ambiguous transfer semantics", () => {
  ...
  assert.throws(
    ...
    (error) => hasCode(error, "INVALID_INTENT")
  );
  assert.throws(
