...
test('intent fields bind to completed execution', () => { const { nonce: _nonce, ...missingNonce } = execution; assert.equal(buildReceipt({ intent, execution, issuer: 'test' }).outcome, 'COMPLETED'); assert.equal(buildReceipt({ intent, execution: missingNonce, issuer: 'test' }).outcome, 'UNDETERMINED'); assert.equal(buildReceipt({ intent, execution: { ...execution, recipient: `0x${'c'.repeat(40)}` }, issuer: 'test' }).outcome, 'UNDETERMINED'); assert.equal(buildReceipt({ intent, execution: { ...execution, status: 'PENDING' }, issuer: 'test' }).outcome, 'PENDING'); });
test('memory persistence rejects different signed evidence under the same receipt ID', async () => { const store = createMemoryStore(); const receipt = signReceipt(buildReceipt({ intent, execution, issuer: 'test', keyId: 'k1', issuedAt: 100 }), priv); await store.saveReceipt(receipt); await assert.rejects(() => store.saveReceipt({ ...receipt, validUntil: receipt.validUntil + 1 }), (error) => hasCode(error, 'RECEIPT_ID_CONFLICT')); assert.deepEqual(await store.saveReceipt(structuredClone(receipt)), receipt); });
