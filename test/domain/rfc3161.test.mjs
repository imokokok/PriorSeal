// Generated from rfc3161.test.mts by npm run core:build. Do not edit directly.
...
test("DigiCert RFC 3161 evidence verifies offline against pinned roots", async () => {
  ...
  assert.deepEqual(result, { valid: true, code: "OK", timestamp: 1788685922, serialNumber: "009efe6bb23356d66cf73c0924a34f4db3", profile: "digicert-rfc3161-v1" });
});
...
test("a timestamp policy fails closed when no TSA provider is configured", async () => {
  ...
  const draft = buildAuthorization({
    intent: { intentId: "timestamp-required-1", chainId: 8453, action: "TRANSFER", asset: "eip155:8453/native", amount: "1", sender: `0x${"a".repeat(40)}`, recipient: `0x${"b".repeat(40)}`, validUntil: 2e3, nonce: "1" },
    principal: { type: "user", id: "user-1", account: account.address },
    authorizer: { type: "eip712", address: account.address },
    delegate: { agentId: "agent-1", executor: `0x${"a".repeat(40)}` },
    issuedAt: 1e3,
    notBefore: 1e3,
    expiresAt: 2e3,
  ...
  await assert.rejects(() => authorizeIntent({ input: authorization, store, privateKeyPem: issuer, issuer: "test", keyId: "key-1", policy, requireTimestamp: true, now: () => 1001e3 }), (error) => hasCode(error, "TIMESTAMP_NOT_CONFIGURED"));
