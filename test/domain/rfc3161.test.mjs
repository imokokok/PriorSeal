...
const timestamp = 1_788_672_364;
const nonce = '4d5af0397733db00';
...
test('DigiCert RFC 3161 evidence verifies offline against pinned roots', async () => {
  ...
  assert.deepEqual(result, { valid: true, code: 'OK', timestamp, serialNumber: '00e8846f9943d437b70730bc6a1b9de61a', profile: 'digicert-rfc3161-v1' });
});
...
test('a timestamp policy fails closed when no TSA provider is configured', async () => {
  ...
  const draft = buildAuthorization({
    ...
    issuedAt: 1_000, notBefore: 1_000, expiresAt: 2_000, authorizationNonce: `0x${'2'.repeat(64)}`, maxUses: '1', audience: 'runproof', policyHash: `0x${hashJson(policy)}`,
  });
