...
const requestUrl = (input: Parameters<typeof fetch>[0]) => typeof input === 'string' ? input : input instanceof URL ? input.href : input.url;
...
type WitnessKeys = ReturnType<typeof keys>;
function witnessPolicy(entries: WitnessKeys[]) {
...
type WitnessServer = ReturnType<typeof createWitnessHttpServer>;
type InvokeOptions = { method: string; headers: Record<string, string>; body?: string; bodyStream?: Readable };
type MockResponse = EventEmitter & {
  ...
  headers: HeadersInit;
  body: BodyInit | null;
  writeHead: (status: number, headers: HeadersInit) => MockResponse;
  end: (value?: BodyInit | null) => void;
};
async function invokeServer(server: WitnessServer, path: string, options: InvokeOptions) {
  ...
  res.writableEnded = false; res.destroyed = false; res.status = 200; res.headers = {}; res.body = null;
  res.writeHead = (status, headers) => { res.status = status; res.headers = headers; return res; };
  res.end = (value) => { res.writableEnded = true; res.body = value ?? null; res.emit('finish'); res.emit('close'); };
...
test('signed authorization policy binds witness keys and receipt verification enforces pre-execution quorum', async () => {
  ...
  const witnessProvider: NonNullable<Parameters<typeof authorizeIntent>[0]['witnessProvider']> = async ({ authorization: value, acceptance }) => {
