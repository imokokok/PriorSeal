...
test('a timestamp policy fails closed when no TSA provider is configured', async () => {
  ...
  await assert.rejects(() => authorizeIntent({ input: authorization, store, privateKeyPem: issuer, issuer: 'test', keyId: 'key-1', policy, requireTimestamp: true, now: () => 1_001_000 }), (error) => hasCode(error, 'TIMESTAMP_NOT_CONFIGURED'));
