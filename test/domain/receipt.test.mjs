// Generated from receipt.test.mts by npm run core:build. Do not edit directly.
...
const intent = buildIntent({ intentId: "i-1", chainId: 8453, action: "TRANSFER", asset: "eip155:8453/native", amount: "10", sender, recipient, validUntil: 2e9, nonce: "1" });
...
test("canonical JSON sorts objects but preserves arrays", () => {
  assert.equal(hashJson({ b: 2, a: 1 }), hashJson({ a: 1, b: 2 }));
  assert.notEqual(hashJson({ a: [1, 2] }), hashJson({ a: [2, 1] }));
  assert.equal(canonicalize({ a: null, b: "x", c: 2 }), '{"a":null,"b":"x","c":2}');
});
test("receipt verifies permanently and detects mutation", () => {
  const receipt = signReceipt(buildReceipt({ intent: { ...intent, validUntil: 101 }, execution, issuer: "test", keyId: "k1", issuedAt: 100 }), priv);
  assert.equal(verifyReceipt(receipt, pub, { keyId: "k1", now: 101 }).valid, true);
  assert.equal(verifyReceipt(receipt, pub, { keyId: "k1", now: 1e4 }).valid, true);
  assert.equal(verifyReceipt(receipt, pub, { key: { issuer: "test", keyId: "other", algorithm: "Ed25519", status: "active", validFrom: null, validUntil: null } }).code, "UNKNOWN_KEY");
  assert.equal(verifyReceipt(receipt, pub, { key: { issuer: "test", keyId: "k1", algorithm: "Ed25519", status: "active", validFrom: 101, validUntil: null } }).code, "KEY_NOT_YET_VALID");
  assert.equal(verifyReceipt({ ...receipt, outcome: "FAILED" }, pub, { keyId: "k1", now: 101 }).code, "INVALID_SIGNATURE");
  assert.equal(verifyReceipt({ ...receipt, executionHash: "bad" }, pub, { keyId: "k1", now: 101 }).code, "EXECUTION_HASH_MISMATCH");
});
test("intent fields bind to completed execution", () => {
  const { nonce: _nonce, ...missingNonce } = execution;
  assert.equal(buildReceipt({ intent, execution, issuer: "test" }).outcome, "COMPLETED");
  assert.equal(buildReceipt({ intent, execution: missingNonce, issuer: "test" }).outcome, "UNDETERMINED");
  assert.equal(buildReceipt({ intent, execution: { ...execution, recipient: `0x${"c".repeat(40)}` }, issuer: "test" }).outcome, "UNDETERMINED");
  assert.equal(buildReceipt({ intent, execution: { ...execution, status: "PENDING" }, issuer: "test" }).outcome, "PENDING");
});
test("memory persistence rejects different signed evidence under the same receipt ID", async () => {
  const store = createMemoryStore();
  const receipt = signReceipt(buildReceipt({ intent, execution, issuer: "test", keyId: "k1", issuedAt: 100 }), priv);
  await store.saveReceipt(receipt);
  await assert.rejects(() => store.saveReceipt({ ...receipt, validUntil: receipt.validUntil + 1 }), (error) => hasCode(error, "RECEIPT_ID_CONFLICT"));
...
test("receipt bytes and Ed25519 signature match the pre-migration vector", () => {
  ...
  const vectorIntent = buildIntent({ intentId: "receipt-migration-vector", chainId: 8453, action: "TRANSFER", asset: "eip155:8453/native", amount: "10", sender, recipient, validUntil: 2e9, nonce: "1" });
