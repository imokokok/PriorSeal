// Generated from intent-policy.test.mts by npm run core:build. Do not edit directly.
...
test("policy permits an allowlisted intent", () => {
  const result = evaluateIntentPolicy(intent, { policyId: "treasury-v1", allowedChainIds: [8453], allowedActions: ["TRANSFER"], allowedAssets: ["native"], allowedRecipients: ["0xbb"], maxAmount: "1000", maxValiditySeconds: 200 }, 1e3);
  assert.equal(result.allowed, true);
  assert.deepEqual(result.reasonCodes, []);
});
test("policy rejects with stable reason codes", () => {
  const result = evaluateIntentPolicy(intent, { allowedChainIds: [1], allowedRecipients: ["0xcc"], maxAmount: "10" }, 1e3);
  assert.equal(result.allowed, false);
  assert.deepEqual(result.reasonCodes, ["POLICY_CHAIN_NOT_ALLOWED", "POLICY_RECIPIENT_NOT_ALLOWED", "POLICY_AMOUNT_EXCEEDED"]);
});
test("policy can require a signed minimum confirmation threshold", () => {
  assert.deepEqual(evaluateIntentPolicy({ ...intent, constraints: { minConfirmations: 2 } }, { minConfirmations: 12 }, 1e3).reasonCodes, ["POLICY_MIN_CONFIRMATIONS_REQUIRED"]);
  assert.equal(evaluateIntentPolicy({ ...intent, constraints: { minConfirmations: 12 } }, { minConfirmations: 12 }, 1e3).allowed, true);
});
test("authorization policy binds a reviewed principal identity to its account type", () => {
  ...
  assert.equal(evaluateAuthorizationPolicy(authorization, policy, 1e3).allowed, true);
  assert.deepEqual(evaluateAuthorizationPolicy({ ...authorization, principal: { ...authorization.principal, id: "lookalike" } }, policy, 1e3).reasonCodes, ["POLICY_PRINCIPAL_NOT_ALLOWED"]);
});
test("authorization policy can require the authorizer and executor to be distinct", () => {
  ...
  assert.equal(evaluateAuthorizationPolicy(authorization, policy, 1e3).allowed, true);
  assert.deepEqual(
    evaluateAuthorizationPolicy({ ...authorization, delegate: { agentId: "agent", executor: authorizer } }, policy, 1e3).reasonCodes,
    ["POLICY_AUTHORIZER_EXECUTOR_NOT_DISTINCT"]
  );
});
test("runtime policy evaluation fails closed on malformed restrictions", () => {
  assert.deepEqual(evaluateIntentPolicy(intent, { allowedRecipients: "0xcc" }, 1e3).reasonCodes, ["POLICY_INVALID"]);
  assert.deepEqual(evaluateAuthorizationPolicy({ intent, principal: { id: "user", type: "user", account: "0xaa" }, authorizer: { type: "eip712" } }, { principals: "everyone" }, 1e3).reasonCodes, ["POLICY_INVALID"]);
  assert.deepEqual(evaluateIntentPolicy(intent, { requireDistinctAuthorizerAndExecutor: "yes" }, 1e3).reasonCodes, ["POLICY_INVALID"]);
});
