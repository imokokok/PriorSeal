...
const errorCodeIs = (error: unknown, code: string): boolean => error instanceof Error && 'code' in error && error.code === code;
...
test('new issuance rejects contradictory executor identity without changing historical verification', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => errorCodeIs(error, 'INVALID_AUTHORIZATION'),
  );
});
...
test('ERC-1271 verifier availability failures remain distinguishable from invalid signatures', async () => {
  ...
  const unavailable = await verifyAuthorization(authorization, { now: 1_001, verifyContractSignature: async () => { const error: Error & { code?: string } = new Error('RPC unavailable'); error.code = 'AUTHORIZATION_VERIFIER_UNAVAILABLE'; throw error; } });
...
test('accepted authorization is single-use and produces a self-checking v3 receipt while v2 remains verifiable', async () => {
  ...
  assert.equal((await verifyAuthorizedReceipt(receipt, publicKeyPem, { key: { keyId: 'key-1', issuer: 'test', algorithm: 'Ed25519', publicKey: publicKeyPem, status: 'retired', validFrom: 1_050, validUntil: null } })).code, 'KEY_NOT_YET_VALID');
  ...
  (extendedAuthorization.authorizationEvidence.authorization as Record<string, unknown>).unsignedPrivilege = true;
...
test('v3 receipt embeds and verifies the principal identity policy snapshot', async () => {
  ...
  const receipt = signReceipt(buildAuthorizedReceipt({ authorization: accepted.response.authorization, acceptance: accepted.response.acceptance, policyEvidence: accepted.response.policyEvidence as Parameters<typeof buildAuthorizedReceipt>[0]['policyEvidence'], execution, issuer: 'test', keyId: 'key-1', issuedAt: 1_101 }), privateKeyPem);
  const embeddedPolicy = receipt.authorizationEvidence.policy.document as typeof policy;
  ...
  (mutated.authorizationEvidence.policy.document as typeof policy).principals[0].id = 'lookalike';
...
test('required transparency failure does not consume the authorization', async () => {
  ...
  const observer: Parameters<typeof observeExecution>[0]['observer'] = async () => normalizeExecution({ chainId: 8453, txHash, status: 'CONFIRMED', action: 'TRANSFER', sender: executor, recipient: intentInput.recipient, asset: intentInput.asset, amount: intentInput.amount, nonce: intentInput.nonce, executedAt: 1_100, observedAt: 1_101, confirmations: 12, gasUsed: '21000', transfers: [], finalityState: 'CONFIRMED' });
  let before: number | undefined;
  await assert.rejects(() => observeExecution({ input: { authorizationId: authorization.authorizationId, chainId: 8453, txHash, confirmations: 12 }, store, observer, issuer: 'test', keyId: 'key-1', transparencyProvider: async (_acceptance, options) => { before = options?.before; const error: Error & { code?: string } = new Error('anchor required'); error.code = 'TRANSPARENCY_ANCHOR_REQUIRED'; throw error; } }), (error: unknown) => errorCodeIs(error, 'TRANSPARENCY_ANCHOR_REQUIRED'));
...
test('legacy authorization schemas cannot be newly accepted', async () => {
  ...
  await assert.rejects(() => authorizeIntent({ input: legacy, store: createMemoryStore(), privateKeyPem, issuer: 'test', keyId: 'key-1', now: () => 1_001_000 }), (error: unknown) => errorCodeIs(error, 'INVALID_AUTHORIZATION'));
});
...
test('authorization nonce uniqueness and atomic acceptance are enforced in memory', async () => {
  ...
  await assert.rejects(() => authorizeIntent({ input: second, store, privateKeyPem, issuer: 'test', keyId: 'key-1', now: () => 1_001_000 }), (error: unknown) => errorCodeIs(error, 'AUTHORIZATION_NONCE_REUSED'));
