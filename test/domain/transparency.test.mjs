// Generated from transparency.test.mts by npm run core:build. Do not edit directly.
...
const mockRpcClient = (handler) => ({
  async call(url, method, params) {
    return await handler(url, method, params);
  }
});
const logEntry = (sequence, authorizationHash, acceptedAt, previousEntryHash) => {
...
test("Merkle checkpoints provide a bounded inclusion proof and bind a root anchor", async () => {
  ...
  assert.ok(Buffer.byteLength(JSON.stringify(evidence)) < 2e3);
...
test("an external anchor is accepted only when its successful on-chain call matches", async () => {
  ...
  const anchor = { type: "eip155", chainId: 8453, contract, txHash, blockNumber: 100, anchoredAt: 1e3, size: 7, headEntryHash };
