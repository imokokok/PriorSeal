// Generated from joint-review-fixture.mts by npm run core:build. Do not edit directly.
...
async function pretrade(source = true, overrides = {}) {
  const data = {
    ...
    schemaVersion: 3,
    verdict: "PASS",
    sourceAssetId: source ? sourceAssetId : destinationAssetId,
    destinationAssetId: source ? destinationAssetId : sourceAssetId,
    subjectChainId: 8453,
    action: "swap",
    tradeAmountUsd: 1e11,
    consensusPrice: source ? 1e8 : 25e10,
    checkedAt: 900,
    validUntil: 1600,
    participantCount: 3,
    ...
    requestHash: `0x${(source ? "2" : "3").repeat(64)}`,
    ...overrides
  };
  return signInsight(data, V3_DOMAIN, V3_TYPES, V3_PRIMARY_TYPE);
}
async function createJointReviewFixture({ priorChain = 8453, c4Overrides = {}, sourceOverrides = {}, destinationOverrides = {}, c4Version = 5, withAnchor = false, psExecutedAt = 1100, commitmentOverride } = {}) {
  ...
  const data = {
    ...defaultData(types[EXECUTION_PRIMARY_TYPE]),
    schemaVersion: c4Version,
    ...
    preTradeUidsHash: keccak256(concat([source.uid, destination.uid])),
    ...
    blockNumber: 100,
    executedAt: 1100,
    preTradeSignedAt: 900,
    attestationAgeAtExecSeconds: 200,
    participantCount: 3,
    requiredParticipantCount: 3,
    sourceGroupCount: 2,
    requiredSourceGroupCount: 2,
    ...
    ...c4Version === 5 ? { profileId: EXECUTION_PROFILE_V1_ID } : {},
    ...typeof c4Overrides === "function" ? c4Overrides(source, destination) : c4Overrides
  };
  ...
  const draft = buildAuthorization({ intent, principal: { type: "user", id: "fixture-user", account: authorizer.address }, authorizer: { type: "eip712", address: authorizer.address }, delegate: { agentId: "fixture-agent", executor }, issuedAt: 1e3, notBefore: 1e3, expiresAt: 1500, authorizationNonce: `0x${"9".repeat(64)}`, maxUses: "1", audience: "joint-review-test", policyHash: zeroHash });
  ...
  const accepted = await authorizeIntent({ input: authorization, store, privateKeyPem: privateKey, issuer: key.issuer, keyId: key.keyId, audience: "joint-review-test", now: () => 1001e3 });
  ...
  const transparency = withAnchor ? buildTransparencyEvidence({ entries, acceptance: accepted.response.acceptance, issuer: key.issuer, keyId: key.keyId, privateKeyPem: privateKey, issuedAt: 1099, before: psExecutedAt, anchor: { type: "eip155", chainId: priorChain, contract: router, txHash: `0x${"8".repeat(64)}`, blockNumber: 99, anchoredAt: 1099, size: 1, headEntryHash: entries[0].entryHash } }) : void 0;
  ...
  const insightKeyRegistry = { keys: [{ key_id: "fixture-insight-key", public_key: insightSigner.address, validFrom: (/* @__PURE__ */ new Date(0)).toISOString(), validUntil: null, revoked: false, role: "attester" }] };
