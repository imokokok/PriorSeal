// Generated from checkpoint.test.mts by npm run core:build. Do not edit directly.
...
const hasCode = (error, code) => record(error) && error.code === code;
const hasDetails = (error, code, predicate) => {
...
const isCheckpoint = (value) => record(value) && value.schema === "priorseal.authorization-checkpoint.v1" && ["PREPARED", "SIGNED", "ACCEPTED"].includes(String(value.stage)) && typeof value.account === "string" && record(value.request) && record(value.prepared) && typeof value.acceptIdempotencyKey === "string";
const typedDataMessage = (checkpoint) => {
...
function setup({ expire = false } = {}) {
  ...
  const client = createPriorSealClient({ fetch: async (url, init) => {
    ...
    if (String(url).endsWith("/prepare")) {
      prepares++;
      const authorization2 = buildAuthorization(body);
      return new Response(JSON.stringify({ authorization: authorization2, typedData: authorizationTypedData(authorization2) }, (_key, value) => typeof value === "bigint" ? value.toString() : value), { headers: { "content-type": "application/json" } });
    }
  ...
  const provider = { request: async ({ params }) => {
    signs++;
...
test("signed checkpoint resumes lost acceptance with the same bytes and idempotency key, without another wallet signature", async () => {
  const f = setup();
  let checkpoint;
  await assert.rejects(f.client.authorizeWithWallet(f.input, f.provider, { onCheckpoint: (value) => {
    checkpoint = value;
  } }), (error) => hasDetails(error, "NETWORK_ERROR", (details) => record(details.checkpoint) && details.checkpoint.stage === "SIGNED"));
  assert.ok(checkpoint);
  const roundTripValue = JSON.parse(JSON.stringify(checkpoint));
...
test("expired prepare never opens wallet signing and observation timeout preserves a resumable job", async () => {
  ...
  const job = { jobId: "job-1", input: { authorizationId: "auth-1", txHash: `0x${"1".repeat(64)}` }, state: "RETRY_WAIT", nextAttemptAt: Date.now() + 6e4 };
...
const checkpointMutations = [
  ["audience", (state) => {
    state.prepared.authorization.audience = "other-deployment";
  }],
  ["expiry", (state) => {
    state.prepared.authorization.expiresAt -= 1;
  }],
  ["notBefore", (state) => {
    state.prepared.authorization.notBefore += 1;
  }],
  ["issuedAt", (state) => {
    state.prepared.authorization.issuedAt -= 1;
  }],
  ["nonce", (state) => {
    state.prepared.authorization.authorizationNonce = `0x${"5".repeat(64)}`;
  }],
  ["uses", (state) => {
    state.prepared.authorization.maxUses = "2";
  }],
  ["type", (state) => {
    state.prepared.authorization.authorizer.type = "eip1271";
  }],
  ["typed data", (state) => {
    typedDataMessage(state).audience = "other-deployment";
  }],
  ["opaque intent hash", (state) => {
    state.prepared.authorization.intentHash = "6".repeat(64);
  }],
  ["stage/signature contradiction", (state) => {
...
test("checkpoint with a different valid signer signature never reaches acceptance", async () => {
  ...
  checkpoint.signature = await otherSigner.signTypedData(checkpoint.prepared.typedData);
