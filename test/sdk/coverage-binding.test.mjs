// Generated from coverage-binding.test.mts by npm run core:build. Do not edit directly.
...
const key = privateKeyToAccount(`0x${"12".repeat(32)}`), now = 18e8;
...
async function requirements() {
  const report = buildCoverageReport({
    ...
    evidenceChainId: 1,
    evaluatedAt: now,
    observations: ["chainlink", "api3", "twap"].map((provider) => ({ provider, evidenceChainId: 1, price: 1, status: "success", observedAt: now - 10, retrievedAt: now, timestampProvenance: "provider_timestamp", excluded: false }))
  }, policy);
...
for (const mode of ["missing", "duplicate", "wrong-digest", "expired", "revoked", "extended-lifetime", "unsigned"]) test(`does not enter provider: ${mode}`, async () => {
  ...
  let called = false;
  await assert.rejects(withCoverageBoundIntent(intent, req, async () => {
    called = true;
  }, () => mode === "expired" ? now + 60 : now));
