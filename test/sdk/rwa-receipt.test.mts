...
async function fixture(): Promise<any>{
  ...
  const f: any=rwaFixture(sdk),authority=await signRwaFixture(sdk,account,f),execution=await signRwaFixture(sdk,account,rwaFixture(sdk,f.now+2));
  ...
  const observed: any={chainId:8453,txHash:'0x'+'66'.repeat(32),status:'CONFIRMED',action:'CONTRACT_CALL',sender:f.transaction.from,recipient:f.transaction.to,target:f.transaction.to,calldataHash:f.input.request.call.calldataHash,nativeValue:'0',asset:intent.asset,amount:intent.amount,nonce:intent.nonce,executedAt:f.now+2,observedAt:f.now+3,confirmations:12,gasUsed:'100000',transfers:[],finalityState:'CONFIRMED'};
