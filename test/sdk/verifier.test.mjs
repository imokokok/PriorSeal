...
test('SDK verifier independently validates authorization-bound receipt v3, policy, compliance and signatures', async () => {
  ...
  lateAnchorReceipt.authorizationEvidence.transparency.checkpoint.signature = sign(null, Buffer.from(canonicalize(unsignedCheckpoint)), keys.privateKey).toString('base64url');
...
test('SDK verifier independently validates exact-call receipts with multi-transfer execution', async () => {
  ...
  const execution = { chainId: 8453, txHash: `0x${'6'.repeat(64)}`, status: 'CONFIRMED', action: 'CONTRACT_CALL', sender: executor, recipient: `0x${'d'.repeat(40)}`, target: router, calldataHash, nativeValue: '0', asset: 'eip155:8453/erc20:0x2222222222222222222222222222222222222222', amount: '999', nonce: '7', executedAt: 1_100, observedAt: 1_101, confirmations: 12, gasUsed: '180000', transfers: [{}, {}], finalityState: 'CONFIRMED' };
