...
type MutableFixture = { proof: any; trust: any };
type FixtureOptions = {
  version?: number;
...
async function expectCode(proof: any, trust: any, expected: string) {
...
test('real v5 signature and pinned production protocol pass as separate checks', async (t: any) => {
...
test('trust windows and revocation are evaluated at signed execution time', async () => {
  ...
  for (const value of ['invalid', undefined]) {
    const changed = clone(trust);
    changeSnapshot(changed, (raw: any) => {
...
test('a valid native nonproduction signature cannot satisfy production protocol trust', async (t: any) => {
  t.mock.method(Date, 'now', () => 1200000);
...
test('candidate equal to floor is admitted only when its exact body is available', async () => {
  ...
  assert.equal((await evaluateInsightProtocolTrust(proof, trust)).valid, true);
  trust.registryReleases = trust.registryReleases.filter((entry: any) => entry.releaseId !== releaseId);
...
test('immutable policy pins/hash/reachability cannot be relabelled by the caller', async () => {
  ...
  changePolicy(disabled, (body: any) => {
