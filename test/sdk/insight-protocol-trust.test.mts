...
const differentId = `0x${'f'.repeat(64)}` as `0x${string}`;
const clone = <T,>(value: T): T => structuredClone(value);
type Proof = Awaited<ReturnType<typeof createSignedExecution>>;
type MutableFixture = { proof: Proof; trust: InsightProtocolTrust };
type RegistrySnapshot = Record<string, unknown> & {
  schemas: Record<string, unknown> & { ExecutionReceipt: {
    eip712: { domain: Record<string, unknown>; types: { ExecutionReceipt: unknown[] } };
    semanticProfile?: { profileId: string };
  } };
  public_keys: Array<{ key_id: string; public_key: string; role?: string; revoked?: boolean; validFrom: string; validUntil?: string | null }>;
  revoked_keys: Array<{ key_id: string }>;
  registryRelease: { releaseId: string };
};
type ReleaseBody = { schemaCatalog: { ExecutionReceipt: { profileId?: string; domain: Record<string, unknown> } } };
type PolicyBody = Record<string, unknown>;
type FixtureOptions = {
  version?: number;
  legacyWithoutRelease?: boolean;
  withPolicy?: boolean;
  overrides?: Record<string, unknown>;
};

async function fixture(options: FixtureOptions = {}): Promise<MutableFixture> {
  return {
    proof: await createSignedExecution({ signer, ...options }),
    trust: createInsightProtocolTrust({ signer, ...options }),
  };
}
function changeSnapshot(trust: InsightProtocolTrust, mutate: (snapshot: RegistrySnapshot) => void) {
  const snapshot = JSON.parse(trust.registrySnapshot.rawJson) as RegistrySnapshot;
  mutate(snapshot);
  trust.registrySnapshot = snapshotPin(snapshot);
}
function unpinPolicy(trust: InsightProtocolTrust) {
  delete trust.consumerPolicy.policyId;
  delete trust.consumerPolicy.policyRawJson;
}
function changePolicy(trust: InsightProtocolTrust, mutate: (policy: PolicyBody) => void) {
  const { policyId: _ignored, ...body } = JSON.parse(trust.consumerPolicy.policyRawJson ?? '{}') as PolicyBody;
  mutate(body);
...
async function expectCode(proof: Proof, trust: InsightProtocolTrust | undefined, expected: string) {
...
test('real v5 signature and pinned production protocol pass as separate checks', async (t: TestContext) => {
...
test('protocol verification does not claim signature validity; every caller must check both', async () => {
  const { proof, trust } = await fixture();
  proof.signature = `0x${'0'.repeat(130)}` as `0x${string}`;
...
test('absent trust/snapshot and independently mismatched hash or byte count fail closed', async () => {
  ...
  delete (missing as Partial<InsightProtocolTrust>).registrySnapshot;
  await expectCode(proof, missing, 'REGISTRY_SNAPSHOT_REQUIRED');
  for (const mutate of [
    (t: InsightProtocolTrust) => {
...
test('SHA-256 covers exact UTF-8 bytes including non-ASCII metadata', async () => {
  const { proof, trust } = await fixture();
  changeSnapshot(trust, (raw: RegistrySnapshot) => {
    raw.note = '离线证据';
  });
...
test('trust windows and revocation are evaluated at signed execution time', async () => {
  ...
  for (const value of ['invalid', undefined]) {
    const changed = clone(trust);
    changeSnapshot(changed, (raw: RegistrySnapshot) => {
...
test('a valid native nonproduction signature cannot satisfy production protocol trust', async (t: TestContext) => {
  t.mock.method(Date, 'now', () => 1200000);
...
test('release must bind the known v5 profile and matching schema descriptor', async () => {
  ...
  for (const [mutate, code] of [
  ...
  ] as Array<[(body: ReleaseBody) => void, string]>) {
    const changed = clone(trust);
    ...
    await expectCode(proof, changed, code as string);
  }
});
...
test('immutable policy pins/hash/reachability cannot be relabelled by the caller', async () => {
  ...
  changePolicy(disabled, (body: PolicyBody) => {
