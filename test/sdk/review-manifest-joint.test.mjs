...
import { createJointReviewFixture as fixture, zeroHash, router } from '../helpers/joint-review-fixture.mjs';
...
for (const [name, mutate] of [
  ['registry raw bytes', trust => { trust.registrySnapshot.rawJson += ' '; }],
  ['registry byte length', trust => { trust.registrySnapshot.byteLength++; }],
  ['missing lineage body', trust => { trust.registryReleases.pop(); }],
  ['changed immutable profile', trust => { trust.executionProfiles[0].rawJson = '{}'; }],
  ['unadmitted execution schema', trust => { trust.consumerPolicy.allowedSchemaVersions = [4]; }],
  ['mismatched immutable policy pins', trust => { trust.consumerPolicy.allowedProfileIds = [zeroHash]; }],
]) {
