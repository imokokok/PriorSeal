...
test('two simultaneous callers with independent store handles cannot broadcast twice',async t=>{
  ...
  assert.equal(x.calls,1,JSON.stringify(results));assert.ok(results.some(r=>r.status==='fulfilled'));
});
test('lost RPC response remains uncertain after restart; observer reconciles without resend',async t=>{
  const x=await setup(t);
  await assert.rejects(executeRwaAuthorized(x.input,{...x.deps,submit:async()=>{x.calls++;throw Error('response lost')}}),/response lost/);
...
test('new authorization cannot reuse claimed chain/sender/nonce',async t=>{
  ...
  assert.equal(claim.claimed,false);assert.equal(claim.code,'RWA_NONCE_ALREADY_RESERVED');
});
...
test('trust-key permutations and casing do not change v2 pair admission',async t=>{
  ...
  x.input.authority.trust.keys.push(extra);x.input.execution.trust.keys.unshift(extra);
