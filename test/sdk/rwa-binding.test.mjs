...
test('opt-in binding preserves other commitments, caps expiry, and executes exact call',async()=>{
  const x=await setup();assert.equal(x.intent.contextCommitments.length,2);assert.equal(x.intent.validUntil,x.f.now+30);
  let call;await sdk.withRwaBoundIntent(x.intent,x.authority,x.f.transaction,async tx=>call=tx,()=>x.f.now);
...
const changes={
  ...
  'duplicate commitment':x=>x.intent.contextCommitments.push(x.intent.contextCommitments.find(c=>c.namespace===sdk.RWA_COMMITMENT_NAMESPACE)),
  'changed commitment':x=>x.intent.contextCommitments.find(c=>c.namespace===sdk.RWA_COMMITMENT_NAMESPACE).digest='0x'+'00'.repeat(32),
  ...
  'untrusted proof':x=>x.authority.trust.keys[0].revoked=true,
};
...
for(const mode of ['same-report','halt','stale','wrong-pins','changed-transaction','expiry-during-verification'])test('fresh execution guard rejects '+mode,async()=>{
  ...
  if(mode==='halt'){const f=rwaFixture(sdk,x.f.now+2);f.input.market.halt='HALTED';x.execution=await signRwaFixture(sdk,key,f);x.execution.trust=x.authority.trust;}
