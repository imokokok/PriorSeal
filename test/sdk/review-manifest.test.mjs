// Generated from review-manifest.test.mts by npm run core:build. Do not edit directly.
...
function fixture() {
  ...
  const intent = buildIntent({ intentId: "review-test", chainId: 8453, action: "TRANSFER", asset: "eip155:8453/native", amount: "10", sender: `0x${"a".repeat(40)}`, recipient: `0x${"b".repeat(40)}`, validUntil: 2e3, nonce: "1" });
  ...
  const bundle = buildVerificationBundle({ receipt, keyRegistry: { schema: "priorseal.keys.v1", issuer: "test", keys: [key] }, assembledAt: 1101 });
  return { key, bundle };
}
...
test("unknown external proof, missing Insight evidence and changed raw bytes cannot become complete verification", async () => {
  ...
  assert.equal(attachment.rawJson, rawJson);
  ...
  const tamperedAttachment = tampered.attachments[0];
  assert.ok(tamperedAttachment);
  tamperedAttachment.rawJson = '{"signed":"exact bytes"}';
...
test("trust profile rejects ambiguous key identity and invalid windows while preserving custom audience", () => {
  ...
  assert.throws(() => parseTrustProfile({ ...profile, keys: [{ ...key, validFrom: 2e3, validUntil: 1e3 }] }), /window/);
});
test("Insight trust follows the published default attester role and rejects ambiguous registries", () => {
  ...
  assert.equal(parseTrustProfile({ ...profile, insightKeyRegistry: { keys: [{ ...insightKey, role: void 0 }] } }).insightKeyRegistry?.keys?.[0]?.role, void 0);
