...
const key=privateKeyToAccount(`0x${'12'.repeat(32)}` as `0x${string}`);
...
const rwaCommitment = (value: Setup) => {
...
test('opt-in binding preserves other commitments, caps expiry, and executes exact call',async()=>{
  const x=await setup();assert.equal(commitments(x).length,2);assert.equal(x.intent.validUntil,x.f.now+30);
  let call: sdk.ExactCallTransaction | undefined;await sdk.withRwaBoundIntent(x.intent,x.authority,x.f.transaction,async tx=>{call=tx;},()=>x.f.now);
  assert.deepEqual(call,x.f.transaction);
  assert.equal((await sdk.verifyRwaExecutionPair({...x,executionTime:x.f.now+2})).valid,true);
});
const changes: Record<string, (value: Setup) => void>={
  'missing commitment':x=>{x.intent.contextCommitments=[];},
  'duplicate commitment':x=>{commitments(x).push(structuredClone(rwaCommitment(x)));},
  'changed commitment':x=>{rwaCommitment(x).digest=`0x${'00'.repeat(32)}`;},
  ...
  'untrusted proof':x=>{const trustKey=x.authority.trust.keys[0];if(!trustKey)throw new TypeError('Expected trust key');trustKey.revoked=true;},
};
for(const [name,change] of Object.entries(changes))test('callback never entered: '+name,async()=>{
  const x=await setup();change(x);let called=false;
...
test('snapshot prevents caller mutation during async verification',async()=>{
  const x=await setup();let data: `0x${string}` | undefined;
  const task=sdk.withRwaBoundIntent(x.intent,x.authority,x.f.transaction,async tx=>{data=tx.data;},()=>x.f.now);
  x.f.transaction.data='0xffff';await task;assert.equal(data,'0x1234');
});
test('signature verification crossing expiry cannot enter callback',async()=>{
  const x=await setup();let reads=0,called=false;
...
for(const mode of ['same-report','halt','stale','wrong-pins','changed-transaction','expiry-during-verification'])test('fresh execution guard rejects '+mode,async()=>{
  ...
  if(mode==='halt'){const f=rwaFixture(sdk,x.f.now+2);if(!f.input.market)throw new TypeError('Expected market state');f.input.market.halt='HALTED';x.execution=await signRwaFixture(sdk,key,f);x.execution.trust=x.authority.trust;}
  ...
  const clock=()=>{reads++;return mode==='stale'||(mode==='expiry-during-verification'&&reads>1)?x.f.now+32:x.f.now+2};
