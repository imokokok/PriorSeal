...
function trustedKey(publicKey: string) {
...
test('SDK verifier validates a v1 receipt locally and detects mutations', async () => {
  ...
  const tamperedExecution = tampered.receipt.execution as unknown as Record<string, unknown>;
...
test('SDK verifier independently validates authorization-bound receipt v3, policy, compliance and signatures', async () => {
  ...
  const execution = { chainId: 8453, txHash: `0x${'3'.repeat(64)}`, status: 'CONFIRMED' as const, action: 'TRANSFER', sender: executor, recipient: intent.recipient, asset: intent.asset, amount: intent.amount, nonce: intent.nonce, executedAt: 1_100, observedAt: 1_101, confirmations: 12, gasUsed: '21000', transfers: [], finalityState: 'CONFIRMED' };
  ...
  const transparency = buildTransparencyEvidence({ entries, acceptance: accepted.response.acceptance, issuer: 'test', keyId: 'key-1', privateKeyPem: keys.privateKey, issuedAt: 1_099, anchor, before: execution.executedAt });
  const receipt = signReceipt(buildAuthorizedReceipt({ authorization: accepted.response.authorization, acceptance: accepted.response.acceptance, policyEvidence: accepted.response.policyEvidence, transparency, execution, issuer: 'test', keyId: 'key-1', issuedAt: 1_101 }), keys.privateKey);
  ...
  const checkpoint = lateAnchorReceipt.authorizationEvidence.transparency.checkpoint as Record<string, unknown>;
  ...
  const extendedAuthorizationValue = extendedAuthorization.authorizationEvidence.authorization as unknown as Record<string, unknown>;
...
test('SDK verifier enforces the expected authorization audience', async () => {
  ...
  const execution = { chainId: 8453, txHash: `0x${'7'.repeat(64)}`, status: 'CONFIRMED' as const, action: 'TRANSFER', sender: executor, recipient: intent.recipient, asset: intent.asset, amount: intent.amount, nonce: intent.nonce, executedAt: 1_100, observedAt: 1_101, confirmations: 12, gasUsed: '21000', transfers: [], finalityState: 'CONFIRMED' };
...
test('SDK verifier independently validates exact-call receipts with multi-transfer execution', async () => {
  ...
  const execution = { chainId: 8453, txHash: `0x${'6'.repeat(64)}`, status: 'CONFIRMED' as const, action: 'CONTRACT_CALL', sender: executor, recipient: `0x${'d'.repeat(40)}`, target: router, calldataHash, nativeValue: '0', asset: 'eip155:8453/erc20:0x2222222222222222222222222222222222222222', amount: '999', nonce: '7', executedAt: 1_100, observedAt: 1_101, confirmations: 12, gasUsed: '180000', transfers: [{}, {}], finalityState: 'CONFIRMED' };
