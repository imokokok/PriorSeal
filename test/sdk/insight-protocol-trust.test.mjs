// Generated from insight-protocol-trust.test.mts by npm run core:build. Do not edit directly.
...
test("native v5 rejects tampered UID, environment, profile and transaction fields", async () => {
  const { proof, trust } = await fixture();
  const options = { keyRegistry: JSON.parse(trust.registrySnapshot.rawJson) };
...
test("absent trust/snapshot and independently mismatched hash or byte count fail closed", async () => {
  const { proof, trust } = await fixture();
  await expectCode(proof, void 0, "INSIGHT_PROTOCOL_TRUST_REQUIRED");
...
test("trust windows and revocation are evaluated at signed execution time", async () => {
  ...
  for (const mutate of [
    ...
    (raw) => {
      raw.public_keys[0].validFrom = (/* @__PURE__ */ new Date(1101e3)).toISOString();
    },
    (raw) => {
      raw.public_keys[0].validUntil = (/* @__PURE__ */ new Date(1099e3)).toISOString();
    }
  ]) {
    const changed = clone(trust);
    changeSnapshot(changed, mutate);
  ...
  for (const value of ["invalid", void 0]) {
    const changed = clone(trust);
