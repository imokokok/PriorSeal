// Generated from cloudflare-bindings.test.mts by npm run core:build. Do not edit directly.
...
test("Cloudflare runtime requires production bindings and uses the Git version tag", () => {
  ...
  assert.throws(() => assertCloudflareBindings(bindings({ HTTP_RATE_LIMITER: void 0 })), /HTTP_RATE_LIMITER/);
  assert.throws(() => assertCloudflareBindings(bindings({ DB: void 0 })), /DB D1 binding/);
  assert.throws(() => assertCloudflareBindings(bindings({ CF_VERSION_METADATA: void 0 })), /CF_VERSION_METADATA/);
});
test("Cloudflare rate limiter awaits the shared binding and fails closed", async () => {
  const keys = [];
  const limiter = createCloudflareRateLimiter({ async limit(input) {
    keys.push(input.key);
  ...
  await assert.rejects(() => createCloudflareRateLimiter({ async limit() {
    throw new Error("binding failed");
  } }).allow("principal-1"), (error) => typeof error === "object" && error !== null && "code" in error && error.code === "RATE_LIMITER_UNAVAILABLE");
});
