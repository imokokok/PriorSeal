// Generated from evidence-archive.test.mts by npm run core:build. Do not edit directly.
...
test("archive credentials bind project, environment and role; malformed and duplicate credentials fail closed", () => {
  ...
  for (const value of [void 0, null, 123, true, {}, []]) {
...
test("delimiter-bearing tenant identifiers cannot alias another archive namespace", async () => {
  ...
  assert.equal(await store.getArchiveEntry(otherTenant, saved.id), void 0);
...
test("private archive isolates same artifact by project, keeps immutable history and pages a stable snapshot", async () => {
  ...
  await store.saveArchiveEntry(archiveEntry(artifact(2), a, { now: 2e3, supersedesId: first.id }));
...
test("capabilities never equate an active issuer with workflow readiness and suppress incompatible exact-call policy", async () => {
  const base = { issuer: "test", audience: "test-env", keyConfigured: true, policy: { allowedChainIds: [8453], allowedAssets: ["eip155:8453/native"] }, proofMode: "rfc3161", timestampConfigured: false, witnessConfigured: false, anchorConfigured: false, rpcChainIds: [8453], store: createMemoryStore(), now: 1e3, archiveEnabled: false };
  ...
  assert.equal((await deploymentCapabilities({ ...base, timestampConfigured: true, store: { health: async () => {
    throw new Error("down");
  } } })).dependencies.storage, "unavailable");
});
test("capability action matching follows the case-insensitive policy evaluator", async () => {
  const base = { issuer: "test", audience: "test-env", keyConfigured: true, proofMode: "issuer", timestampConfigured: false, witnessConfigured: false, anchorConfigured: false, rpcChainIds: [8453], store: createMemoryStore(), now: 1e3, archiveEnabled: false };
