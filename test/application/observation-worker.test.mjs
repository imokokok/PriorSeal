// Generated from observation-worker.test.mts by npm run core:build. Do not edit directly.
...
test("observation worker is idempotent and retries pending observations", async () => {
  ...
  const first = worker.enqueue({ chainId: 8453, txHash: "0x1", confirmations: 2, idempotencyKey: "same" });
  assert.equal(worker.enqueue({ chainId: 8453, txHash: "0x1", idempotencyKey: "same" }).jobId, first.jobId);
  await worker.runOnce();
  assert.equal((await worker.get(first.jobId))?.state, "RETRY_WAIT");
  now += 10;
  await worker.runOnce();
  assert.equal((await worker.get(first.jobId))?.state, "COMPLETED");
...
test("re-observation detects changed blocks and transactions removed by a reorg", () => {
  ...
  assert.equal(detectReorg({ txHash: "0x1", blockHash: "0xa" }, { txHash: "0x1", status: "NOT_FOUND", blockHash: null }), true);
  assert.equal(detectReorg({ txHash: "0x1", blockHash: "0xa" }, { txHash: "0x1", blockHash: "0xa" }), false);
});
test("persistent worker does not double-count atomically claimed attempts", async () => {
  let saved;
  const claimed = {
    jobId: "job-1",
    input: { chainId: 8453, txHash: "0x1" },
    state: "RUNNING",
    attempts: 1,
    createdAt: 0,
    nextAttemptAt: 0,
