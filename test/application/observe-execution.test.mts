...
async function acceptAuthorization({ intentId, nonce, store, privateKeyPem, keyId = 'key-1' }: { intentId: string; nonce: string; store: MemoryStore; privateKeyPem: string; keyId?: string }) {
...
test('pending transactions remain candidates and only final correlated execution claims authorization', async () => {
  ...
  const acceptance = buildAuthorizationReceipt({ authorization, issuer: 'test', acceptedAt: 1_001 });
  await store.saveAuthorization({ authorization, acceptance, policyEvidence: { schema: 'priorseal.policy-evidence.v1', policyHash: authorization.policyHash, document: null, result: { allowed: true, reasonCodes: [], policyId: null, evaluatedAt: 1_001 } }, status: 'ACCEPTED', boundTxHash: null, uses: 0 });
...
test('complete exact-call correlation is reported without reopening a final mismatched attempt', async () => {
  ...
  const acceptance = buildAuthorizationReceipt({ authorization, issuer: 'test', acceptedAt: 1_001 });
  await store.saveAuthorization({ authorization, acceptance, policyEvidence: { schema: 'priorseal.policy-evidence.v1', policyHash: authorization.policyHash, document: null, result: { allowed: true, reasonCodes: [], policyId: null, evaluatedAt: 1_001 } }, status: 'ACCEPTED', boundTxHash: null, uses: 0 });
  const finalObservation = (hash: string, overrides: Record<string, unknown> = {}) => ({ chainId: 8453, txHash: hash, intentHash: authorization.intentHash, status: 'CONFIRMED', executionDataAvailable: true, action: 'CONTRACT_CALL', sender, recipient: callTarget, target: callTarget, calldataHash, nativeValue: '3', nonce: '9', executedAt: 1_100, observedAt: 1_101, confirmations: 12, finalityState: 'CONFIRMED', ...overrides });
...
test('signed confirmation constraints cannot be relaxed by the observer request', async () => {
  const store = createMemoryStore(); let confirmations: number | undefined;
...
test('observation idempotency rejects a reused key with different input', async () => {
  ...
  await store.saveIntent(intent);
  const observer = async ({ txHash: hash }: { txHash: string }) => ({ chainId: 8453, txHash: hash, status: 'PENDING', action: 'TRANSFER', observedAt: 1, sender, recipient, asset: intent.asset, amount: intent.amount, finalityState: 'PENDING' });
...
test('intent rejects impossible confirmation floors and cross-chain assets', () => {
  ...
  assert.throws(() => buildIntent({ ...base, constraints: { minConfirmations: 10_001 } }), (error) => hasCode(error, 'INVALID_CONSTRAINT'));
