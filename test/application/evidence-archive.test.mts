...
import { createArchiveAccess, archiveEntry, archiveQuery, type ArchiveAccess } from '../../src/application/archive/evidence-archive.mjs';
...
const a: ArchiveAccess = { projectId: 'alpha', environment: 'prod', role: 'writer' };
...
const query = (url: string, access: ArchiveAccess = a) => archiveQuery(new URL(url, 'https://example.test'), access);
...
test('delimiter-bearing tenant identifiers cannot alias another archive namespace', async () => {
  ...
  const firstTenant: ArchiveAccess = { projectId: 'a:b', environment: 'c', role: 'writer' };
  const otherTenant: ArchiveAccess = { projectId: 'a', environment: 'b:c', role: 'writer' };
...
test('archive metadata rejects malformed objects and hashes with an explicit request error', () => {
  const replaceReceipt = (fields: Record<string, unknown>) => ({ ...artifact(1), receipt: { ...artifact(1).receipt, ...fields } });
...
test('capability action matching follows the case-insensitive policy evaluator', async () => {
  const base = { issuer: 'test', audience: 'test-env', keyConfigured: true, proofMode: 'issuer', timestampConfigured: false, witnessConfigured: false, anchorConfigured: false, rpcChainIds: [8453], store: createMemoryStore(), now: 1000, archiveEnabled: false };
