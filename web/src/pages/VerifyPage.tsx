...
type ReviewSummary = {
  valid: boolean; code: string; verificationOrigin: string; priorSeal: LocalVerificationResult
  artifacts: { id: string; role: string; profile: string; integrityValid: boolean; signatureValid: boolean | null; trusted: boolean | null; code: string }[]
...
export function VerifyPage() {
  ...
  useEffect(() => { const cleared = () => { setProfiles([]); setProfile(null); setKeyTrusted(false); setPublicKey(''); setIssuer('') }; window.addEventListener('priorseal:trust-clear', cleared); return () => window.removeEventListener('priorseal:trust-clear', cleared) }, [])
  useEffect(() => { verificationRun.current += 1; setResult(null); setReview(null); setVerifiedWithTrust(false) }, [raw, publicKey, issuer, audience, keyTrusted, profile, insightKeys, insightTrusted])
  ...
  function loadProfile(value: unknown, alreadyConfirmed = false) {
    ...
    if (parsed.insightKeyRegistry) { setInsightKeys(JSON.stringify(parsed.insightKeyRegistry, null, 2)); setInsightTrusted(false) }
  ...
  async function verifyText(text: string) {
    ...
    try {
      ...
      if (manifest) {
        const evaluated = await verifier.verifyReviewManifestLocally(parsed, { ...options, ...(insightTrusted && insightKeys.trim() ? { insightKeyRegistry: JSON.parse(insightKeys) } : {}) })
