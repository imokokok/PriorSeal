...
export async function observeEvm({ chainId, txHash, confirmations = 0, rpcUrls, timeoutMs = 10000, signal, rpcClient = createRpcClient({ timeoutMs }) }) {
  ...
  let lastError; for (const url of urls) { try {
    ...
    const block = await rpcClient.call(url, 'eth_blockNumber', [], signal);
    const confirmationsSeen = Math.max(0, Number(BigInt(block) - BigInt(receipt.blockNumber) + 1n));
