...
async function observeEvm({ chainId, txHash, confirmations = 0, rpcUrls, timeoutMs = 1e4, signal, rpcClient = createRpcClient({ timeoutMs }) }) {
  ...
  for (const [index, url] of urls.entries()) {
    try {
      ...
      if (!receipt) {
        fallbackObservation = normalizeExecution({ chainId, txHash, status: "PENDING", action: actionFor(tx), nonce: transactionNonce, sender: tx.from, recipient: tx.to, target: tx.to, calldataHash: keccak256(tx.input ?? "0x"), nativeValue: hexBig(tx.value), executionDataAvailable: true, observationSource, finalityState: "PENDING" });
        continue;
      }
