...
export async function observeEvm({ chainId, txHash, confirmations = 0, rpcUrls, timeoutMs = 10000, signal, rpcClient = createRpcClient({ timeoutMs }) }) {
  ...
  let lastError; for (const [index, url] of urls.entries()) { try {
    ...
    if (!receipt) return normalizeExecution({ chainId, txHash, status: 'PENDING', action: actionFor(tx), sender: tx.from, recipient: tx.to, nativeValue: hexBig(tx.value), executionDataAvailable: true, observationSource, finalityState: 'PENDING' });
