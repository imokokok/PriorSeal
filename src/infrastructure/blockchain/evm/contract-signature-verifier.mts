 ...
 */
export function createContractSignatureVerifier({ rpcClient, rpcUrls }: { rpcClient: { call: (url: string, method: string, params: unknown[]) => Promise<unknown> }; rpcUrls: (chainId: number) => string[] | null }) {
  ...
  return async ({ authorization, digest, signature }: { authorization: Authorization; digest: `0x${string}`; signature: string }) => {
    ...
    for (const url of urls) {
      try {
        ...
        const result = await rpcClient.call(url, 'eth_call', [{ to: authorization.authorizer.address, data: erc1271CallData(digest, signature as `0x${string}`) }, 'latest']);
