...
function authorizationRecord(row) {
  const policyEvidence = parsed(row.policy_json);
  return {
    authorization: parsed(row.authorization_json),
    acceptance: parsed(row.acceptance_json),
...
function jobRecord(row, includeLease = false) {
  return {
    ...
    observation: parsed(row.observation_json),
...
function createD1Store(database) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      try {
        ...
        if (record) {
          ...
          if (saved) return logEntry(await first("SELECT * FROM authorization_log WHERE sequence=?", parsed(saved.acceptance_json).sequence));
  ...
  const store = {
    ...
    async getArchiveEntry(access, id) {
      ...
      return row && { ...parsed(row.entry_json), sequence: Number(row.sequence) };
    },
    ...
    async saveIntent(intent) {
      ...
      return parsed(saved.intent_json);
    },
    async getIntent(id) {
      return parsed((await first("SELECT intent_json FROM intents WHERE intent_id=?", id))?.intent_json) ?? void 0;
    },
    ...
    async getObservation(chainId, txHash) {
      return parsed((await first("SELECT observation_json FROM execution_observations WHERE chain_id=? AND tx_hash=? ORDER BY id DESC LIMIT 1", Number(chainId), txHash))?.observation_json) ?? void 0;
    },
    ...
    async saveReceipt(receipt) {
      ...
      const existing = parsed((await first("SELECT receipt_json FROM receipts WHERE receipt_id=?", receipt.receiptId))?.receipt_json);
      assertReceiptIdentity(existing, receipt);
      return existing;
    },
    async getReceipt(id) {
      return parsed((await first("SELECT receipt_json FROM receipts WHERE receipt_id=?", id))?.receipt_json);
    },
    async getIdempotency(scope, key) {
      ...
      return row ? { requestHash: row.request_hash, response: parsed(row.response_json), expiresAt: Number(row.expires_at) } : void 0;
    },
