  ...
  return {
    ...
    async saveAcceptedAuthorization({ authorization, acceptedAt, createRecord }: { authorization: AuthorizationRecord['authorization']; acceptedAt: number; createRecord: (log: LogEntry) => AuthorizationRecord }) {
      ...
      try {
        ...
        if (existingNonce.rows[0]) { const error: Error & { code?: string } = new Error('Authorization nonce has already been used'); error.code = 'AUTHORIZATION_NONCE_REUSED'; throw error; }
        ...
        if (!intentResult.rows[0]) { const error: Error & { code?: string } = new Error('DUPLICATE_INTENT'); error.code = 'DUPLICATE_INTENT'; throw error; }
    ...
    async getAuthorization(id: string) { const result = await pool.query<PgAuthorizationRow>('SELECT * FROM authorizations WHERE authorization_id=$1', [id]); return result.rows[0] && rowToAuthorization(result.rows[0]); },
      ...
      try {
        ...
        if (claimAuthorization) {
          ...
          if (!bound.rows[0]) {
            ...
            const error: Error & { code?: string } = new Error(existing.rows[0] ? 'Authorization has already been bound to another transaction' : 'Authorization not found'); error.code = existing.rows[0] ? 'AUTHORIZATION_ALREADY_USED' : 'AUTHORIZATION_NOT_FOUND'; throw error;
          }
        }
