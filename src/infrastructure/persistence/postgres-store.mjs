// Generated from postgres-store.mts by npm run core:build. Do not edit directly.
...
function createPostgresStore(pool) {
  ...
  return {
    ...
    async getIntent(intentId) {
      const result = await pool.query("SELECT intent_json FROM intents WHERE intent_id = $1 LIMIT 1", [intentId]);
      return result.rows[0] ? persistedIntent(result.rows[0].intent_json) : void 0;
    },
    async saveObservation(observation) {
      ...
      return result.rows[0] ? persistedObservation(result.rows[0].observation_json) : observation;
    },
    async getObservation(chainId, txHash) {
      const result = await pool.query("SELECT observation_json FROM execution_observations WHERE chain_id = $1 AND tx_hash = $2 ORDER BY id DESC LIMIT 1", [chainId, txHash]);
      return result.rows[0] ? persistedObservation(result.rows[0].observation_json) : void 0;
    },
    async appendAuthorizationLog({ authorizationHash, acceptedAt }) {
      ...
      try {
        ...
        if (existing.rows[0]) {
          ...
          return { sequence: Number(row.sequence), authorizationHash: row.authorization_hash, acceptedAt: Number(row.accepted_at), previousEntryHash: row.previous_entry_hash, entryHash: row.entry_hash };
        }
    ...
    async listAuthorizationLog() {
      const result = await pool.query("SELECT sequence,authorization_hash,accepted_at,previous_entry_hash,entry_hash FROM authorization_log ORDER BY sequence");
      return result.rows.map((row) => ({ sequence: Number(row.sequence), authorizationHash: row.authorization_hash, acceptedAt: Number(row.accepted_at), previousEntryHash: row.previous_entry_hash, entryHash: row.entry_hash }));
    },
    ...
    async saveAcceptedAuthorization({ authorization, acceptedAt, createRecord }) {
      ...
      try {
        ...
        if (existingAuthorization.rows[0]) {
          await client.query("COMMIT");
          return rowToAuthorization(existingAuthorization.rows[0]);
        }
        ...
        if (existingLog.rows[0]) {
          const row = existingLog.rows[0];
          log = { sequence: Number(row.sequence), authorizationHash: row.authorization_hash, acceptedAt: Number(row.accepted_at), previousEntryHash: row.previous_entry_hash, entryHash: row.entry_hash };
        } else {
          const previous = await client.query("SELECT sequence,entry_hash FROM authorization_log ORDER BY sequence DESC LIMIT 1");
          const sequence = Number(previous.rows[0]?.sequence ?? 0) + 1;
          const previousEntryHash = previous.rows[0]?.entry_hash ?? null;
        ...
        await client.query("COMMIT");
        return rowToAuthorization(result.rows[0]);
      } catch (error) {
        await client.query("ROLLBACK");
        throw error;
      } finally {
    ...
    async getAuthorization(id) {
      const result = await pool.query("SELECT * FROM authorizations WHERE authorization_id=$1", [id]);
      return result.rows[0] && rowToAuthorization(result.rows[0]);
    },
      ...
      try {
        ...
        if (claimAuthorization) {
          ...
          if (!bound.rows[0]) {
            ...
            const error = new Error(existing.rows[0] ? "Authorization has already been bound to another transaction" : "Authorization not found");
            error.code = existing.rows[0] ? "AUTHORIZATION_ALREADY_USED" : "AUTHORIZATION_NOT_FOUND";
            throw error;
          }
        }
        ...
        await client.query("ROLLBACK");
        if (errorCode(error) === "AUTHORIZATION_ALREADY_USED" || errorCode(error) === "AUTHORIZATION_NOT_FOUND") return { ok: false, code: errorCode(error) };
        throw error;
      } finally {
        client.release();
      }
    ...
    async saveReceipt(receipt) {
      ...
      if (result.rows[0]) return persistedReceipt(result.rows[0].receipt_json);
      const existing = await pool.query("SELECT receipt_json FROM receipts WHERE receipt_id = $1", [receipt.receiptId]);
      const stored = existing.rows[0] ? persistedReceipt(existing.rows[0].receipt_json) : void 0;
      assertReceiptIdentity(stored, receipt);
      return stored;
    },
    async getReceipt(receiptId) {
      const result = await pool.query("SELECT receipt_json FROM receipts WHERE receipt_id = $1", [receiptId]);
      return result.rows[0] ? persistedReceipt(result.rows[0].receipt_json) : void 0;
    },
    async getIdempotency(scope, key) {
      const result = await pool.query("SELECT request_hash,response_json,expires_at FROM idempotency_records WHERE scope = $1 AND idempotency_key = $2", [scope, key]);
      const row = result.rows[0];
      return row ? { requestHash: row.request_hash, response: persistedJson(row.response_json, "idempotency response"), expiresAt: persistedTimestamp(row.expires_at, "idempotency.expiresAt") } : void 0;
    },
    async reserveIdempotency({ scope, key, requestHash, response, expiresAt }) {
      ...
      return { replay: true, response: persistedJson(existing.rows[0].response_json, "idempotency response") };
    },
    ...
    async claimDueJobs(now, limit = 10, leaseDurationMs = 15 * 6e4) {
      ...
      try {
        ...
        await client.query("COMMIT");
        return result.rows.map((row) => rowToJob(row, { includeLease: true }));
      } catch (error) {
        await client.query("ROLLBACK");
        throw error;
      } finally {
    ...
    async getJob(jobId) {
      const result = await pool.query("SELECT * FROM observation_jobs WHERE job_id = $1", [jobId]);
      return result.rows[0] && rowToJob(result.rows[0]);
    },
    async health() {
