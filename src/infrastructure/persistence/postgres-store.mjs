...
function createPostgresStore(pool) {
  ...
  return {
    ...
    async saveObservation(observation) {
      ...
      return result.rows[0]?.observation_json ?? observation;
    },
    async getObservation(chainId, txHash) {
      ...
      return result.rows[0]?.observation_json;
    },
    ...
    async saveReceipt(receipt) {
      ...
      if (result.rows[0]) return result.rows[0].receipt_json;
      const existing = await pool.query("SELECT receipt_json FROM receipts WHERE receipt_id = $1", [receipt.receiptId]);
      assertReceiptIdentity(existing.rows[0]?.receipt_json, receipt);
      return existing.rows[0]?.receipt_json;
    },
    ...
    async getIdempotency(scope, key) {
      ...
      return row ? { requestHash: row.request_hash, response: row.response_json, expiresAt: new Date(row.expires_at).getTime() } : void 0;
    },
...
function rowToJob(row, { includeLease = false } = {}) {
  return { jobId: row.job_id, idempotencyKey: row.idempotency_key, input: row.input_json, state: row.state, attempts: row.attempts, nextAttemptAt: new Date(row.next_attempt_at).getTime(), observation: row.observation_json, result: row.result_json ?? null, error: row.error_json, createdAt: new Date(row.created_at).getTime(), ...includeLease ? { leaseToken: row.lease_token, leaseExpiresAt: new Date(row.lease_expires_at).getTime() } : {} };
}
function rowToAuthorization(row) {
  return { authorization: row.authorization_json, acceptance: row.acceptance_json, policy: row.policy_json?.result ?? row.policy_json, policyEvidence: row.policy_json?.schema === "priorseal.policy-evidence.v1" ? row.policy_json : void 0, ...row.timestamp_evidence_json ? { timestampEvidence: row.timestamp_evidence_json } : {}, ...row.witness_evidence_json ? { witnessEvidence: row.witness_evidence_json } : {}, status: row.status, boundTxHash: row.bound_tx_hash, uses: row.uses };
}
