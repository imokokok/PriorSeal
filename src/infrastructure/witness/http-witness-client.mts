...
type WitnessEndpoint = { witnessId: string; url: string; bearerToken?: string };

export function readWitnessEndpoints(file: string | null | undefined, { requireHttps = false } = {}) {
  if (!file) return null;
  let input;
  try { input = JSON.parse(readFileSync(file, 'utf8')); } catch (error) { throw new TypeError(`Unable to read witness endpoints: ${errorMessage(error)}`); }
  return parseWitnessEndpoints(input, { requireHttps });
}

export function parseWitnessEndpoints(inputValue: unknown, { requireHttps = false } = {}): WitnessEndpoint[] | null {
  const input = inputValue;
  if (input == null) return null;
  assertSafeJson(input);
  const document = assertOnlyFields(input, ['schema', 'endpoints'], 'witness endpoints');
  if (document.schema !== 'priorseal.witness-endpoints.v1' || !Array.isArray(document.endpoints)) throw new TypeError('Witness endpoint file must use priorseal.witness-endpoints.v1');
  const endpoints = document.endpoints.map((value: unknown, index: number) => {
    const entry = assertOnlyFields(value, ['witnessId', 'url', 'bearerToken'], `witness endpoint ${index}`);
    let url;
    try { url = new URL(String(entry.url)); } catch { throw new TypeError(`Witness endpoint ${index} has an invalid URL`); }
...
export function createHttpWitnessProvider({ policy, endpoints, requester, fetchImpl = globalThis.fetch, timeoutMs = 5_000 }: { policy: unknown; endpoints: WitnessEndpoint[] | null; requester: string; fetchImpl?: typeof fetch; timeoutMs?: number }) {
  ...
  const configured = new Map<string, WitnessEndpoint>((endpoints ?? []).map((entry) => [entry.witnessId, entry]));
  ...
  return async ({ authorization, acceptance }: { authorization: { authorizationId: string; intentHash: string; expiresAt: number }; acceptance: { acceptedAt: number } }) => {
    ...
    const results = await Promise.allSettled(normalizedPolicy.witnesses.map(async (witness) => {
      ...
      if (!endpoint) throw new TypeError(`Missing endpoint for witness ${witness.witnessId}`);
      ...
      try {
        ...
        const attestation = envelope && typeof envelope === 'object' && 'attestation' in envelope ? envelope.attestation : undefined;
...
function safeReason(error: unknown) {
  ...
  return String(error instanceof Error ? error.message : 'request failed').slice(0, 160);
}
