// Generated from http-witness-client.mts by npm run core:build. Do not edit directly.
...
function readWitnessEndpoints(file, { requireHttps = false } = {}) {
  if (!file) return null;
  let input;
  try {
    input = JSON.parse(readFileSync(file, "utf8"));
  } catch (error) {
    throw new TypeError(`Unable to read witness endpoints: ${errorMessage(error)}`);
  }
  return parseWitnessEndpoints(input, { requireHttps });
}
function parseWitnessEndpoints(inputValue, { requireHttps = false } = {}) {
  ...
  const endpoints = document.endpoints.map((value, index) => {
    const entry = assertOnlyFields(value, ["witnessId", "url", "bearerToken"], `witness endpoint ${index}`);
    let url;
    try {
      url = new URL(String(entry.url));
    } catch {
      throw new TypeError(`Witness endpoint ${index} has an invalid URL`);
    }
...
function createHttpWitnessProvider({ policy, endpoints, requester, fetchImpl = globalThis.fetch, timeoutMs = 5e3 }) {
  const normalizedPolicy = buildWitnessPolicy(policy);
  if (typeof fetchImpl !== "function") throw new TypeError("A fetch implementation is required");
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 6e4) throw new TypeError("Witness timeout must be between 100 and 60000 milliseconds");
  ...
  return async ({ authorization, acceptance }) => {
    ...
    const results = await Promise.allSettled(normalizedPolicy.witnesses.map(async (witness) => {
      ...
      try {
        ...
        const attestation = envelope && typeof envelope === "object" && "attestation" in envelope ? envelope.attestation : void 0;
