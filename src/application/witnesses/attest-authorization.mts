...
type WitnessStore = {
  get: (requestHash: string, witnessId: string) => Promise<Record<string, unknown> | null | undefined>;
  save: (requestHash: string, witnessId: string, attestation: Record<string, unknown>) => Promise<Record<string, unknown>>;
};

export async function attestAuthorization({ input, witnessId, keyId, privateKeyPem, store, now = () => Date.now(), maxRequestAgeSeconds = 300 }: { input: unknown; witnessId: string; keyId: string; privateKeyPem?: string; store: WitnessStore; now?: () => number; maxRequestAgeSeconds?: number }) {
