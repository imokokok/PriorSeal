// Generated from idempotency.mts by npm run core:build. Do not edit directly.
import { PriorSealError } from "../domain/errors.mjs";
import { hashJson } from "../domain/hashing.mjs";
const IDEMPOTENCY_KEY = /^[A-Za-z0-9._:-]{1,128}$/;
async function findIdempotentReplay({ scope, key, request, store, now = () => Date.now() }) {
...
async function reserveIdempotentResponse({ scope, key, request, requestHash, response, store, now = () => Date.now() }) {
  if (!key || !store.reserveIdempotency) return { replay: false, response };
  return store.reserveIdempotency({ scope, key, requestHash: requestHash ?? hashJson(request), response, expiresAt: now() + 864e5 });
}
export {
  findIdempotentReplay,
  reserveIdempotentResponse
};
