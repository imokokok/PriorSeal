...
export function createArchiveAccess(entries: unknown) {
...
export function archiveEntry(artifactValue: unknown, access: ArchiveAccess, { supersedesId = null, now = Date.now() }: { supersedesId?: string | null; now?: number } = {}) {
  ...
  if (artifact.schema === 'priorseal.verification-bundle.v1') { kind = 'verification-bundle'; receipt = asRecord(artifact.receipt); }
  else if (artifact.schema === 'priorseal.review-manifest.v1') { kind = 'review-manifest'; receipt = asRecord(asRecord(artifact.bundle)?.receipt); }
  else if (asRecord(artifact.authorization)?.schema === 'priorseal.authorization.v2' && artifact.acceptance) { kind = 'authorization'; authorization = asRecord(artifact.authorization); }
...
export function archiveQuery(url: URL, access: ArchiveAccess): ArchiveQuery {
  ...
  const filters: ArchiveFilters = { txHash: url.searchParams.get('txHash')?.toLowerCase() || null, authorizationId: url.searchParams.get('authorizationId') || null, status: url.searchParams.get('status') || null, from: null, to: null };
  if (filters.txHash && !/^0x[0-9a-f]{64}$/.test(filters.txHash)) throw new PriorSealError('INVALID_REQUEST', 'Invalid transaction hash');
  for (const key of ['authorizationId', 'status'] as const) if (filters[key] && !identifier.test(filters[key])) throw new PriorSealError('INVALID_REQUEST', `Invalid ${key}`);
  for (const key of ['from', 'to'] as const) if (url.searchParams.has(key)) { const value = Number(url.searchParams.get(key)); if (!Number.isSafeInteger(value) || value < 0) throw new PriorSealError('INVALID_REQUEST', 'Archive times must be Unix seconds'); filters[key] = value; }
...
export function archivePage(rows: ArchiveRow[], query: ArchiveQuery, snapshot: number) {
