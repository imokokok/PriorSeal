...
function required(value: string | undefined, name: string) { if (!value?.trim()) throw new TypeError(`${name} is required`); return value.trim(); }
function integer(value: string, name: string, min: number, max: number) { const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < min || parsed > max) throw new TypeError(`${name} must be an integer between ${min} and ${max}`); return parsed; }
