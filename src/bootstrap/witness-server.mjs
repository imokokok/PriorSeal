// Generated from witness-server.mts by npm run core:build. Do not edit directly.
import pg from "pg";
...
const port = integer(process.env.PORT ?? "3101", "PORT", 1, 65535);
...
if (environment === "production") {
  try {
    await assertWitnessSchema(pool);
  } catch (error) {
...
if (environment === "production" && (statSync(privateKeyFile).mode & 63) !== 0) throw new TypeError("Witness private key must not be readable by group or others");
...
function integer(value, name, min, max) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < min || parsed > max) throw new TypeError(`${name} must be an integer between ${min} and ${max}`);
  return parsed;
}
