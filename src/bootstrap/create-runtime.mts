...
import type { ObservationJob, ObservationWorkInput } from '../application/observations/observation-worker.mjs';
...
export async function createPriorSealRuntime({ config, environment = process.env, database, databaseConnectionString = config?.databaseUrl, poolOptions = {}, staticDir, dispatchObservationJob, rateLimiter, assertSchema = true }: RuntimeOptions = {}) {
  ...
  try {
    ...
    const rpcUrls = (chainId: number | string | undefined) => getRpcUrls(chainId ?? -1, environment) ?? [];
    ...
    const transparencyProvider = store && privateKeyPem ? async (acceptance: AuthorizationAcceptance, { before }: { before?: number } = {}) => {
      ...
      // Historical anchors commit to the hash-chain head and require the v1
      // chain proof. New Merkle anchors (and unanchored modes) use a compact v2
      // inclusion proof without transferring the entire log from PostgreSQL.
