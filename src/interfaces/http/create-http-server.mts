...
import type { createPostgresStore } from '../../infrastructure/persistence/postgres-store.mjs';
import type { createD1Store } from '../../infrastructure/persistence/d1-store.mjs';
...
import type { createDigiCertTimestampProvider } from '../../infrastructure/timestamp/digicert-rfc3161-client.mjs';
import type { createHttpWitnessProvider } from '../../infrastructure/witness/http-witness-client.mjs';
...
import type { parsePolicyDocument } from '../../infrastructure/policy/file-policy-provider.mjs';
...
function requester(req: IncomingMessage, trustProxy: boolean) { return trustProxy ? String(req.headers['cf-connecting-ip'] ?? '').trim() || String(req.headers['x-forwarded-for'] ?? '').split(',')[0].trim() || req.socket.remoteAddress || 'unknown' : req.socket.remoteAddress || 'unknown'; }

type HttpStore = ReturnType<typeof createMemoryStore> | ReturnType<typeof createPostgresStore> | ReturnType<typeof createD1Store>;
type HttpServerOptions = { archiveCredentials?: unknown; rpcChainIds?: number[]; proofMode?: string; store?: HttpStore; issuer?: string; privateKeyPem?: string; publicKeyPem?: string; keyId?: string; keyRegistry?: ReturnType<typeof createKeyRegistry>; policy?: ReturnType<typeof parsePolicyDocument>; authorizationAudience?: string; verifyContractSignature?: (input: { authorization: ReturnType<typeof buildAuthorization>; digest: `0x${string}`; signature: string }) => Promise<boolean> | boolean; timestampProvider?: ReturnType<typeof createDigiCertTimestampProvider> | null; requireTimestamp?: boolean; witnessProvider?: ReturnType<typeof createHttpWitnessProvider> | null; requireWitnessQuorum?: boolean; transparencyProvider?: ((acceptance: AuthorizationAcceptance, options?: { before?: number }) => Promise<{ checkpoint?: { anchor?: { anchoredAt: number } | null } | null }>) | null; observationWorker?: ReturnType<typeof createObservationWorker> | null; maxBodyBytes?: number; rateLimit?: number; rateLimiter?: { allow: (key: string, now: number) => boolean | Promise<boolean> }; requestTimeoutMs?: number; observer?: typeof observeEvm; corsOrigins?: string[]; trustProxy?: boolean; version?: string; staticDir?: string; logger?: { error?: (entry: Record<string, unknown>) => void } | null; now?: () => number };

export function createHttpServer({ archiveCredentials = null, rpcChainIds, proofMode, store = createMemoryStore(), issuer = 'priorseal-local', privateKeyPem, publicKeyPem, keyId = 'default', keyRegistry = createKeyRegistry(publicKeyPem ? [{ issuer, keyId, algorithm: 'Ed25519', publicKey: publicKeyPem, status: 'active', validFrom: null, validUntil: null }] : []), policy = null, authorizationAudience = 'priorseal', verifyContractSignature, timestampProvider = null, requireTimestamp = false, witnessProvider = null, requireWitnessQuorum = false, transparencyProvider = null, observationWorker = null, maxBodyBytes = 64 * 1024, rateLimit = 60, rateLimiter = createMemoryRateLimiter({ limit: rateLimit }), requestTimeoutMs = 15_000, observer = observeEvm, corsOrigins = [], trustProxy = false, version = process.env.PRIORSEAL_BUILD_VERSION ?? 'dev', staticDir, logger = null, now = () => Date.now() }: HttpServerOptions = {}) {
  ...
  return createServer({ requestTimeout: requestTimeoutMs }, async (req, res) => {
    ...
    const deadline = createRequestDeadline(req, requestTimeoutMs); const cleanup = () => deadline.cleanup();
    res.once('close', cleanup); res.once('finish', cleanup);
    const path = requestPath(req); const origin = req.headers.origin; const cors = origin && corsOrigins.includes(origin) ? { 'access-control-allow-origin': origin, vary: 'Origin', 'access-control-allow-headers': 'content-type,idempotency-key,x-request-id,authorization', 'access-control-allow-methods': 'GET,POST,OPTIONS' } : {};
    const respond = (status: number, body: unknown, headers: OutgoingHttpHeaders = {}) => sendJson(res, status, body, requestId, { ...cors, ...(path.startsWith('/v1/archive') ? { 'cache-control': 'private, no-store', vary: 'Authorization, Origin' } : {}), ...headers });
    const dispatch = async () => {
      ...
      if (path === '/v1/archive' || path.startsWith('/v1/archive/')) {
        ...
        const access = archiveAccess.authenticate(typeof req.headers.authorization === 'string' ? req.headers.authorization : undefined);
        ...
        if (entryMatch) { const entry = await store.getArchiveEntry(access, entryMatch[1]); if (!entry) throw new PriorSealError('NOT_FOUND', 'Archive entry not found'); return respond(200, entry); }
      ...
      if (req.method === 'POST' && path === '/v1/intents') {
        const result = await createIntent({ input: body, idempotencyKey: typeof req.headers['idempotency-key'] === 'string' ? req.headers['idempotency-key'] : undefined, store, policy, now });
        return respond(result.replay ? 200 : 201, { ...(result.response as Record<string, unknown>), requestId }, result.replay ? { 'idempotency-replayed': 'true' } : {});
      }
    ...
    try {
      ...
      const code = error instanceof PriorSealError ? error.code : errorCode(error) || 'INTERNAL_ERROR'; const status: number = ERROR_STATUS[code as keyof typeof ERROR_STATUS] ?? 500;
      ...
      return respond(status, errorBody(code, status === 500 ? 'Internal server error' : errorMessage(error), requestId, status < 500 ? (error instanceof PriorSealError ? error.details : undefined) : undefined));
    } finally { cleanup(); }
  });
}

function jsonSafe(value: unknown) { return JSON.parse(JSON.stringify(value, (_, item: unknown) => typeof item === 'bigint' ? item.toString() : item)); }

function keyRegistryDocument(issuer: string, keyRegistry: ReturnType<typeof createKeyRegistry>) { return { schema: 'priorseal.keys.v1', issuer, keys: keyRegistry.list(), verifierVersion: '3.0.0', schemaVersions: ['priorseal.intent.v1', 'priorseal.intent.v2', 'priorseal.execution-profile.exact-call.v1', 'priorseal.context-commitment.v1', 'priorseal.authorization.v1', 'priorseal.authorization.v2', 'priorseal.rfc3161-evidence.v1', 'priorseal.execution-receipt.v1', 'priorseal.execution-receipt.v2', 'priorseal.execution-receipt.v3', 'priorseal.compliance-assessment.v1', 'priorseal.verification-bundle.v1'] }; }
