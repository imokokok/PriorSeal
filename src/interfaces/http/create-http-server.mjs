...
import { authorizationTypedData, buildAuthorization, verifyAuthorizedReceipt } from '../../domain/authorization.mjs';
...
export function createHttpServer({ store = createMemoryStore(), issuer = 'runproof-local', privateKeyPem, publicKeyPem, keyId = 'default', keyRegistry = createKeyRegistry(publicKeyPem ? [{ issuer, keyId, algorithm: 'Ed25519', publicKey: publicKeyPem, status: 'active', validFrom: null, validUntil: null }] : []), policy = null, authorizationAudience = 'runproof', verifyContractSignature, timestampProvider = null, requireTimestamp = false, witnessProvider = null, requireWitnessQuorum = false, transparencyProvider = null, observationWorker = null, maxBodyBytes = 64 * 1024, rateLimit = 60, rateLimiter = createMemoryRateLimiter({ limit: rateLimit }), requestTimeoutMs = 15_000, observer = observeEvm, corsOrigins = [], trustProxy = false, version = process.env.RUNPROOF_BUILD_VERSION ?? 'dev', staticDir, logger = null, now = () => Date.now() } = {}) {
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/v1/version') return respond(200, { service: 'runproof', version, protocol: ['runproof.intent.v1', 'runproof.authorization.v1', 'runproof.rfc3161-evidence.v1', 'runproof.witness-attestation.v1', 'runproof.execution-receipt.v1', 'runproof.execution-receipt.v2'], requestId });
      ...
      if (authorizationMatch) { const record = await store.getAuthorization?.(decodeURIComponent(authorizationMatch[1])); if (!record) throw new RunProofError('AUTHORIZATION_NOT_FOUND', 'Authorization not found'); return respond(200, record); }
      if (req.method === 'POST' && path === '/v1/executions/observe') {
        ...
        if (observationWorker && ['PENDING', 'NOT_FOUND', 'RPC_ERROR', 'RPC_TIMEOUT'].includes(result.response.observation.status)) await observationWorker.enqueuePersistent({ ...body, idempotencyKey: `${body.authorizationId ?? body.intentId}:${body.chainId}:${body.txHash}:${body.confirmations ?? 0}` });
      ...
      if (transparencyMatch) { const record = await store.getAuthorization?.(decodeURIComponent(transparencyMatch[1])); if (!record) throw new RunProofError('AUTHORIZATION_NOT_FOUND', 'Authorization not found'); if (!transparencyProvider) throw new RunProofError('ISSUER_NOT_CONFIGURED', 'Transparency checkpoint signing is unavailable'); return respond(200, await transparencyProvider(record.acceptance)); }
      const jobMatch = req.method === 'GET' && path.match(/^\/v1\/observation-jobs\/([^/]+)$/);
      if (jobMatch && observationWorker) { const job = await observationWorker.get(decodeURIComponent(jobMatch[1])); if (!job) throw new RunProofError('NOT_FOUND', 'Observation job not found'); return respond(200, job); }
      ...
      if (req.method === 'POST' && path === '/v1/receipts/verify') { assertOnlyFields(body, ['receipt'], 'verification request'); const entry = keyRegistry.get(body.receipt?.keyId); const result = !entry ? { valid: false, code: 'UNKNOWN_KEY' } : body.receipt?.schema === 'runproof.execution-receipt.v2' ? await verifyAuthorizedReceipt(body.receipt, entry.publicKey, { audience: authorizationAudience, verifyContractSignature }) : verifyReceipt(body.receipt, entry.publicKey, { keyId: entry.keyId, now: Math.floor(now() / 1000), key: entry }); return respond(200, { convenienceEndpoint: true, independentVerification: 'Use the local verifier; do not trust this API response alone.', result, requestId }); }
      if (req.method === 'GET' && path === '/.well-known/runproof-keys.json') return respond(200, { schema: 'runproof.keys.v1', issuer, keys: keyRegistry.list(), verifierVersion: '2.2.0', schemaVersions: ['runproof.execution-receipt.v1', 'runproof.execution-receipt.v2', 'runproof.authorization.v1', 'runproof.rfc3161-evidence.v1'], requestId }, { 'cache-control': 'public, max-age=300' });
