...
function requester(req, trustProxy) { return trustProxy ? String(req.headers['cf-connecting-ip'] ?? '').trim() || String(req.headers['x-forwarded-for'] ?? '').split(',')[0].trim() || req.socket.remoteAddress || 'unknown' : req.socket.remoteAddress || 'unknown'; }
  ...
  return createServer({ requestTimeout: requestTimeoutMs }, async (req, res) => {
    ...
    const dispatch = async () => {
      ...
      if (path === '/v1/archive' || path.startsWith('/v1/archive/')) {
        ...
        const access = archiveAccess.authenticate(req.headers.authorization);
      ...
      if (req.method === 'POST' && path === '/v1/executions/observe') {
        const result = await observeExecution({ input: body, store, observer, signal: deadline.signal, privateKeyPem, issuer, keyId, publicKeyPem, idempotencyKey: req.headers['idempotency-key'], transparencyProvider, authorizationAudience, verifyContractSignature, now });
    ...
    try {
      ...
      const code = error instanceof PriorSealError ? error.code : error?.code || 'INTERNAL_ERROR'; const status = ERROR_STATUS[code] ?? 500;
