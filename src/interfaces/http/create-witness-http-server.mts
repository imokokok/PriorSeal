...
type WitnessStore = ReturnType<typeof createMemoryWitnessStore>;
type WitnessServerOptions = { witnessId?: string; keyId?: string; privateKeyPem?: string; publicKeyPem?: string; store?: WitnessStore; bearerToken?: string; maxBodyBytes?: number; rateLimit?: number; rateLimiter?: ReturnType<typeof createMemoryRateLimiter>; requestTimeoutMs?: number; maxRequestAgeSeconds?: number; now?: () => number; logger?: { error?: (entry: Record<string, unknown>) => void } | null };
export function createWitnessHttpServer({ witnessId, keyId = 'default', privateKeyPem, publicKeyPem, store = createMemoryWitnessStore(), bearerToken, maxBodyBytes = 16 * 1024, rateLimit = 120, rateLimiter = createMemoryRateLimiter({ limit: rateLimit }), requestTimeoutMs = 10_000, maxRequestAgeSeconds = 300, now = () => Date.now(), logger = null }: WitnessServerOptions = {}) {
  ...
  return createServer({ requestTimeout: requestTimeoutMs }, async (req, res) => {
    ...
    const respond = (status: number, body: unknown, headers: OutgoingHttpHeaders = {}) => sendJson(res, status, body, requestId, headers);
    ...
    try {
      ...
      const status: number = ERROR_STATUS[code as keyof typeof ERROR_STATUS] ?? 500;
      if (code === 'REQUEST_TIMEOUT') deadline.closeAfterResponse(res);
      logger?.error?.({ event: 'witness.request_failed', requestId, code, status });
      return respond(status, errorBody(code, status === 500 ? 'Internal server error' : errorMessage(error), requestId));
    } finally { deadline.cleanup(); }
  });
}

function authorized(header: string | undefined, expected: string | undefined) {
