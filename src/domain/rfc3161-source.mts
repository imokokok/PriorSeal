...
export function buildTimestampPolicy(inputValue: unknown = {}): TimestampPolicy {
  if (!inputValue || typeof inputValue !== 'object') throw new TypeError('timestampPolicy must be an object');
  ...
  if (typeof maxClockSkewSeconds !== 'number' || !Number.isSafeInteger(maxClockSkewSeconds) || maxClockSkewSeconds < 0 || maxClockSkewSeconds > 3_600) throw new TypeError('timestampPolicy.maxClockSkewSeconds must be between 0 and 3600');
...
export async function buildTimestampEvidence({ response, authorizationHash, requestedAt, nonce, tsaUrl = DIGICERT_RFC3161_URL, cryptoProvider = globalThis.crypto }: { response: Uint8Array; authorizationHash: string; requestedAt: number; nonce: string; tsaUrl?: string; cryptoProvider?: Crypto }): Promise<TimestampEvidence> {
...
export async function verifyTimestampEvidence(evidence: TimestampEvidence | null | undefined, data: Uint8Array, policy: unknown, { authorizationHash, requestedAt, before, cryptoProvider = globalThis.crypto }: TimestampOptions = {}) {
  try {
    ...
    const crypto = requireCrypto(cryptoProvider);
    configureEngine(crypto);
    if (!evidence) return invalid('INVALID_TIMESTAMP_EVIDENCE');
    ...
    let result: boolean | VerificationDetails;
    try {
      // PKI.js forwards these options to SignedData.verify at runtime, while
      // TimeStampResp.verify declares only the smaller RFC 3161 parameter set.
      result = await response.verify({
        ...
        extendedMode: true,
      } as Parameters<SignedData['verify']>[0]) as boolean | VerificationDetails;
    } catch {
      return invalid('INVALID_TIMESTAMP_SIGNATURE');
    }
    if (result === false || (typeof result === 'object' && (!result.signatureVerified || !result.signerCertificateVerified))) return invalid('INVALID_TIMESTAMP_SIGNATURE');
    const signer = result === true ? null : result.signerCertificate;
    const parsedEku = signer?.extensions?.find((extension: { extnID: string }) => extension.extnID === '2.5.29.37')?.parsedValue;
    const eku = parsedEku && 'keyPurposes' in parsedEku ? parsedEku.keyPurposes : undefined;
...
export function validateTimestampEvidenceClaims(evidence: TimestampEvidence | null | undefined, policy: unknown, { authorizationHash, requestedAt, before }: TimestampOptions = {}) {
  try {
    ...
    const referenceTime = requestedAt ?? NaN;
    if (!Number.isSafeInteger(evidence.timestamp) || evidence.timestamp < referenceTime - normalizedPolicy.maxClockSkewSeconds || evidence.timestamp > referenceTime + normalizedPolicy.maxClockSkewSeconds) return invalid('TIMESTAMP_CLOCK_SKEW');
...
function timestampMetadata(response: TimeStampResp) {
  if (!response.timeStampToken) throw new TypeError('RFC 3161 token is absent');
...
function certificateFingerprint(certificate: Certificate) {
...
function configureEngine(crypto: Crypto) {
  setEngine('priorseal-rfc3161', crypto, new CryptoEngine({ name: 'priorseal-rfc3161', crypto, subtle: crypto.subtle }));
}

function requireCrypto(value: Crypto) {
  if (!value?.subtle || !value?.getRandomValues) throw new TypeError('WebCrypto is required for RFC 3161 verification');
  return value;
}

async function sha256Hex(value: Uint8Array, crypto: Crypto = globalThis.crypto) {
  const digest = await requireCrypto(crypto).subtle.digest('SHA-256', toArrayBuffer(toBytes(value)));
  return [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, '0')).join('');
}

function integerHex(value: asn1js.Integer) { return [...value.valueBlock.valueHexView].map((byte) => byte.toString(16).padStart(2, '0')).join(''); }
function toBytes(value: Uint8Array | ArrayBuffer): Uint8Array { return value instanceof Uint8Array ? value : new Uint8Array(value); }
function toArrayBuffer(value: Uint8Array | ArrayBuffer): ArrayBuffer { return Uint8Array.from(toBytes(value)).buffer; }
...
function encodeBase64Url(value: Uint8Array) { return encodeBase64(toBytes(value)).replaceAll('+', '-').replaceAll('/', '_').replace(/=+$/, ''); }
function decodeBase64Url(value: string) { return decodeBase64(String(value).replaceAll('-', '+').replaceAll('_', '/')); }
...
function decodeBase64(value: string) { const normalized = value + '='.repeat((4 - value.length % 4) % 4); return Uint8Array.from(atob(normalized), (character) => character.charCodeAt(0)); }
