// Generated from witness.mts by npm run core:build. Do not edit directly.
...
const WITNESS_POLICY_SCHEMA = "priorseal.witness-policy.v1";
const WITNESS_REQUEST_SCHEMA = "priorseal.witness-request.v1";
const WITNESS_ATTESTATION_SCHEMA = "priorseal.witness-attestation.v1";
const WITNESS_EVIDENCE_SCHEMA = "priorseal.witness-evidence.v1";
const HASH = /^[0-9a-f]{64}$/;
function buildWitnessPolicy(inputValue) {
  ...
  const witnesses = input.witnesses.map((entry, index) => {
    ...
    try {
      ...
      throw new PriorSealError("INVALID_WITNESS_POLICY", `witness ${index} public key is not Ed25519`);
    }
...
function buildWitnessRequest(inputValue) {
  assertSafeJson(inputValue);
...
function witnessRequestForAuthorization(authorization, { requester, requestedAt }) {
...
function buildWitnessAttestation({ request, witnessId, keyId, observedAt }) {
...
function signWitnessAttestation(attestation, privateKeyPem) {
...
function verifyWitnessAttestation(attestation, request, witness) {
  let checked;
  try {
  ...
  if (!Number.isSafeInteger(checked.observedAt) || checked.observedAt < 0 || checked.observedAt > request.expiresAt) return false;
  return verifyEd25519Statement(checked, witness.publicKey);
}
function buildWitnessEvidence({ request, attestations, policy }) {
...
function verifyWitnessEvidence(evidence, authorization, policy, { expectedRequestedAt, before } = {}) {
  ...
  try {
    ...
    return invalid(error?.code ?? "INVALID_WITNESS_EVIDENCE");
  }
  ...
  if (expectedRequestedAt !== void 0 && request.requestedAt !== expectedRequestedAt) return invalid("WITNESS_ACCEPTANCE_MISMATCH");
  ...
  for (const attestation of candidate.attestations) {
    ...
    if (before !== void 0 && attestation.observedAt > before) continue;
