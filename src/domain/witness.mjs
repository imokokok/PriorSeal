...
export function buildWitnessPolicy(input) {
  ...
  const witnesses = input.witnesses.map((entry, index) => {
    ...
    if (entry.algorithm !== 'Ed25519' || typeof entry.publicKey !== 'string' || !entry.publicKey.trim()) throw new PriorSealError('INVALID_WITNESS_POLICY', 'every witness requires an Ed25519 public key');
    try {
...
export function verifyWitnessAttestation(attestation, request, witness) {
  ...
  if (!attestation?.signature || attestation.schema !== WITNESS_ATTESTATION_SCHEMA || attestation.domain !== 'priorseal/witness-attestation/v1') return false;
...
export function verifyWitnessEvidence(evidence, authorization, policy, { expectedRequestedAt, before } = {}) {
  ...
  if (evidence?.schema !== WITNESS_EVIDENCE_SCHEMA || evidence.domain !== 'priorseal/witness-evidence/v1') return invalid('INVALID_WITNESS_EVIDENCE');
