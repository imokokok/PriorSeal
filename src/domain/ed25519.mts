...
export function signEd25519Statement<T extends Record<string, unknown>>(statement: T | null | undefined, privateKeyPem: string) {
  const { signature: _existingSignature, ...unsigned } = (statement ?? {}) as T & { signature?: unknown };
...
export function verifyEd25519Statement(statement: Record<string, unknown> | null | undefined, publicKeyPem: string): boolean {
...
export function assertEd25519KeyPair(privateKeyPem: string, publicKeyPem: string): true {
