...
const RFC3161_POLICY_SCHEMA = "priorseal.timestamp-policy.v1";
const RFC3161_EVIDENCE_SCHEMA = "priorseal.rfc3161-evidence.v1";
const DIGICERT_RFC3161_PROFILE = "digicert-rfc3161-v1";
const DIGICERT_RFC3161_URL = "http://timestamp.digicert.com";
const DIGICERT_POLICY_OID = "2.16.840.1.114412.7.1";
...
function buildTimestampPolicy(inputValue = {}) {
  ...
  if (typeof maxClockSkewSeconds !== "number" || !Number.isSafeInteger(maxClockSkewSeconds) || maxClockSkewSeconds < 0 || maxClockSkewSeconds > 3600) throw new TypeError("timestampPolicy.maxClockSkewSeconds must be between 0 and 3600");
  return { schema: RFC3161_POLICY_SCHEMA, profile: DIGICERT_RFC3161_PROFILE, maxClockSkewSeconds };
}
async function createTimestampRequest(data, cryptoProvider = globalThis.crypto) {
...
async function buildTimestampEvidence({ response, authorizationHash, requestedAt, nonce, tsaUrl = DIGICERT_RFC3161_URL, cryptoProvider = globalThis.crypto }) {
...
async function verifyTimestampEvidence(evidence, data, policy, { authorizationHash, requestedAt, before, cryptoProvider = globalThis.crypto } = {}) {
  try {
    ...
    const parsedEku = signer?.extensions?.find((extension) => extension.extnID === "2.5.29.37")?.parsedValue;
    const eku = parsedEku && "keyPurposes" in parsedEku ? parsedEku.keyPurposes : void 0;
...
function validateTimestampEvidenceClaims(evidence, policy, { authorizationHash, requestedAt, before } = {}) {
  try {
    const normalizedPolicy = buildTimestampPolicy(policy);
    ...
    if (before !== void 0 && evidence.timestamp > before) return invalid("TIMESTAMP_AFTER_EXECUTION");
...
function timestampMetadata(response) {
  ...
  return { timestamp: Math.floor(info.genTime.getTime() / 1e3), serialNumber: integerHex(info.serialNumber), nonce: integerHex(info.nonce), policyOid: info.policy };
}
...
function integerHex(value) {
  return [...value.valueBlock.valueHexView].map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
...
function encodeBase64Url(value) {
  return encodeBase64(toBytes(value)).replaceAll("+", "-").replaceAll("/", "_").replace(/=+$/, "");
}
function decodeBase64Url(value) {
  return decodeBase64(String(value).replaceAll("-", "+").replaceAll("_", "/"));
}
...
export {
  ...
  RFC3161_EVIDENCE_SCHEMA,
  RFC3161_POLICY_SCHEMA,
