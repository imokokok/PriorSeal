...
export const AUTHORIZATION_SCHEMA = 'runproof.authorization.v1';
export const AUTHORIZATION_RECEIPT_SCHEMA = 'runproof.authorization-receipt.v1';
export const AUTHORIZATION_DOMAIN = 'runproof/authorization/v1';
export const AUTHORIZED_RECEIPT_SCHEMA = 'runproof.execution-receipt.v2';

const AUTHORIZATION_TYPES = Object.freeze({
  RunProofAuthorization: [
...
export function buildAuthorization(input) {
  ...
  if (!input.principal?.id || !input.delegate?.agentId) throw new RunProofError('INVALID_AUTHORIZATION', 'principal.id and delegate.agentId are required');
  const intent = buildIntent(stripIntentMetadata(input.intent));
  if (input.intentHash && input.intentHash !== intent.intentHash) throw new RunProofError('INVALID_AUTHORIZATION', 'intentHash does not match intent');
  const authorizerType = String(input.authorizer?.type ?? '');
  if (!['eip712', 'eip1271'].includes(authorizerType)) throw new RunProofError('INVALID_AUTHORIZATION', 'authorizer.type must be eip712 or eip1271');
  const principalType = String(input.principal?.type ?? '');
  if (!['user', 'organization'].includes(principalType)) throw new RunProofError('INVALID_AUTHORIZATION', 'principal.type must be user or organization');
  ...
  if (notBefore < issuedAt || expiresAt < notBefore || expiresAt > intent.validUntil) throw new RunProofError('INVALID_AUTHORIZATION', 'authorization time window must be ordered and cannot exceed the intent validity');
  const authorizationNonce = String(input.authorizationNonce ?? '').toLowerCase();
  if (!/^0x[0-9a-f]{64}$/.test(authorizationNonce)) throw new RunProofError('INVALID_AUTHORIZATION', 'authorizationNonce must be a 32-byte hex value');
  const policyHash = String(input.policyHash ?? `0x${'0'.repeat(64)}`).toLowerCase();
  if (!/^0x[0-9a-f]{64}$/.test(policyHash)) throw new RunProofError('INVALID_AUTHORIZATION', 'policyHash must be a 32-byte hex value');
  const maxUses = uintString(input.maxUses ?? '1', 'maxUses');
  if (maxUses !== '1') throw new RunProofError('INVALID_AUTHORIZATION', 'v1 authorizations are single-use and maxUses must be 1');
  const authorization = {
    ...
    authorizationNonce,
    maxUses,
    audience: protocolId(input.audience ?? 'runproof', 'audience'),
    policyHash,
    ...(input.signature ? { signature: String(input.signature) } : {}),
  };
  if (authorization.principal.account !== authorization.authorizer.address) throw new RunProofError('INVALID_AUTHORIZATION', 'principal.account must be the signing account');
  const { signature, ...unsigned } = authorization;
  const authorizationId = `auth_${hashJson(unsigned).slice(0, 32)}`;
  if (input.authorizationId && input.authorizationId !== authorizationId) throw new RunProofError('INVALID_AUTHORIZATION', 'authorizationId does not match authorization');
...
export function authorizationTypedData(value) {
  ...
  return {
    domain: { name: 'RunProof', version: '1', chainId: authorization.intent.chainId },
...
export async function verifyAuthorization(value, { now = Math.floor(Date.now() / 1000), audience = 'runproof', verifyContractSignature } = {}) {
  let authorization;
...
export function buildAuthorizationReceipt({ authorization, issuer, keyId = 'default', acceptedAt, sequence = 1, previousEntryHash = null }) {
  ...
  return { schema: AUTHORIZATION_RECEIPT_SCHEMA, domain: 'runproof/authorization-receipt/v1', authorizationId: authorization.authorizationId, authorizationHash, intentHash: authorization.intentHash, acceptedAt, sequence, previousEntryHash, entryHash, status: 'ACCEPTED', issuer, algorithm: 'Ed25519', keyId };
}
...
export function buildAuthorizedReceipt({ authorization, acceptance, policyEvidence = null, timestampEvidence = null, witnessEvidence = null, transparency = null, execution, issuer, keyId = 'default', issuedAt = Math.floor(Date.now() / 1000), verifierVersion = '2.2.0' }) {
  ...
  } else if (timestampEvidence) throw new RunProofError('TIMESTAMP_POLICY_MISMATCH', 'Timestamp evidence is not bound by the signed authorization policy');
  ...
  } else if (witnessEvidence) throw new RunProofError('WITNESS_POLICY_MISMATCH', 'Witness evidence is not bound by the signed authorization policy');
  ...
  const unsigned = { schema: AUTHORIZED_RECEIPT_SCHEMA, domain: 'runproof/execution-receipt/v2', receiptId: '', intentHash: intent.intentHash, authorizationHash, executionHash, authorizationEvidence: { authorization, acceptance, policy: evidencePolicy, ...(timestampEvidence ? { timestamp: timestampEvidence } : {}), ...(witnessEvidence ? { witnesses: witnessEvidence } : {}), ...(transparency ? { transparency } : {}) }, execution, issuer, issuedAt, validUntil: intent.validUntil, outcome, reasonCodes, binding, algorithm: 'Ed25519', keyId, verifierVersion };
  unsigned.receiptId = authorizedReceiptId(unsigned);
  return unsigned;
}

export function authorizedReceiptId(receipt) {
  return `rpr_${hashJson({ authorizationHash: receipt.authorizationHash, executionHash: receipt.executionHash, issuer: receipt.issuer, keyId: receipt.keyId }).slice(0, 32)}`;
}

export function validateAuthorizedReceiptClaims(receipt) {
  try {
    if (receipt?.schema !== AUTHORIZED_RECEIPT_SCHEMA || receipt.domain !== 'runproof/execution-receipt/v2') return invalid('UNSUPPORTED_SCHEMA');
...
export async function verifyAuthorizedReceipt(receipt, publicKeyPem, options = {}) {
  ...
  const authorizationResult = await verifyAuthorization(claims.authorization, { now: acceptance.acceptedAt, audience: options.audience ?? 'runproof', verifyContractSignature: options.verifyContractSignature });
...
function defaultPolicyEvidence(authorization, evaluatedAt) {
  if (authorization.policyHash !== `0x${'0'.repeat(64)}`) throw new RunProofError('MISSING_POLICY_EVIDENCE', 'A non-default policy must be embedded in an authorized receipt');
  return { schema: 'runproof.policy-evidence.v1', policyHash: authorization.policyHash, document: null, result: { allowed: true, reasonCodes: [], policyId: null, evaluatedAt } };
}
function validatePolicyEvidence(authorization, evidence, evaluatedAt) {
  if (!evidence || evidence.schema !== 'runproof.policy-evidence.v1') throw new RunProofError('INVALID_POLICY_EVIDENCE', 'Policy evidence is missing or malformed');
  const expectedHash = evidence.document ? `0x${hashJson(evidence.document)}` : `0x${'0'.repeat(64)}`;
  if (evidence.policyHash !== authorization.policyHash || evidence.policyHash !== expectedHash) throw new RunProofError('INVALID_POLICY_EVIDENCE', 'Policy evidence hash does not match the authorization');
  ...
  if (!expectedResult.allowed || hashJson(expectedResult) !== hashJson(evidence.result)) throw new RunProofError('INVALID_POLICY_EVIDENCE', 'Policy evaluation does not match the embedded policy');
  return true;
}
