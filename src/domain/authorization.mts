...
import { bindIntentExecution, type BindingExecution, type BindingResult } from './binding.mjs';
...
export type Authorization = ReturnType<typeof buildAuthorization>;
export type AuthorizationAcceptance = ReturnType<typeof buildAuthorizationReceipt> & { signature?: string };
type PolicyEvidence = { schema: string; policyHash: string; document: Record<string, unknown> | null; result: unknown };
type ReceiptExecution = BindingExecution & { status?: string; sender?: string | null };
type ReceiptTransparency = { checkpoint?: { anchor?: { anchoredAt: number } | null } | null };
type ContractSignatureVerifier = (input: { authorization: Authorization; digest: `0x${string}`; signature: string }) => Promise<boolean> | boolean;
...
export function buildAuthorization(inputValue: unknown) {
...
export function authorizationTypedData(value: ReturnType<typeof buildAuthorization>) {
...
export async function verifyAuthorization(value: unknown, { now = Math.floor(Date.now() / 1000), audience = 'priorseal', verifyContractSignature }: { now?: number; audience?: string; verifyContractSignature?: ContractSignatureVerifier } = {}) {
  let authorization;
  try { authorization = buildAuthorization(value); } catch (error) { return invalid(errorCode(error) ?? 'INVALID_AUTHORIZATION'); }
  ...
  try {
    const valid = authorization.authorizer.type === 'eip1271'
      ...
      : await verifyTypedData({ ...typedData, address: authorization.authorizer.address as `0x${string}`, signature: authorization.signature as `0x${string}` });
  ...
  } catch (error) { return invalid(errorCode(error) === 'AUTHORIZATION_VERIFIER_UNAVAILABLE' ? 'AUTHORIZATION_VERIFIER_UNAVAILABLE' : 'INVALID_AUTHORIZATION_SIGNATURE'); }
}
...
export function assertIssuableAuthorization<T extends ReturnType<typeof buildAuthorization>>(authorization: T): T {
...
export function erc1271CallData(digest: `0x${string}`, signature: `0x${string}`) {
...
export function buildAuthorizationReceipt({ authorization, issuer, keyId = 'default', acceptedAt, sequence = 1, previousEntryHash = null }: { authorization: ReturnType<typeof buildAuthorization>; issuer: string; keyId?: string; acceptedAt: number; sequence?: number; previousEntryHash?: string | null }) {
...
export function signAuthorizationReceipt<T extends Record<string, unknown>>(receipt: T, privateKeyPem: string) {
  return signEd25519Statement(receipt, privateKeyPem);
}

export function verifyAuthorizationReceipt(receiptValue: unknown, publicKeyPem: string) {
...
export function buildAuthorizedReceipt<E extends ReceiptExecution>({ authorization, acceptance, policyEvidence = null, timestampEvidence = null, witnessEvidence = null, transparency = null, execution, issuer, keyId = 'default', issuedAt = Math.floor(Date.now() / 1000), verifierVersion, schema = AUTHORIZED_RECEIPT_SCHEMA }: { authorization: Authorization; acceptance: AuthorizationAcceptance; policyEvidence?: PolicyEvidence | null; timestampEvidence?: TimestampEvidence | null; witnessEvidence?: unknown; transparency?: ReceiptTransparency | null; execution: E; issuer: string; keyId?: string; issuedAt?: number; verifierVersion?: string; schema?: string }) {
  ...
  if (anchoredAt !== undefined && anchoredAt > executedAt) throw new PriorSealError('TRANSPARENCY_AFTER_EXECUTION', 'Transparency anchor must precede execution');
  ...
  const binding: BindingResult = { bound: baseBinding.bound && !executorMismatch && !authorizationAfterExecution && !outsideAuthorizationWindow, reasonCodes: [...new Set([...baseBinding.reasonCodes, ...(executorMismatch ? ['EXECUTOR_MISMATCH' as const] : []), ...(authorizationAfterExecution ? ['AUTHORIZATION_AFTER_EXECUTION' as const] : []), ...(outsideAuthorizationWindow ? ['OUTSIDE_AUTHORIZATION_WINDOW' as const] : [])])] };
...
export function authorizedReceiptId(receipt: { authorizationHash: string; executionHash: string; issuer: string; keyId: string; schema: string }) {
...
export function validateAuthorizedReceiptClaims(receiptValue: unknown) {
  // The checks below are the runtime validator for this wire object. Missing or
  // malformed nested fields are caught and converted to INVALID_RECEIPT.
  const receipt = receiptValue as (ReturnType<typeof buildAuthorizedReceipt> & { signature?: string }) | null;
  try {
    ...
    return { valid: true as const, code: 'OK', authorization };
  } catch (error) { return invalid(errorCode(error) ?? 'INVALID_RECEIPT'); }
}

export async function verifyAuthorizedReceipt(receiptValue: unknown, publicKeyPem: string, options: { key?: KeyEntry; now?: number; audience?: string; verifyContractSignature?: ContractSignatureVerifier } = {}) {
  const receipt = receiptValue as (ReturnType<typeof buildAuthorizedReceipt> & { signature?: string }) | null;
  ...
  const fail = (code: string) => ({ valid: false, code, ...resultFields() });
...
function verifyEd25519Statement(statement: Record<string, unknown>, publicKeyPem: string) {
  return verifyEd25519StatementStrict(statement, publicKeyPem);
}
function validKeyWindow(key: KeyEntry) {
  for (const field of ['validFrom', 'validUntil'] as const) if (key[field] != null && (!Number.isSafeInteger(key[field]) || key[field]! <= 0)) return false;
  return key.validFrom == null || key.validUntil == null || key.validFrom <= key.validUntil;
}

function stripIntentHash(intent: ReturnType<typeof buildIntent>) { const { intentHash, ...unsigned } = intent; return unsigned; }
function stripIntentMetadata(intent: unknown) { const { intentHash, ...input } = intent && typeof intent === 'object' ? intent as Record<string, unknown> : {}; return input; }
function defaultPolicyEvidence(authorization: Authorization, evaluatedAt: number): PolicyEvidence {
...
function validatePolicyEvidence(authorization: Authorization, evidence: PolicyEvidence | null | undefined, evaluatedAt: number) {
...
function invalid(code: string) { return { valid: false as const, code }; }
