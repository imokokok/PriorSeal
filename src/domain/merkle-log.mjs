// Generated from merkle-log.mts by npm run core:build. Do not edit directly.
import { hashJson } from "./hashing.mjs";
const merkleLeafHash = (entryHash) => hashJson({ domain: "priorseal/transparency-leaf/v1", entryHash });
const merkleParentHash = (left, right) => hashJson({ domain: "priorseal/transparency-node/v1", left, right });
const merkleNodeKey = (start, level) => `${start}:${level}`;
...
function requiredMerkleNodes(sequence, size) {
...
function createMerkleProof(entryHash, sequence, size, nodes) {
...
function verifyMerkleProof(entryHash, sequence, size, path, expectedRoot) {
  if (!/^[0-9a-f]{64}$/.test(entryHash ?? "") || !/^[0-9a-f]{64}$/.test(expectedRoot ?? "") || !Array.isArray(path)) return false;
  let expectedSides;
  try {
    expectedSides = merkleProofSides(sequence, size);
  } catch {
    return false;
  }
...
function merkleAppendNodes(sequence, entryHash, getNode) {
