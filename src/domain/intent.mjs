...
export function buildIntent(input) {
  ...
  if (constraints !== undefined) {
    ...
    if (constraints.minConfirmations != null && (!Number.isSafeInteger(Number(constraints.minConfirmations)) || Number(constraints.minConfirmations) < 0)) throw new PriorSealError('INVALID_CONSTRAINT', 'minConfirmations must be a non-negative integer');
  ...
  if (!/^eip155:[1-9][0-9]*\/(native|erc20:0x[0-9a-fA-F]{40})$/.test(intent.asset)) throw new PriorSealError('INVALID_ASSET', 'asset must be eip155:<chain>/native or eip155:<chain>/erc20:<address>');
