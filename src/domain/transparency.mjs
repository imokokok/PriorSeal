// Generated from transparency.mts by npm run core:build. Do not edit directly.
...
const TRANSPARENCY_CHECKPOINT_SCHEMA = "priorseal.transparency-checkpoint.v1";
const MERKLE_TRANSPARENCY_CHECKPOINT_SCHEMA = "priorseal.transparency-checkpoint.v2";
function buildMerkleTransparencyEvidence({ acceptance, snapshot, issuer, keyId, privateKeyPem, issuedAt, anchor = null, before }) {
  ...
  if (anchor) {
    if (!anchor.merkleRoot || anchor.size !== snapshot.size || anchor.merkleRoot !== snapshot.merkleRoot || anchor.headEntryHash !== snapshot.headEntryHash || before !== void 0 && anchor.anchoredAt > before) throw new PriorSealError("TRANSPARENCY_AFTER_EXECUTION", "Verified Merkle anchor must cover the authorization before execution");
  }
...
function buildTransparencyEvidence({ entries, acceptance, issuer, keyId, privateKeyPem, issuedAt, anchor = null, before }) {
  ...
  if (anchor) {
    if (before !== void 0 && anchor.anchoredAt > before) throw new PriorSealError("TRANSPARENCY_AFTER_EXECUTION", "Transparency anchor must precede execution");
...
function verifyTransparencyEvidence(evidence, acceptance, publicKeyPem, { before } = {}) {
  ...
  if (checkpoint.anchor && (!validAnchor(checkpoint.anchor) || checkpoint.anchor.merkleRoot || checkpoint.anchor.size !== checkpoint.size || checkpoint.anchor.headEntryHash !== checkpoint.headEntryHash || before !== void 0 && checkpoint.anchor.anchoredAt > before)) return false;
...
function verifyMerkleTransparencyEvidence(evidence, acceptance, publicKeyPem, { before }) {
  ...
  if (checkpoint.anchor && (!validAnchor(checkpoint.anchor) || !checkpoint.anchor.merkleRoot || checkpoint.anchor.merkleRoot !== checkpoint.merkleRoot || checkpoint.anchor.headEntryHash !== checkpoint.headEntryHash || checkpoint.anchor.size !== checkpoint.size || before !== void 0 && checkpoint.anchor.anchoredAt > before)) return false;
  return verifyEd25519Statement(checkpoint, publicKeyPem);
}
function validAnchor(anchor) {
  return anchor.type === "eip155" && Number.isSafeInteger(anchor.chainId) && anchor.chainId > 0 && /^0x[0-9a-f]{40}$/.test(anchor.contract) && /^0x[0-9a-f]{64}$/.test(anchor.txHash) && Number.isSafeInteger(anchor.blockNumber) && anchor.blockNumber >= 0 && Number.isSafeInteger(anchor.anchoredAt) && anchor.anchoredAt >= 0 && Number.isSafeInteger(anchor.size) && anchor.size > 0 && (anchor.merkleRoot ? /^[0-9a-f]{64}$/.test(anchor.merkleRoot) && (anchor.headEntryHash === void 0 || /^[0-9a-f]{64}$/.test(anchor.headEntryHash ?? "")) : /^[0-9a-f]{64}$/.test(anchor.headEntryHash ?? ""));
}
export {
  MERKLE_TRANSPARENCY_CHECKPOINT_SCHEMA,
  TRANSPARENCY_CHECKPOINT_SCHEMA,
