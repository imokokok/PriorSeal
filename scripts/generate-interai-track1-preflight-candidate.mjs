#!/usr/bin/env node
// Generated from generate-interai-track1-preflight-candidate.mts by npm run core:build. Do not edit directly.
...
const REGISTRY_CURRENT_URL = "https://www.oracleinsight.xyz/.well-known/oracle-registry/current.json";
...
function iso(seconds) {
  return new Date(Number(seconds) * 1e3).toISOString();
}
function json(value) {
...
const [block, pendingNonce, ethBalance, wethBalance, routerAllowance, pool, codes] = await Promise.all([
...
try {
  ...
  simulation = {
    success: false,
    error: error instanceof Error && "shortMessage" in error && typeof error.shortMessage === "string" ? error.shortMessage : error instanceof Error ? error.message.split("\n")[0] : String(error)
  };
}
...
const packageDocument = {
  ...
  action: {
    ...
    executorWarning: "Public Hardhat development key; Base Sepolia only; never eligible for production value.",
    pendingNonce,
    target: ROUTER,
    value: "0",
    currency: "USD",
  ...
  oracleSafetyCheckV3: {
    ...
    sourceAssertion: {
      ...
      reason: "The local signer resolves to an expired registry key and audit persistence failed; no stale, sample, synthetic, or unsigned artifact is substituted."
    },
  ...
  handoff: {
    ...
    nextSafeStep: "Keep NO_RUN. Establish the separately scoped allowance outside this phase and restore access to the current production signer plus required audit persistence; then regenerate all volatile fields and both fresh PASS assertions."
  }
};
