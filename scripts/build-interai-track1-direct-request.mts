/** Build the exact InterAI Track 1 pilot wire from a JIT candidate. */
...
async function main(): Promise<void> {
	...
	assert(
		action.schema === "interai-canonical-action/v1" &&
			...
			Object.keys(actionArgs).sort().join(",") ===
				"amount_usd,calldataHash,chainId,currency,nonce,sender,target,value" &&
			actionArgs.chainId === 84532 &&
			actionArgs.target?.toString().toLowerCase() ===
				"0x94cc0aac535ccdb3c01d6787d6413c739ae12bc4" &&
			actionArgs.value === "0" &&
			actionArgs.amount_usd === 4 &&
			actionArgs.currency === "USD" &&
			typeof actionArgs.sender === "string" &&
			/^0x[0-9a-fA-F]{40}$/.test(actionArgs.sender) &&
			typeof actionArgs.calldataHash === "string" &&
			/^0x[0-9a-f]{64}$/.test(actionArgs.calldataHash) &&
			typeof actionArgs.nonce === "string" &&
			/^(0|[1-9][0-9]*)$/.test(actionArgs.nonce) &&
			projection.chainId === actionArgs.chainId &&
			projection.sender === actionArgs.sender &&
			projection.target === actionArgs.target &&
			projection.value === actionArgs.value &&
			projection.calldataHash === actionArgs.calldataHash &&
			projection.nonce === actionArgs.nonce,
	...
	assert(
		typeof calldata === "string" &&
			...
			keccak256(calldata as `0x${string}`) === actionArgs.calldataHash &&
			calldata === destinationBinding.exactCalldata &&
			JSON.stringify(sourceBinding.exactCall) === JSON.stringify(projection) &&
			JSON.stringify(destinationBinding.exactCall) ===
	...
	const externalEvidence = (
		...
		assert(
			assertion.schemaVersion === 3 &&
				...
				data.verdict === "PASS" &&
				data.sourceAssetId?.toString().toLowerCase() === from &&
				data.destinationAssetId?.toString().toLowerCase() === to &&
