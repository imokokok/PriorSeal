...
const EXACT_ALLOWANCE = 1_000_000_000_000_000n;
...
const [chainId, wethCode, ethBalance, wethBalance, allowanceBefore] =
	await Promise.all([
...
if (allowanceBefore !== EXACT_ALLOWANCE) {
	...
	const matching = logs
		.filter((log) => log.args.value === EXACT_ALLOWANCE)
		.at(-1);
	assert(
...
await writeFile(output, serialize(record), { flag: "wx", mode: 0o600 });
