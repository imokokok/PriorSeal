...
type RpcTransaction = { to: string; blockNumber: string; input?: `0x${string}`; data?: `0x${string}` };
...
const transactionHash = txHash as `0x${string}`;
...
let evidence: TransparencyAnchor | undefined;
try { for (const url of rpcUrls) {
  try {
    const reportedChainId = Number(BigInt(await rpcClient.call<string>(url, 'eth_chainId', [])));
    if (reportedChainId !== chainId) continue;
    const [transaction, receipt, head] = await Promise.all([rpcClient.call<RpcTransaction | null>(url, 'eth_getTransactionByHash', [transactionHash]), rpcClient.call<RpcReceipt | null>(url, 'eth_getTransactionReceipt', [transactionHash]), rpcClient.call<string>(url, 'eth_blockNumber', [])]);
    ...
    const block = await rpcClient.call<RpcBlock | null>(url, 'eth_getBlockByNumber', [receipt.blockNumber, false]);
    if (!block) continue;
    ...
    const candidate: TransparencyAnchor = { type: 'eip155', chainId, contract: contractAddress.toLowerCase(), txHash: transactionHash.toLowerCase(), blockNumber, anchoredAt: Number(BigInt(block.timestamp)), size, headEntryHash, ...(commitment === merkleRoot ? { merkleRoot } : {}) };
...
function anchorAbi() { return [{ type: 'function', name: 'anchor', stateMutability: 'nonpayable', inputs: [{ name: 'size', type: 'uint256' }, { name: 'head', type: 'bytes32' }], outputs: [] }] as const; }
function secureConnectionString(value: string | undefined): string | undefined { if (!value) return value; const url = new URL(value); if (['prefer', 'require', 'verify-ca'].includes(url.searchParams.get('sslmode') ?? '')) url.searchParams.set('sslmode', 'verify-full'); return url.toString(); }
