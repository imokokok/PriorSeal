...
async function verifyEnvelope(envelope, label, expectedSource, expectedDestination, registry2, now2) {
  ...
  assert(
    envelope.data.tradeAmountUsd === DECLARED_AMOUNT_USD * 1e6,
  ...
  assert(
    envelope.validUntil - now2 >= MIN_ASSERTION_REMAINING_SECONDS,
    `${label} assertion is not fresh enough`
  );
  assert(
    asNumber(envelope.data.participantCount, `${label}.participantCount`) >= asNumber(
  ...
  assert(
    asNumber(envelope.data.sourceGroupCount, `${label}.sourceGroupCount`) >= asNumber(
  ...
  const publicKeys = registry2.public_keys;
  assert(Array.isArray(publicKeys), "registry public_keys must be an array");
  const key = publicKeys.map((entry, index) => asRecord(entry, `registry.public_keys[${index}]`)).find((entry) => {
    const publicKey = typeof entry.public_key === "string" ? entry.public_key : "";
    return isAddress(publicKey) && getAddress(publicKey) === envelope.attester;
  });
  ...
  assert(
    key.validUntil === null || signedAt <= Date.parse(asString(key.validUntil, "registry key validUntil")),
    `${label} signer expired`
  );
}
async function writePair(output, archive, name, bytes) {
  await Promise.all([
...
await Promise.all([
  mkdir(options.output, { recursive: false, mode: 448 }),
  mkdir(options.archive, { recursive: false, mode: 448 })
]);
...
for (const envelope of [source, destination]) {
  const row = persistedRows.map((entry, index) => asRecord(entry, `audit row ${index}`)).find((entry) => entry.attestation_uid === envelope.uid);
  ...
  assert(
    row.chain_id === CHAIN_ID && Number(row.trade_amount_usd) === DECLARED_AMOUNT_USD,
    `Audit binding mismatch for ${envelope.uid}`
  );
}
const client = createPublicClient({
  chain: baseSepolia,
  transport: http(RPC_URL, { timeout: 3e4 })
});
const [
  chainId,
  block,
  ...
  Promise.all(
    [ROUTER, QUOTER, FACTORY, WETH, USDC].map(
...
const packageDocument = {
  ...
  allowancePrerequisite: {
    ...
    transaction: asRecord(allowanceRecord.transaction, "allowance transaction").hash,
...
process.stdout.write(
  serialize({
    ...
    quoteExpiresAt: new Date(quoteExpiresAt * 1e3).toISOString(),
