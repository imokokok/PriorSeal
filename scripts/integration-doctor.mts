...
export async function integrationDoctor(options: DoctorOptions = {}) {
  ...
  const read = async (base: string, path: string, authenticated = false): Promise<{ check: DiagnosticCheck; body: JsonObject | null }> => {
    ...
    try {
      ...
      const check: DiagnosticCheck = { path: path.split('?')[0], ok: false, status: null, durationMs: Math.round(performance.now() - startedAt), error: 'REQUEST_UNAVAILABLE' };
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    const result = await integrationDoctor({ offer: values.offer as DoctorOptions['offer'], probe: values.probe, samples: Number(values.samples ?? 1), asset: values.asset, chainId: Number(values.chain ?? 1), maxSourceAgeSeconds: Number(values.freshness ?? 300) });
