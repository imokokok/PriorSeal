...
const ROUTER = '0x94cC0AaC535CCDB3C01d6787D6413C739ae12bc4' as `0x${string}`;
const QUOTER = '0xC5290058841028F1614F3A6F0F5816cAd0df5E27' as `0x${string}`;
const FACTORY = '0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24' as `0x${string}`;
const WETH = '0x4200000000000000000000000000000000000006' as `0x${string}`;
const USDC = '0x036CbD53842c5426634e7929541eC2318f3dCF7e' as `0x${string}`;
const EXECUTOR = '0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc' as `0x${string}`;
const FEE = 3_000;
...
function iso(seconds: number | bigint) {
...
function gitHead(directory: string) {
...
async function writeBoth(name: string, bytes: Buffer) {
  await Promise.all([
...
const [block, pendingNonce, ethBalance, wethBalance, routerAllowance, pool, codes] =
  await Promise.all([
    ...
    Promise.all([ROUTER, QUOTER, FACTORY, WETH, USDC].map((address: `0x${string}`) => client.getCode({ address }))),
  ]);
...
let simulation: { success: boolean; error: string | null } = { success: true, error: null };
try {
  ...
  simulation = {
    success: false,
    error:
      error instanceof Error && 'shortMessage' in error && typeof error.shortMessage === 'string'
        ? error.shortMessage
