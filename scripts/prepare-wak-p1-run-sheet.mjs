// Generated from prepare-wak-p1-run-sheet.mts by npm run core:build. Do not edit directly.
...
function parseArgs(argv) {
  ...
  const insightDir2 = values.get("--insight-dir");
  const output2 = values.get("--output");
  assert(insightDir2 && output2, "--insight-dir and --output are required");
  return { insightDir: resolve(insightDir2), output: resolve(output2) };
}
...
function insightTypedData(envelope) {
  return {
...
function insightPairCommitment(source2, destination2) {
  return keccak256(encodeAbiParameters(
    [{ type: "bytes32" }, { type: "bytes32" }, { type: "bytes32" }, { type: "bytes32" }, { type: "uint16" }],
    [source2.uid, destination2.uid, source2.data.requestHash, destination2.data.requestHash, MAX_SLIPPAGE_BPS]
  ));
}
...
async function validateInsight(envelope, expectedSource, expectedDestination, registry2) {
  ...
  const keys = registry2.public_keys;
  assert(Array.isArray(keys), "Insight registry public_keys missing");
  const key = keys.map((entry) => record(entry, "registry key")).find(
    (entry) => String(entry.public_key).toLowerCase() === envelope.attester.toLowerCase()
  );
...
const source = await readJson(`${insightDir}/source-oracle-safety-check-v3.json`);
const destination = await readJson(`${insightDir}/destination-oracle-safety-check-v3.json`);
...
const client = createPublicClient({ chain: baseSepolia, transport: http(rpcUrl, { timeout: 3e4 }) });
...
const amountOutMinimum = quotedAmountOut * BigInt(1e4 - MAX_SLIPPAGE_BPS) / 10000n;
...
const evaluatedAt = Math.floor(Date.now() / 1e3);
const policyPayload = {
...
const acceptance = signAuthorizationReceipt(buildAuthorizationReceipt({
  ...
  issuer,
  keyId,
  acceptedAt: Math.floor(Date.now() / 1e3)
}), privateKeyPem);
...
const runSheet = {
  ...
  roles: {
    ...
    executor: account.address,
    transactionSigner: account.address,
    custody: "PUBLIC_HARDHAT_TEST_ACCOUNT_5_NONPRODUCTION_ONLY",
    priorSealIssuer: issuer,
    priorSealKeyId: keyId
  },
  approvedTest: {
    path: "WETH_TO_USDC_EXACT_INPUT_SINGLE",
    amountInWei: AMOUNT_IN.toString(),
    amountInEth: "0.000001",
    maximumNativeValueWei: AMOUNT_IN.toString(),
  ...
  insight: {
    ...
    sourceVerdict: source.data.verdict,
    destinationVerdict: destination.data.verdict,
...
await mkdir(output, { recursive: false, mode: 448 });
await Promise.all([
...
await writeJson(`${output}/manifest.json`, {
  ...
  transactionSignatureIncluded: false
});
