...
// These deterministic private values are fixture-only and must never be used
// for production PriorSeal authorization or receipt signing.
...
async function makeReceipt({ name, digest, issuedAt, expiresAt, nonceByte, commitments }) {
  const intent = {
    ...
    nonce: String(1000 + Number.parseInt(nonceByte, 16)),
    callTarget: target,
    calldataHash,
  ...
  const execution = {
    ...
    status: 'CONFIRMED',
    blockNumber: 33_100_000 + Number.parseInt(nonceByte, 16),
