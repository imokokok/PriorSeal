...
export async function runReadOnlySmoke({ env = process.env, fetcher = fetch, log = console.log }: SmokeOptions = {}) {
  ...
  async function get(path: string, { token, statuses = [200], text = false }: GetOptions = {}): Promise<{ response: Response; body: any }> {
  ...
  assert(registry.body.issuer === expectedIssuer && registry.body.keys?.some((key: Record<string, any>) => key.issuer === expectedIssuer && key.status === 'active'), 'Expected issuer has no published active key');
  ...
  for (const chainId of requiredChains) assert(capabilities.chains?.includes(chainId) && capabilities.chainReadiness?.some((entry: Record<string, any>) => entry.chainId === chainId && entry.rpc === 'configured'), `Required chain eip155:${chainId} is not configured`);
  ...
  if (env.PRIORSEAL_ARCHIVE_SMOKE_CREDENTIALS_FILE) {
    ...
    const secret = JSON.parse(await readFile(env.PRIORSEAL_ARCHIVE_SMOKE_CREDENTIALS_FILE, 'utf8')) as { credentials: ArchiveCredential[] };
