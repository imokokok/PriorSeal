...
// Deterministic synthetic keys only. Never reuse these values in production.
...
const output = new URL('../examples/web3-agent-kit-base-swap-v2/', import.meta.url);
...
const AMOUNT_OUT_MIN = 2_487_500n;
const MAX_SLIPPAGE_BPS = 50;
const AUTHORIZATION_TIME = Date.parse('2026-09-17T10:00:00.000Z') / 1000;
...
const ROUTER_DEADLINE = AUTHORIZATION_TIME + 1_200;
const INSIGHT_ROTATION_AT = AUTHORIZATION_TIME + 1_800;
...
function seededEd25519(seedByte: string) {
...
function sha256Canonical(value: unknown): `0x${string}` {
...
function requestHash({ sourceAssetId, destinationAssetId, tradeAmountUsd }: { sourceAssetId: string; destinationAssetId: string; tradeAmountUsd: number }) {
...
type Observation = { provider: string; feedId: string; value: string; timestamp: number; dataAgeSeconds: number; included: boolean; exclusionReason: string };
...
async function makeInsightAttestation({ sourceAssetId, destinationAssetId, consensusPrice, observations }: { sourceAssetId: string; destinationAssetId: string; consensusPrice: number; observations: Observation[] }) {
  const tradeAmountUsd = 2_500_000;
  const data = {
    verdict: 'PASS',
    sourceAssetId,
    destinationAssetId,
  ...
  return {
    uid: hashTypedData(typedData as Parameters<typeof hashTypedData>[0]),
    schemaVersion: 3,
    ...
    signature: await insightAccount.signTypedData(typedData as Parameters<typeof hashTypedData>[0]),
...
function makeObservations(values: number[], asset: string): Observation[] {
...
const insightSource = await makeInsightAttestation({
  ...
  observations: makeObservations(
    [250_000_000_000, 249_950_000_000, 250_050_000_000, 250_025_000_000],
    'weth-usd',
  ),
});
const insightDestination = await makeInsightAttestation({
  ...
  observations: makeObservations(
    [100_000_000, 99_990_000, 100_010_000, 100_000_000],
    'usdc-usd',
  ),
});
  ...
  keys: [
    {
      ...
      status: 'retired',
      validFrom: AUTHORIZATION_TIME - 86_400,
...
const unsignedBundle = {
  schema: 'web3-agent-kit.base-swap-evidence.v2',
  fixtureMode: 'SYNTHETIC_NO_BROADCAST',
  assembledAt: INSIGHT_ROTATION_AT + 3_600,
  sources: {
...
const trustRoots = {
  schema: 'web3-agent-kit.base-swap-trust-roots.v2',
  fixtureMode: 'SYNTHETIC_NO_BROADCAST',
