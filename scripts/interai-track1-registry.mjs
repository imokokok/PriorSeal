// Generated from interai-track1-registry.mts by npm run core:build. Do not edit directly.
import { isDeepStrictEqual } from "node:util";
const TRACK1_RELEASE_ID = "0x6e3bd18c541cc80e326754a7743f05050df348257102b93f9a13bc82c7e69f6b";
const TRACK1_SIGNER = "0x6506F789Edd43338A416f59822A63F309f97E8ce";
const EXPECTED_RELEASE = {
  releaseId: TRACK1_RELEASE_ID,
  current: "https://www.oracleinsight.xyz/.well-known/oracle-registry/current.json",
...
function assertTrack1Registry(registryValue, now = Date.now()) {
  ...
  const signers = registry.public_keys.map((entry) => object(entry, "Insight registry public key")).filter(
    (key) => typeof key.public_key === "string" && key.public_key.toLowerCase() === TRACK1_SIGNER.toLowerCase()
  );
  assert(signers.length === 1, "Pinned Insight signer missing or duplicated");
  const signer = signers[0];
  assert(
    signer?.key_id === "insight-oracle-safety-v2-202609" && signer.algorithm === "EIP-712/secp256k1" && signer.revoked === false && signer.validFrom === "2026-08-26T17:35:36.000Z" && signer.validUntil === null && Date.parse(signer.validFrom) <= now,
    "Pinned Insight signer trust root changed or is not active"
  );
}
function assertTrack1SignedDescriptor(assertionValue) {
