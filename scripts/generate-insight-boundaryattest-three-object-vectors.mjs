// Generated from generate-insight-boundaryattest-three-object-vectors.mts by npm run core:build. Do not edit directly.
import {
  createHash,
  createPrivateKey,
  createPublicKey,
...
const authorizationTime = Date.parse("2026-09-17T03:00:00.000Z") / 1e3;
...
const SOURCE_ASSET = "eip155:8453/erc20:0x036cbd53842c5426634e7929541ec2318f3dcf7e";
const DESTINATION_ASSET = "eip155:8453/erc20:0x4200000000000000000000000000000000000006";
...
function contextCommitments(insightCommitment2, boundaryReceipt, overrides = {}) {
  const base = [
    insightCommitment2,
    {
  ...
  if (overrides.wrongInsight)
    return base.map(
      (entry) => entry.namespace === "insight.pretrade-pair.v1" ? { ...entry, digest: `0x${"f".repeat(64)}` } : entry
    );
  if (overrides.wrongBoundary)
    return base.map(
      (entry) => entry.namespace.startsWith("boundaryattest.insight-handoff") ? { ...entry, digest: `0x${"e".repeat(64)}` } : entry
    );
  if (overrides.wrongSigner)
    return base.map(
      (entry) => entry.namespace.startsWith("boundaryattest.governance-signer") ? { ...entry, digest: `0x${"d".repeat(64)}` } : entry
    );
  return base;
}
...
async function makePriorSealReceipt({
  name,
  insightCommitment: insightCommitment2,
  boundaryReceipt,
  commitmentOverrides,
  ...
  const execution = {
    ...
    status: "CONFIRMED",
    blockNumber: 332e5 + receiptCounter,
...
const staleInsightSource = await makeInsightAttestation({
  sourceAssetId: SOURCE_ASSET,
  destinationAssetId: DESTINATION_ASSET,
  checkedAt: authorizationTime - 1e3,
  consensusPrice: 1e8
});
...
const baseClaim = {
  ...
  decision_record: {
    ...
    evidence: {
      ...
      rule_ref: "fixture:insight-risk-reviewed-before-principal-authorization:v1"
    },
    opened: {
    ...
    adjudication: {
      ...
      reason: "The governance review permits the principal to decide whether to authorize.",
  ...
  stopped: makeBoundaryReceipt(baseClaim, {
    ...
    decision_record: {
      ...
      resolution: {
        ...
        reason: "Governance review stopped the handoff before principal authorization.",
        tally: {
...
for (const [name, receipt] of Object.entries(boundaryReceipts)) {
  const suffix = name === "base" ? "" : `-${name.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}`;
...
const expected = {
  ...
  boundaryAttest: {
    ...
    deletionTest: {
      exactFact: "The independently pinned governance signer reviewed the exact named Insight evidence for the exact proposed EVM call and resolved that it may proceed to principal authorization.",
      materialFactLost: true,
      providerReason: "Support the portable claim because it preserves an independently signed governance review and handoff fact that neither native Insight risk evidence nor native PriorSeal authorization and execution evidence establishes."
    }
  },
  priorSeal: {
    issuer,
