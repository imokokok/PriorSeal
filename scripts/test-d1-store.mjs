...
const authorization = { authorizationId: 'test-authorization', intentHash: intent.intentHash, intent, principal: { id: 'principal', account: 'account' }, authorizer: { address: 'address', type: 'eip712' }, delegate: { executor: 'executor' }, authorizationNonce: 'auth-nonce', expiresAt: 2_000_000_000, maxUses: 1 };
const accepted = await store.saveAcceptedAuthorization({ authorization, acceptedAt: 1_700_000_000, createRecord: (log) => ({ authorization, acceptance: { ...log }, policy: { allowed: true }, status: 'ACCEPTED', boundTxHash: null, uses: 0 }) });
