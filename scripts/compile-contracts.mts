...
type CompilerMessage = { severity: string; formattedMessage: string };
type ContractArtifact = { abi: unknown; evm: { bytecode: { object: string } } };
type CompilerOutput = {
...
function parseCompilerOutput(value: unknown): CompilerOutput {
  ...
  const errors = messages.map((entry): CompilerMessage => {
  ...
  if (!isRecord(value.contracts)) {
    if (errors.some((entry) => entry.severity === 'error')) return { contracts: {}, errors };
  ...
  for (const [sourceName, sourceValue] of Object.entries(value.contracts)) {
    ...
    const sourceContracts: Record<string, ContractArtifact> = {};
    for (const [contractName, artifactValue] of Object.entries(sourceValue)) {
      if (!isRecord(artifactValue) || !isRecord(artifactValue.evm) || !isRecord(artifactValue.evm.bytecode)
        || typeof artifactValue.evm.bytecode.object !== 'string' || !('abi' in artifactValue)) {
