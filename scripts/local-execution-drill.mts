...
import type { Eip1193Provider, ObserveExecutionInput, WalletAuthorizationInput } from '../sdk/dist/index.js';

// No RPC URL or signing key is accepted. The only chain is an in-process EVM
// with newly generated test accounts and valueless local balances.
...
const { default: ganache } = await import(pathToFileURL(process.env.GANACHE_MODULE).href) as { default: { provider(options: unknown): { request(args: { method: string; params?: unknown[] }): Promise<unknown>; getInitialAccounts(): Record<string, { secretKey: `0x${string}` }>; disconnect(): Promise<void> } } };
...
let db = new PGlite(), server: Server | undefined, state: Awaited<ReturnType<typeof openDurableState>> | undefined;
...
const startServer = async () => {
  const pool = { query: (sql: string, params?: unknown[]) => db.query(sql, params), connect: async () => ({ query: (sql: string, params?: unknown[]) => db.query(sql, params), release() {} }) } as unknown as Pool;
  server = createHttpServer({ store: createPostgresStore(pool), privateKeyPem, publicKeyPem, issuer, keyId: 'local-1', authorizationAudience: audience,
    observer: input => observeEvm({ ...input, rpcUrls: ['in-process-evm'], rpcClient: { call: async <T,>(_url: string, method: string, params: unknown[]): Promise<T> => await chain.request({ method, params }) as T } }) });
  server.listen(0, '127.0.0.1');
  await once(server, 'listening');
  return `http://127.0.0.1:${(server.address() as AddressInfo).port}`;
};
const stopServer = async () => { if (server?.listening) { const closed = once(server, 'close'); server.close(); server.closeAllConnections(); await closed; } };
const restart = async () => {
  await stopServer();
...
try {
  ...
  const wallet: Eip1193Provider = { request: async ({ method, params }) => {
    assert.equal(method, 'eth_signTypedData_v4'); walletSignatures++;
    assert.ok(params && typeof params[1] === 'string');
    return signer.signTypedData(JSON.parse(params[1]));
  } };
  const now = Math.floor(Date.now() / 1000);
  const transaction = { from, to, chainId: 8453, nonce: 0, value: '0', data: '0x1234' as const };
  ...
  const request: WalletAuthorizationInput = { intent, account: from, principal: { type: 'user', id: 'local-fixture' }, delegate: { agentId: 'local-executor', executor: from }, issuedAt: now, expiresAt: now + 600, audience };
  ...
  const client = createPriorSealClient({ baseUrl, fetch: async (url, options) => {
    ...
    if (dropAcceptance && String(url).endsWith('/v1/authorizations') && response.ok) {
      dropAcceptance = false; await response.text(); throw new Error('Injected response loss after database commit');
    }
    return response;
  } });
  ...
  assert.equal((await state.load<{ stage: string }>('authorization'))?.stage, 'SIGNED');
  baseUrl = await restart();
  const resumed = createPriorSealClient({ baseUrl });
  const authorization = await resumed.authorizeWithWallet(request, wallet, { checkpoint: await state.load('authorization') ?? undefined, onCheckpoint: checkpoint => state!.save('authorization', checkpoint) });
  ...
  const txHash = txHashValue as string;
  ...
  const observationInput = await state.load<ObserveExecutionInput>('execution');
  assert.ok(observationInput);
  const lostResponse = createPriorSealClient({ baseUrl, fetch: async (url, options) => {
    const response = await fetch(url, options);
    if (String(url).endsWith('/v1/executions/observe') && response.ok) { await response.text(); throw new Error('Injected observation response loss'); }
    return response;
  } });
  await assert.rejects(lostResponse.observeExecution(observationInput, { idempotencyKey: 'drill-observation' }), { code: 'NETWORK_ERROR' });
  baseUrl = await restart();
  const restoredInput = await state.load<ObserveExecutionInput>('execution');
  ...
  assert.equal(Number((await db.query<{ n: number }>('SELECT count(*) AS n FROM receipts')).rows[0].n), 1);
...
} finally { await stopServer(); await state?.close(); await db.close(); await chain.disconnect(); await rm(root, { recursive: true, force: true }); }
