...
export async function runReadOnlySmoke({ env = process.env, fetcher = fetch, log = console.log } = {}) {
  ...
  for (const chainId of requiredChains) assert(capabilities.chains?.includes(chainId) && capabilities.chainReadiness?.some((entry) => entry.chainId === chainId && entry.rpc === 'configured'), `Required chain eip155:${chainId} is not configured`);
  ...
  assert(entry?.startsWith('/assets/') && entry.endsWith('.js'), 'Build the current console before checking its deployed asset identity');
