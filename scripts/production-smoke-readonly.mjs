...
async function runReadOnlySmoke({ env = process.env, fetcher = fetch, log = console.log } = {}) {
  ...
  for (const chainId of requiredChains) assert(Array.isArray(capabilities.chains) && capabilities.chains.includes(chainId) && Array.isArray(capabilities.chainReadiness) && capabilities.chainReadiness.some((entry2) => optionalRecord(entry2)?.chainId === chainId && optionalRecord(entry2)?.rpc === "configured"), `Required chain eip155:${chainId} is not configured`);
  ...
  if (env.PRIORSEAL_ARCHIVE_SMOKE_CREDENTIALS_FILE) {
    ...
    const secret = record(JSON.parse(await readFile(env.PRIORSEAL_ARCHIVE_SMOKE_CREDENTIALS_FILE, "utf8")), "Archive smoke credentials");
    ...
    for (const credential2 of credentials) {
      const listed = await get("/v1/archive?limit=1", { token: credential2.token });
      const exported = await get("/v1/archive/export?limit=1", { token: credential2.token });
      for (const checked of [listed, exported]) {
        const page = record(checked.body, "Archive page");
        assert(page.schema === "priorseal.archive-page.v1" && page.projectId === credential2.projectId && page.environment === credential2.environment && page.role === credential2.role, "Archive read did not match the credential scope");
      ...
      const sample = exportPage.items[0] === void 0 ? null : record(exportPage.items[0], "Archive artifact");
      if (sample) {
        ...
        const retrieved = record((await get(`/v1/archive/${encodeURIComponent(id)}`, { token: credential2.token })).body, "Archive artifact retrieval");
        ...
        scopedSamples.push({ credential: credential2, id });
      }
      archiveChecks.push({ projectId: credential2.projectId, environment: credential2.environment, role: credential2.role, list: "PASSED", export: "PASSED", existingArtifactHash: sample ? "PASSED" : "NO_EXISTING_ARTIFACT", writePermissions: "NOT_TESTED_READ_ONLY" });
    }
    for (const sample of scopedSamples) {
      const other = credentials.find((credential2) => credential2.projectId !== sample.credential.projectId || credential2.environment !== sample.credential.environment);
      if (!other) continue;
