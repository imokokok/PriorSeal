// Generated from rotation-impact.mts by npm run core:build. Do not edit directly.
import pg from "pg";
import { pathToFileURL } from "node:url";
async function rotationImpact(pool, nextKeyId, now = Math.floor(Date.now() / 1e3)) {
  ...
  const result = await pool.query(`SELECT authorization_id,expires_at,acceptance_json->>'keyId' AS acceptance_key_id,COUNT(*) OVER() AS total FROM authorizations WHERE bound_tx_hash IS NULL AND acceptance_json->>'keyId' IS DISTINCT FROM $1 ORDER BY expires_at,authorization_id LIMIT 1000`, [nextKeyId]);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    console.log(JSON.stringify(await rotationImpact(pool, process.argv[2]), null, 2));
  } finally {
    await pool.end();
  }
}
