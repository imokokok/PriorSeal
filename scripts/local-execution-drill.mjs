...
const startServer = async () => {
  ...
  server = createHttpServer({ store: createPostgresStore(pool), privateKeyPem, publicKeyPem, issuer, keyId: 'local-1', authorizationAudience: audience,
    observer: input => observeEvm({ ...input, rpcUrls: ['in-process-evm'], rpcClient: { call: (_url, method, params) => chain.request({ method, params }) } }) });
...
try {
  ...
  const request = { intent, account: from, principal: { type: 'user', id: 'local-fixture', account: from }, delegate: { agentId: 'local-executor', executor: from }, issuedAt: now, expiresAt: now + 600, audience };
  ...
  const authorization = await resumed.authorizeWithWallet(request, wallet, { checkpoint: await state.load('authorization'), onCheckpoint: checkpoint => state.save('authorization', checkpoint) });
  ...
  broadcasts++;
  const txHash = await chain.request({ method: 'eth_sendTransaction', params: [{ from, to, data: transaction.data, value: '0x0', gas: '0x10000' }] });
  ...
  baseUrl = await restart();
  const result = await createPriorSealClient({ baseUrl }).observeExecution(await state.load('execution'), { idempotencyKey: 'drill-observation' });
