...
const previous: any = JSON.parse(await readFile(new URL('../examples/web3-agent-kit-base-swap-v2/evidence-bundle.json', import.meta.url), 'utf8'))
...
function typedData(attestation: any): Parameters<typeof hashTypedData>[0] {
  return {
    domain: { name: 'Insight Oracle Safety', version: '3', chainId: 1 },
    types: insightTypes,
    primaryType: 'OracleSafetyCheck',
    message: Object.fromEntries(Object.entries(attestation.data).map(([key, entry]) => [key, uintFields.has(key) ? BigInt(entry as string | number | bigint | boolean) : entry])),
