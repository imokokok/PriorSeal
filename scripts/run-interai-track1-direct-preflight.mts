/** One bounded, non-broadcast InterAI Track 1 /verify call from the host. */
...
function equalJson(a: unknown, b: unknown): boolean {
	...
	if (a && b && typeof a === "object" && typeof b === "object") {
		...
		return (
			...
			keys.every((key) => Object.hasOwn(bb, key) && equalJson(aa[key], bb[key]))
		);
	}
	return false;
}
...
async function validateInputs(
	...
	assert(
		getAddress(params.tokenIn) === WETH &&
	...
	assert(
		(BigInt(string(quote.quotedAmountOut, "quote.quotedAmountOut")) * 9500n) /
			10000n ===
			params.amountOutMinimum && swap.maxSlippageBps === 500,
		"Quote or slippage binding mismatch",
	);
	assert(
		projection.chainId === 84532 &&
	...
	assert(
		registryRelease.releaseId === releaseId &&
			object(registryKeys.registryRelease, "registryKeys.registryRelease")
				.releaseId === releaseId &&
			object(candidate.trustRoots, "candidate.trustRoots").registryReleaseId ===
				releaseId,
	...
	const activeSigner = registryKeys.public_keys.some((entry: unknown) => {
		const key = object(entry, "registry key");
		return (
			key.public_key === signer &&
			key.revoked === false &&
			typeof key.validFrom === "string" &&
			Date.parse(key.validFrom) <= now &&
			(key.validUntil === null ||
				(typeof key.validUntil === "string" &&
					Date.parse(key.validUntil) > now)) &&
			key.role !== "sample"
		);
	});
	...
	assert(
		audit.expectedProductionSigner === signer &&
			Array.isArray(audit.persistedRows) &&
			audit.persistedRows.length === 2,
	...
	for (const [label, assertion, ref] of [
		...
		assert(
			assertion.uid === ref.uid &&
				...
				assertion.validUntil === ref.validUntil,
		...
		const recovered = await recoverTypedDataAddress({
			...typedData,
			signature: string(assertion.signature, `${label}.signature`) as Hex,
		...
		assert(
			binding.assertionUid === assertion.uid &&
				...
				equalJson(binding.economicDeclaration, {
					amount_usd: 4,
					currency: "USD",
				}),
		...
		const auditRow = audit.persistedRows.find(
			(entry: unknown) =>
				object(entry, "audit row").attestation_uid === assertion.uid,
		);
		assert(
			auditRow &&
				object(auditRow, `${label} audit row`).verdict === "PASS" &&
				object(auditRow, `${label} audit row`).signed === true,
	...
	const [chainId, pendingNonce, allowance, wethBalance, ethBalance] =
		await Promise.all([
			client.getChainId(),
	...
	assert(
		(quote.expiresAt as number) * 1000 - Date.now() >= 15_000,
	...
	assert(
		endpoint === ENDPOINT ||
			(process.env.NODE_ENV === "test" &&
				/^http:\/\/127\.0\.0\.1:\d+\/verify$/.test(endpoint)),
...
export async function runCurl(
	config: string,
): Promise<{ body: Buffer; stderr: string; exitCode: number }> {
	return new Promise((resolve, reject) => {
		...
		child.stdout.on("data", (chunk: Buffer) => {
			...
			if (outputSize > 1_100_000) child.kill();
...
async function main(): Promise<void> {
	...
	const key = await new Promise<string>((resolve, reject) => {
		...
		child.on("close", (code) =>
			code === 0
				...
				: reject(new Error("Pilot credential unavailable in macOS Keychain")),
		);
	});
	...
	await writeFile(responsePath, response.body, { mode: 0o600, flag: "wx" });
	const result: Json = {
	...
	if (response.exitCode === 0 && httpStatus === 200) {
		try {
			...
			const authorization =
				parsed.execution_authorization === null ||
				parsed.execution_authorization === undefined
					? null
					: object(parsed.execution_authorization, "ExecutionAuthorization");
			...
			if (
				parsed.recommended_action === "allow" &&
				parsed.policy_result === "allow" &&
				authorization
			) {
				const intent = object(
				...
				const hostAttested = object(
					object(intent.authoritative_context, "intent.authoritative_context")
						.host_attested,
					"host-attested context",
				);
				if (
					parsed.request_contract === "autonomous_execution" &&
					intent.schema === "interai-canonical-execution-intent/v2" &&
					intent.action_authority === "host_attested_canonical" &&
					equalJson(intent.canonical_action, exactRequest.action) &&
					equalJson(hostAttested, exactRequest.execution_context) &&
					parsed.execution_intent_digest === intentDigest &&
					receipt.receipt_id === parsed.trust_receipt_id &&
					authorization.schema === "interai-execution-authorization/v1" &&
					authorization.decision === "allow" &&
					authorization.single_use === true &&
					authorization.decision_id === parsed.decision_id &&
					authorization.execution_intent_digest === intentDigest &&
					Date.parse(string(authorization.expires_at, "authorization expiry")) >
						Date.now()
				) {
					result.disposition =
