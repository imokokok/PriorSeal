// Generated from production-readiness.mts by npm run core:build. Do not edit directly.
...
if (config.privateKeyFile) {
  ...
  if ((privateKeyStat.mode & 63) !== 0) throw new TypeError("Issuer private key must not be readable by group or others");
}
...
for (const chainId of Object.keys(SUPPORTED_CHAINS).map(Number)) {
  ...
  for (const url of urls) {
    const result = await rpcClient.call(url, "eth_chainId", []);
