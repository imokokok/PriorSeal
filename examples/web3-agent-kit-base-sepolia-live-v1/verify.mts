...
import type { KeyRegistry } from '../../sdk/dist/types.js';
...
function hex(value: unknown, path: string): Hex {
...
const sha256Canonical = (value: unknown) => `0x${createHash('sha256').update(canonicalize(value)).digest('hex')}`;
const fingerprint = (pem: string) => createHash('sha256')
...
const keys = keyRegistry.keys.map((entry: unknown, index: number) => record(entry, `bundle.priorSeal.keyRegistry.keys[${index}]`));
...
const requireClaim = (condition: boolean, code: string) => { if (!condition) failures.push(code); };
...
// The SDK validates the full registry and receipt protocols at this boundary.
const receiptVerification = await verifyReceiptLocally(receipt, {
  trustedKeys: keyRegistry as unknown as KeyRegistry,
  now: assembledAt,
});
...
let online: {
  status: string; blockNumber: number; from: string; to: string | null;
  nonce: string; value: string; calldataHash: Hex;
} | null = null;
