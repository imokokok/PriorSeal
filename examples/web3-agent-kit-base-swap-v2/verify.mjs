...
async function verifyInsight(attestation, registry, roots, decisionTime, executionTime) {
  ...
  requireClaim(hashTypedData(typedData) === attestation.uid, 'INSIGHT_UID_MISMATCH');
  ...
  const groups = new Set(observations
    ...
    .map((entry) => attestation.evidence.providerGroups[entry.provider])
...
export async function verifyEvidenceBundle(bundle, roots) {
  try {
