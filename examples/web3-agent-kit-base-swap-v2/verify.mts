...
import type { ContextCommitment, Intent } from '../../sdk/dist/types.js';

type EvidenceBundle = typeof import('./evidence-bundle.json', { with: { type: 'json' } });
type TrustRoots = typeof import('./trust-roots.json', { with: { type: 'json' } });
type InsightAttestation = EvidenceBundle['insight']['sourceAttestation'];
type InsightData = InsightAttestation['data'];
type InsightObservation = NonNullable<InsightAttestation['evidence']>['providerObservations'][number];
type InsightRegistry = EvidenceBundle['insight']['keyRegistry'];
type InsightKey = InsightRegistry['public_keys'][number];
type CheckedInsightRegistry = Omit<InsightRegistry, 'public_keys' | 'revoked_keys'> & {
  public_keys: InsightKey[];
  revoked_keys: Array<{ key_id: string }>;
};
type TransactionDraft = EvidenceBundle['transactionDraft'];
type GovernorPolicy = EvidenceBundle['governor']['policy'];
type GovernorDecision = EvidenceBundle['governor']['decision'];
type PairCommitment = ContextCommitment & { algorithm: 'keccak256' };
...
const INSIGHT_DOMAIN = { name: 'Insight Oracle Safety', version: '3', chainId: 1 } as const;
const INSIGHT_TYPES = {
  OracleSafetyCheck: [
...
type InsightUintField = (typeof INSIGHT_UINT_FIELDS)[number];
type InsightHexField = 'reasonCodesHash' | 'requestHash' | 'evaluatedAssetIdsHash' | 'providerObservationsHash';
type InsightMessage = {
  [Field in keyof InsightData]: Field extends InsightUintField
    ? bigint
    : Field extends InsightHexField
      ? `0x${string}`
      : InsightData[Field]
};
...
function hex(value: string, code: string): `0x${string}` {
  requireClaim(/^0x(?:[0-9a-fA-F]{2})*$/.test(value), code);
  return value as `0x${string}`;
}

function readEvidenceBundle(): EvidenceBundle {
  const value: unknown = JSON.parse(readFileSync(resolve(directory, 'evidence-bundle.json'), 'utf8'));
  requireClaim(isRecord(value), 'BUNDLE_MALFORMED');
  requireClaim(isRecord(value.insight) && isRecord(value.governor) && isRecord(value.transactionDraft)
    && isRecord(value.priorSeal) && isRecord(value.sources), 'BUNDLE_MALFORMED');
  return value as EvidenceBundle;
}

function readTrustRoots(): TrustRoots {
  const value: unknown = JSON.parse(readFileSync(resolve(directory, 'trust-roots.json'), 'utf8'));
  requireClaim(isRecord(value), 'TRUST_ROOTS_MALFORMED');
  requireClaim(isRecord(value.insight) && isRecord(value.priorSeal)
    && isRecord(value.expectedSourceCommits), 'TRUST_ROOTS_MALFORMED');
  return value as TrustRoots;
}
...
function sha256Canonical(value: JsonValue) {
  return `0x${sha256(canonicalize(value))}`;
}

function insightMessage(data: InsightData): InsightMessage {
  const message: Record<string, string | number | boolean | bigint> = { ...data };
  ...
  for (const field of ['reasonCodesHash', 'requestHash', 'evaluatedAssetIdsHash', 'providerObservationsHash'] as const) {
    message[field] = hex(data[field], 'INSIGHT_BYTES32_MALFORMED');
  }
  return message as InsightMessage;
}

function observationsHash(observations: InsightObservation[]) {
...
function agreementBps(observations: InsightObservation[]) {
...
async function verifyInsight(
  ...
  registry: CheckedInsightRegistry,
  roots: TrustRoots,
  decisionTime: number,
  ...
  const providerGroups: Record<string, string> = attestation.evidence.providerGroups;
  const groups = new Set(observations
    .filter((entry) => entry.included)
    .map((entry) => providerGroups[entry.provider])
  ...
  return {
    ...
    keyRotatedAtAssembly: key.validUntil !== null && Date.parse(key.validUntil) / 1_000 <= registrySnapshotTime(registry),
  };
}

function registrySnapshotTime(registry: CheckedInsightRegistry) {
...
function computePairCommitment(
...
function wakIntentId(draft: TransactionDraft) {
...
function verifyDraft(draft: TransactionDraft) {
...
function evaluateGovernor(
  ...
  requireClaim(
    policy.composition.passWithNativeConfirmation === 'PROCEED_TO_PRINCIPAL_AUTHORIZATION'
      && policy.composition.passWithoutNativeConfirmation === 'PROCEED_TO_PRINCIPAL_AUTHORIZATION',
  ...
  const insightEffect = verdicts.some((verdict) => ['DANGER', 'BLOCK'].includes(verdict))
    ? 'DENY'
    : verdicts.some((verdict) => verdict === 'CAUTION')
...
function matchCommitment(
  intent: EvidenceBundle['priorSeal']['receipt']['authorizationEvidence']['authorization']['intent'],
  expected: ContextCommitment,
  code: string,
) {
  requireClaim(Array.isArray(intent.contextCommitments), 'CONTEXT_COMMITMENTS_MALFORMED');
  const contextCommitments: ContextCommitment[] = intent.contextCommitments.map((entry) => {
    requireClaim(entry && typeof entry.namespace === 'string'
      && (entry.algorithm === 'keccak256' || entry.algorithm === 'sha256')
      && typeof entry.digest === 'string', 'CONTEXT_COMMITMENTS_MALFORMED');
    return { namespace: entry.namespace, algorithm: entry.algorithm, digest: entry.digest };
  });
  const checkedIntent: Pick<Intent, 'contextCommitments'> = { contextCommitments };
  const match = matchUniqueContextCommitment(checkedIntent, expected);
  requireClaim(match.matched, code, match.code);
}

export async function verifyEvidenceBundle(bundle: EvidenceBundle, roots: TrustRoots) {
  try {
    ...
    for (const [name, expected] of Object.entries(roots.expectedSourceCommits)) {
      const source = bundle.sources[name as keyof EvidenceBundle['sources']];
      requireClaim(source?.commit === expected, 'SOURCE_COMMIT_MISMATCH', name);
    }
    ...
    const expectedMinOut = BigInt(source.data.tradeAmountUsd) * 1_000_000n / BigInt(destination.data.consensusPrice)
      * BigInt(10_000 - bundle.insight.maxSlippageBps) / 10_000n * 100n;
...
function withRehashedMutation(bundle: EvidenceBundle, mutate: (copy: EvidenceBundle) => void): EvidenceBundle {
