...
async function verifyInsight(attestation, registry, roots, decisionTime, executionTime) {
  ...
  const groups = new Set(observations.filter((entry) => entry.included).map((entry) => attestation.evidence.providerGroups[entry.provider]).filter((group) => group && group !== "derived"));
...
function evaluateGovernor(policy, decision, draft, source, destination, pairCommitment) {
  ...
  if (nativeAllowed && insightEffect === "ELIGIBLE") {
    finalDecision = policy.nativePolicy.requireConfirmation ? "PROCEED_TO_PRINCIPAL_AUTHORIZATION" : "ALLOW_EXECUTION";
