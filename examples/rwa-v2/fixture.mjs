...
function rwaV2Fixture(sdk, now = 18e8) {
  const f = rwaFixture(sdk, now);
  const quote = "0x" + "44".repeat(20), receiver = "0x" + "55".repeat(20);
  f.transaction.data = encodeFunctionData({ abi: sdk.RWA_SWAP_ABI, functionName: "exactInputSingle", args: [{ tokenIn: quote, tokenOut: f.input.instrument.tokenAddress, fee: 3e3, recipient: receiver, deadline: BigInt(now + 120), amountIn: BigInt(f.input.request.amount), amountOutMinimum: 900000n, sqrtPriceLimitX96: 0n }] });
  f.input.request.call.calldataHash = keccak256(f.transaction.data);
  const callProfile = { schema: "insight.rwa-call-profile.v1", adapter: "uniswap-v3-exact-input-single", chainId: 8453, target: f.transaction.to, instrumentId: f.input.request.instrumentId, quoteToken: quote, allowedFees: [3e3] };
  ...
  const receiverEvidence = { ...eligibility, subject: receiver };
  return Object.assign(f, { callProfile, receiverEvidence });
}
async function signRwaV2Fixture(sdk, account, f, sequence = "0", previousDigest = null) {
