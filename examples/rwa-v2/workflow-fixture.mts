...
import type { Execution } from '../../sdk/src/types.js';
...
export async function workflowFixture(prior: PriorApi, insight: typeof import('../../sdk/dist/index.js') = prior) {
  ...
  const observed: Execution={chainId:8453,txHash,status:'CONFIRMED',action:'CONTRACT_CALL',sender:f.transaction.from,recipient:f.transaction.to,target:f.transaction.to,calldataHash:f.input.request.call.calldataHash,nativeValue:'0',asset:intent.asset,amount:intent.amount,nonce:intent.nonce,executedAt:f.now,observedAt:f.now+1,confirmations:12,gasUsed:'100000',transfers:[{asset:s.inputToken,sender:f.transaction.from,recipient:f.transaction.to,amount:s.inputAmount,logIndex:1},{asset:s.outputToken,sender:f.transaction.to,recipient:s.receiver,amount:s.minimumOutput,logIndex:2}],finalityState:'CONFIRMED'};
  const receipt = (observation: Execution=observed) => signReceipt(buildAuthorizedReceipt({authorization:accepted.response.authorization,acceptance:accepted.response.acceptance,policyEvidence:accepted.response.policyEvidence as Parameters<typeof buildAuthorizedReceipt>[0]['policyEvidence'],execution:observation,issuer:acceptanceKey.issuer,keyId:acceptanceKey.keyId,issuedAt:f.now+1}),privateKeyPem);
