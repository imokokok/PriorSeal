...
export function verifyExactCallDecisionSubject(canonical: unknown, intent: unknown, expected: Expected) {
  ...
  if (
    !isRecord(subject) ||
    Object.keys(subject).length !== SUBJECT_FIELDS.length ||
    ...
    !isCanonicalUint(subject.transactionNonce) ||
    !isCanonicalUint(subject.transactionValue) ||
    typeof subject.calldataHash !== 'string' ||
    !/^0x[0-9a-f]{64}$/.test(subject.calldataHash)
  ) {
...
export async function verifyM2Pair({
  artifact,
  priorSealReceipt,
  keyDocument,
  trustedIssuerKeys,
...
export function verifyCalldataFixtures(fixtures: unknown): { name: string; code: string }[] {
  ...
  return fixtures.cases.map((fixture: unknown) => {
    if (
      ...
      !/^0x(?:[0-9a-f]{2})*$/.test(fixture.data) ||
      typeof fixture.expectedCalldataHash !== 'string' ||
      !/^0x[0-9a-f]{64}$/.test(fixture.expectedCalldataHash)
    ) {
...
export function verifySubjectFixtures(fixtures: unknown, expected: Expected): { name: string; code: string }[] {
  ...
  return fixtures.cases.map((fixture: unknown) => {
    if (
      !isRecord(fixture) ||
      typeof fixture.name !== 'string' ||
      !isRecord(fixture.canonical) ||
      !isRecord(fixture.intent) ||
      typeof fixture.expectedCode !== 'string'
    ) {
...
export async function runFixtureChecks({ log = console.log } = {}) {
  ...
  for (const vector of expected.cases) {
    ...
    log(
      `PASS ${vector.name}: ${result.code}${('subjectBindingCode' in result ? result.subjectBindingCode : undefined) ? ` + ${('subjectBindingCode' in result ? result.subjectBindingCode : undefined)}` : ''}`,
    );
  }
  log(
...
if (
  ...
  directory.endsWith('thoughtproof-sentinel-paired-v2')
) {
  try {
