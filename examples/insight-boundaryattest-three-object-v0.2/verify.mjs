...
export function verifyBoundaryAttestReceipt(receipt, expectedPublicKey) {
...
export function evaluateBoundaryAttestValue(deletionTest) {
  if (
...
export async function runFixtureChecks({ log = console.log } = {}) {
  ...
  for (const caseDefinition of expected.cases) {
    ...
    if (
      ...
      result.valueConclusion !== caseDefinition.expectedValueConclusion
    ) {
      throw new Error(
        `${caseDefinition.name}: expected ${caseDefinition.expectedValueConclusion}, received ${result.valueConclusion}`,
      );
    }
    results.push({ name: caseDefinition.name, ...result });
    log(`PASS ${caseDefinition.name}: ${result.code}`);
  }
  const passing = results.find((result) => result.code === 'OK');
