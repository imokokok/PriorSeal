...
function recomputeInsightRequestHash(data) {
...
async function verifyInsightAttestation(attestation, expectedAttester, authorizationTime) {
  if (!isRecord(attestation) || !isRecord(attestation.data))
    return fail("ATTESTATION_INVALID");
  if (attestation.schemaVersion !== 3 || attestation.data.schemaVersion !== 3 || typeof attestation.uid !== "string" || typeof attestation.signature !== "string" || typeof attestation.attester !== "string") {
    return fail("ATTESTATION_INVALID");
  }
  ...
  try {
    if (recomputeInsightRequestHash(attestation.data) !== attestation.data.requestHash) {
  ...
  if (attestation.validUntil !== attestation.data.validUntil || attestation.data.requiredSourceGroupCount !== 2 || attestation.data.sourceGroupCount < attestation.data.requiredSourceGroupCount || attestation.data.participantCount < attestation.data.requiredParticipantCount) {
    return fail("SIGNED_POLICY_NOT_SATISFIED");
  }
  return { ok: true, code: "OK", uid: attestation.uid, data: attestation.data };
}
function computeInsightPairCommitment(source, destination, maxSlippageBps) {
  if (!Number.isSafeInteger(maxSlippageBps) || maxSlippageBps < 0 || maxSlippageBps > 65535) {
...
function verifyBoundaryAttestReceipt(receipt, expectedPublicKey) {
  if (!isRecord(receipt)) return fail("BOUNDARYATTEST_RECEIPT_INVALID");
  if (Object.keys(receipt).sort().join(",") !== "claim,public_key_id,signature") {
  ...
  if (receipt.claim.receipt_role !== "server_attested" || receipt.claim.action_type !== "governance.decision_evidence_exported" || receipt.claim.status !== "exported") {
    return fail("BOUNDARYATTEST_CORE_EVENT_SEMANTICS_INVALID");
  }
  const decision = receipt.claim.decision_record;
  if (!isRecord(decision) || !isRecord(decision.subject) || !isRecord(decision.resolution)) {
    return fail("BOUNDARYATTEST_DECISION_RECORD_INVALID");
  }
  if (![
    "proceed_to_principal_authorization",
    "stop_before_principal_authorization"
  ].includes(decision.resolution.verdict) || decision.resolution.chosen_option !== decision.resolution.verdict) {
  ...
  const expectedAdjudicationOutcome = decision.resolution.verdict === "proceed_to_principal_authorization" ? "released_to_principal_authorization" : "stopped_before_principal_authorization";
  if (Object.keys(decision.subject).sort().join(",") !== "action_ref,action_type" || typeof decision.decision_id !== "string" || decision.decision_id.length === 0 || !Array.isArray(decision.opened?.options) || decision.opened.options.length !== expectedOptions.length || !expectedOptions.every(
    (option) => decision.opened.options.includes(option)
  ) || !Array.isArray(decision.inputs) || decision.inputs.length === 0 || decision.inputs.some(
    (input) => !isRecord(input) || input.decision_id !== decision.decision_id || input.evidence_bundle_digest !== decision.evidence?.bundle_digest
  ) || decision.adjudication?.decision_id !== decision.decision_id || decision.adjudication?.outcome !== expectedAdjudicationOutcome || decision.execution_observation?.state !== "not_observed") {
    return fail("BOUNDARYATTEST_DECISION_RECORD_INVALID");
  }
  return { ok: true, code: "OK", claim: receipt.claim, decision };
}
...
function checkFreshness({
  ...
  return { ok: true, code: "OK", observed };
}
...
function evaluateBoundaryAttestValue(deletionTest) {
  if (!isRecord(deletionTest) || typeof deletionTest.exactFact !== "string" || deletionTest.exactFact.length === 0 || typeof deletionTest.materialFactLost !== "boolean" || typeof deletionTest.providerReason !== "string" || deletionTest.providerReason.length === 0) {
  ...
  return {
    ok: true,
    code: "OK",
    valueConclusion: materialFactLost ? "BOUNDARYATTEST_ADDS_GOVERNANCE_HANDOFF" : "BOUNDARYATTEST_REFERENCE_ONLY",
    deletionTest: {
...
async function verifyThreeObjectFlow({
  insightSource,
  insightDestination,
  boundaryReceipt,
  boundaryPublicKey,
  ...
  trustedIssuerKeys,
  expected,
  usedSignedExports = /* @__PURE__ */ new Set(),
  usedDecisionIds = /* @__PURE__ */ new Set()
}) {
  ...
  if (source.data.subjectChainId !== destination.data.subjectChainId || source.data.sourceAssetId !== destination.data.destinationAssetId || source.data.destinationAssetId !== destination.data.sourceAssetId || source.data.action !== "swap" || destination.data.action !== "swap") {
...
function applyMutation(caseDefinition, source, boundaryReceipt) {
  ...
  if (caseDefinition.mutation === "tamper-boundary-verdict") {
    boundaryReceipt.claim.decision_record.resolution.verdict = "stop_before_principal_authorization";
  }
}
async function runFixtureChecks({ log = console.log } = {}) {
  ...
  for (const caseDefinition of expected.cases) {
    ...
    const usedSignedExports = /* @__PURE__ */ new Set();
    const usedDecisionIds = /* @__PURE__ */ new Set();
    ...
    if (caseDefinition.expectedValueConclusion && (!result.ok || result.valueConclusion !== caseDefinition.expectedValueConclusion)) {
      throw new Error(
...
export {
  computeInsightPairCommitment,
  evaluateBoundaryAttestValue,
  runFixtureChecks,
  verifyBoundaryAttestReceipt,
  verifyInsightAttestation,
