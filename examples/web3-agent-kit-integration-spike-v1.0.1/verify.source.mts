...
import { encodeAbiParameters, hashTypedData, keccak256, verifyTypedData, type Hex } from 'viem'
import {
  verifyAuthorization,
  verifyAuthorizationReceipt,
  verifyAuthorizedReceipt,
} from '../../src/domain/authorization.mjs'
import type { KeyEntry } from '../../src/domain/key-registry.mjs'

const directory = dirname(fileURLToPath(import.meta.url))
const fixtureDirectory = join(directory, 'fixture')
type Baseline = typeof import('./fixture/baseline.json')
type Cases = typeof import('./fixture/cases.json')
type Roots = typeof import('./fixture/trust-roots.json')
type Attestation = Omit<Baseline['insight']['sourceAttestation'], 'uid' | 'signature' | 'attester' | 'data'> & {
  uid: Hex;
  signature: Hex;
  attester: Hex;
  reasonCodes?: string[];
  data: Baseline['insight']['sourceAttestation']['data'] & { requestHash: Hex; reasonCodesHash: Hex };
}
type JsonRecord = Record<string, unknown>
type ReportCase = {
  id: string; wakVersion: string; inputArtifactHashes: { baselineSha256?: string; caseInputSha256?: string; liveInputSha256?: string };
  expectedTerminal: string; actualTerminal: string; reason: string;
  counts: Cases['cases'][number]['counts'];
  reconstruction?: { differentProviderInstance?: boolean; samePersistedAcceptanceId?: boolean };
}
type LiveReceipt = {
  outcome: string;
  execution: { chainId: number; status: string; txHash: string };
  compliance?: { status: string };
  binding?: { bound: boolean };
  issuedAt: number;
  authorizationEvidence: { authorization: { intent: Baseline['priorSeal']['authorization']['intent'] } };
}
type Report = {
  schema: string; fixtureVersion: string; envelopeDigest: string; policyCommitmentDigest: string;
  instrumentation?: { explicitSignerProtocol?: boolean; explicitBroadcastFn?: boolean };
  adapter?: { newFieldTypes?: number };
  adapterAcceptance?: { expectedNewFieldTypes?: number; actualNewFieldTypes?: number; terminal?: string };
  cases: ReportCase[];
  p1?: {
    receipt: LiveReceipt;
    trustedKey: { publicKey: string };
    wak: { envelopeDigest: string; policyCommitmentDigest: string };
    insightPairCommitment: string;
  };
}
...
class FixtureError extends Error {
  constructor(readonly code: string, detail?: string) { super(detail ?? code) }
}
...
function canonical(value: unknown): string {
  ...
  if (value && typeof value === 'object') return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${canonical((value as JsonRecord)[key])}`).join(',')}}`
...
function commitment(intent: Baseline['priorSeal']['authorization']['intent'], namespace: string, algorithm: string, digest: string) {
...
function typedData(attestation: Attestation): Parameters<typeof hashTypedData>[0] {
  return {
    domain: { name: 'Insight Oracle Safety', version: '3', chainId: 1 },
    types: insightTypes,
    primaryType: 'OracleSafetyCheck',
    message: Object.fromEntries(Object.entries(attestation.data).map(([key, value]) => [key, uintFields.has(key) ? BigInt(value as string | number) : value])),
...
async function verifyInsight(attestation: Attestation, baseline: Baseline, roots: Roots) {
  ...
  const registry = baseline.insight.keyRegistry as unknown as {
    public_keys: Array<{ key_id: string; public_key: string; revoked: boolean; validFrom: string; validUntil: string | null }>;
    revoked_keys: Array<{ key_id: string }>;
  }
...
function verifyManifest() {
  const manifest = readJson(join(fixtureDirectory, 'manifest.json')) as typeof import('./fixture/manifest.json')
  demand(manifest.schema === 'wak-insight-priorseal.fixture-manifest.v1', 'MANIFEST_SCHEMA_INVALID')
  demand(manifest.version === 'v1.0.1', 'MANIFEST_VERSION_INVALID')
...
async function verifyBaseline() {
  ...
  const baseline = readJson(join(fixtureDirectory, 'baseline.json')) as Baseline
  const cases = readJson(join(fixtureDirectory, 'cases.json')) as Cases
  const roots = readJson(join(fixtureDirectory, 'trust-roots.json')) as Roots
  ...
  demand(payload.chainId === transaction.chainId && payload.executor === transaction.from && payload.nonce === transaction.nonce &&
    payload.target === transaction.to && payload.nativeValue === transaction.value && payload.calldataHash === keccak256(transaction.data as Hex), 'ENVELOPE_CALL_MISMATCH')
  ...
  for (const attestation of [insight.sourceAttestation, insight.destinationAttestation, insight.blockedDestinationAttestation]) await verifyInsight(attestation as unknown as Attestation, baseline, roots)
  ...
  demand(insight.sourceAttestation.data.sourceAssetId === insight.destinationAttestation.data.destinationAssetId &&
    insight.sourceAttestation.data.destinationAssetId === insight.destinationAttestation.data.sourceAssetId, 'INSIGHT_PAIR_DIRECTION_MISMATCH')
  demand(pair(insight.sourceAttestation as unknown as Attestation, insight.destinationAttestation as unknown as Attestation) === insight.positivePairCommitment.digest, 'INSIGHT_PAIR_DIGEST_MISMATCH')
  demand(pair(insight.sourceAttestation as unknown as Attestation, insight.blockedDestinationAttestation as unknown as Attestation) === insight.blockedPairCommitment.digest, 'INSIGHT_BLOCK_PAIR_DIGEST_MISMATCH')
  demand(baseline.evaluatedAt >= insight.sourceAttestation.data.checkedAt && baseline.evaluatedAt < insight.sourceAttestation.data.validUntil &&
    baseline.evaluatedAt >= insight.destinationAttestation.data.checkedAt && baseline.evaluatedAt < insight.destinationAttestation.data.validUntil, 'INSIGHT_BASELINE_STALE')
  const authorization = priorSeal.authorization
  const authCheck = await verifyAuthorization(authorization as unknown as Parameters<typeof verifyAuthorization>[0], { now: authorization.issuedAt, audience: 'priorseal' })
  ...
  demand(acceptance.issuer === roots.priorSeal.issuer && acceptance.keyId === roots.priorSeal.keyId &&
    verifyAuthorizationReceipt(acceptance as unknown as Parameters<typeof verifyAuthorizationReceipt>[0], publicKey), 'PRIORSEAL_ACCEPTANCE_INVALID')
  ...
  demand(intent.chainId === transaction.chainId && intent.sender === transaction.from && intent.callTarget === transaction.to &&
    intent.nonce === transaction.nonce && intent.calldataHash === keccak256(transaction.data as Hex) && intent.transactionValue === transaction.value,
  ...
  demand(response.schema === 'priorseal.wak-authorization-response.v1' && response.verificationResult?.valid === true && response.verificationResult?.code === 'OK' &&
    canonical(response.signedAuthorization) === canonical(authorization) && canonical(response.acceptance) === canonical(acceptance), 'PRIORSEAL_RESPONSE_INVALID')
  const expectedMapping = {
    authorization_id: authorization.authorizationId, envelope_digest: wak.callEnvelope.digest,
    executor: transaction.from, authorizer: authorization.authorizer.address,
    valid_from: authorization.notBefore, valid_until: authorization.expiresAt,
    nonce: authorization.authorizationNonce, policy_commitment_digest: policy.digest,
  ...
  for (const [key, value] of Object.entries(expectedMapping)) demand(canonical((response as JsonRecord)[key]) === canonical(value) || key === 'raw', 'PRIORSEAL_RESPONSE_MAPPING_MISMATCH', key)
  ...
  demand(n3.input.mutateAfterAuthorization!.field === 'data' && keccak256(n3.input.mutateAfterAuthorization!.value as Hex) !== payload.calldataHash && n3.counts.signer === 0 && n3.counts.broadcast === 0 && n3.counts.receipt === 0, 'N3_VECTOR_INVALID')
...
function verifyWakReport(report: Report, cases: Cases, baseline: Baseline) {
  ...
  if (ids.length === 7) {
    ...
    demand(actual.id === 'P1' && actual.wakVersion === '1.18.4' && actual.expectedTerminal === 'LIVE_RECEIPT_VERIFIED' && actual.actualTerminal === 'LIVE_RECEIPT_VERIFIED' && actual.reason === 'OK' && canonical(actual.counts) === canonical(cases.cases[6].counts) &&
      /^([0-9a-f]{64})$/.test(actual.inputArtifactHashes?.liveInputSha256 ?? ''), 'REPORT_P1_INVALID')
...
async function verifyLiveReceipt(report: Report, keyPin?: string) {
  ...
  const checked = await verifyAuthorizedReceipt(receipt, key.publicKey, { key: key as KeyEntry, now: receipt.issuedAt, audience: 'priorseal' })
...
export async function runVerification({ reportPath, trustKeySha256 }: { reportPath?: string; trustKeySha256?: string } = {}) {
  try {
    const { baseline, cases, manifest } = await verifyBaseline()
    let wakAcceptance = 'NOT_RUN'
    if (reportPath) {
      const report = readJson(resolve(reportPath)) as Report
    ...
    return {
      ...
      fixtureBundleVersion: 'v1.0.1', reportContractVersion: 'v1',
      ...
      compatibilityNote: 'CLOSED: merged WAK PR #90 defines Chain.BASE_SEPOLIA with chain ID 84532; P1 still requires a frozen run sheet and explicit GO.',
    }
  } catch (error) {
    return { status: 'FAIL', code: error instanceof FixtureError ? error.code : 'VERIFIER_ERROR', detail: (error as Error).message }
  }
}
