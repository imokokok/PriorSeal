...
function demand(condition, code, detail) { if (!condition) throw new FixtureError(code, detail) }
...
function domainDigest(namespace, payload) { return `0x${sha256(`${namespace}\0${canonical(payload)}`)}` }
...
function typedData(attestation) {
  return {
    ...
    message: Object.fromEntries(Object.entries(attestation.data).map(([key, value]) => [key, uintFields.has(key) ? BigInt(value) : value])),
  }
}
...
async function verifyBaseline() {
  ...
  demand(acceptance.issuer === roots.priorSeal.issuer && acceptance.keyId === roots.priorSeal.keyId &&
    verifyAuthorizationReceipt(acceptance, publicKey), 'PRIORSEAL_ACCEPTANCE_INVALID')
...
export async function runVerification({ reportPath, trustKeySha256 } = {}) {
  try {
    ...
    return { status: 'FAIL', code: error instanceof FixtureError ? error.code : 'VERIFIER_ERROR', detail: error.message }
  }
}
