...
type UpstreamCase = {
  rail: string
  request: JsonValue
  response: { url: string; pq_trust: { transparency: { receipt: { leaf_hash: string } } } } & Record<string, JsonValue>
  challenge: JsonValue
  method: string
  body: string
  now: number
...
function isUpstreamCase(value: unknown): value is UpstreamCase {
  if (!isRecord(value) || value.rail !== 'base' || typeof value.method !== 'string'
    || typeof value.body !== 'string' || typeof value.now !== 'number' || !isRecord(value.response)
    || typeof value.response.url !== 'string' || !isRecord(value.response.pq_trust)
    || !isRecord(value.response.pq_trust.transparency)
    || !isRecord(value.response.pq_trust.transparency.receipt)
    || typeof value.response.pq_trust.transparency.receipt.leaf_hash !== 'string') return false
...
const upstream: unknown = JSON.parse(readFileSync(source, 'utf8'))
...
const writeJson = (name: string, value: JsonValue) => {
