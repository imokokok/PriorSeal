...
type RouteResponse = typeof import('../fixture/raw/route-response.json', { with: { type: 'json' } })
type SellerChallenge = typeof import('../fixture/raw/seller-challenge.json', { with: { type: 'json' } })
...
type BindingMessage = {
  routeReceiptDigest: `0x${string}`
  paymentAuthorizationDigest: `0x${string}`
  authorizationNonce: `0x${string}`
  bindingNonce: `0x${string}`
  issuedAt: bigint
  expiresAt: bigint
  audience: string
}
type SignedPairInput = {
  to: `0x${string}`
  value: bigint
  authorizationNonce: `0x${string}`
  bindingNonce: `0x${string}`
...
const response = readJson<RouteResponse>(resolve(raw, 'route-response.json'))
const challenge = readJson<SellerChallenge>(resolve(raw, 'seller-challenge.json'))
const context = readJson<RouteContext>(resolve(raw, 'route-context.json'))
...
const typedPayment = (message: PaymentMessage) => ({
...
const typedBinding = (message: BindingMessage) => ({
...
async function signedPair({ to, value, authorizationNonce, bindingNonce }: SignedPairInput) {
