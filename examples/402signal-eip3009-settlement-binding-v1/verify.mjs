// Standalone offline verifier. Includes 402Signal route-guard v0.7.3 from commit ca6e2f1ee806bec6621487338ba6ed3f1ac09352 and bundled viem dependencies.
...
function decodeEventLog(parameters) {
  ...
  const { name, inputs: inputs2 } = abiItem;
  const isUnnamed = inputs2?.some((x) => !("name" in x && x.name));
  const args = isUnnamed ? [] : {};
  const indexedInputs = inputs2.map((x, i) => [x, i]).filter(([x]) => "indexed" in x && x.indexed);
  ...
  const nonIndexedInputs = inputs2.filter((x) => !("indexed" in x && x.indexed));
...
function authenticate(tr, vkey) {
  ...
  const size2 = Number(note[2]);
  if (!Number.isSafeInteger(size2)) fail("invalid_checkpoint");
  const root2 = decode64(note[3], 32), sig = decode64(note[5], 68);
...
`),
  ...
  if (!Number.isSafeInteger(index) || index < 0 || index >= size2 || !Array.isArray(receipt.inclusion_path) || receipt.inclusion_path.length > 53)
  ...
  if (!fold(index, size2).equals(root2)) fail("invalid_inclusion");
...
function verifyManifest() {
  const manifest2 = readJson("MANIFEST.json");
  assert.equal(manifest2.exactRunCommand, "node verify.mjs");
  for (const entry of manifest2.files) {
    ...
    assert.equal(createHash2("sha256").update(bytes).digest("hex"), entry.sha256, `sha256: ${entry.path}`);
  }
  return manifest2;
}
function routeOptions(raw, context2, now) {
  return {
    routeResponseJson: raw.response,
    routeRequestJson: raw.request,
    trustedLogVkey: context2.trustedLogVkey,
    request: {
      url: context2.sellerRequest.url,
      method: context2.sellerRequest.method,
      body: Buffer.from(context2.sellerRequest.bodyBase64, "base64")
    },
...
async function evaluate(vector, inputs2) {
  let releaseCalls = 0;
  const finish = (result) => {
  ...
  let verifiedRoute;
  try {
    verifiedRoute = verifyRoute(routeOptions(inputs2.raw, inputs2.context, vector.now));
  } catch (error) {
