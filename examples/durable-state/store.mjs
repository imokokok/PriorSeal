// Generated from store.mts by npm run core:build. Do not edit directly.
import { createHash, randomUUID } from "node:crypto";
import { mkdir, open, readFile, rename, rm, stat } from "node:fs/promises";
import { join, resolve } from "node:path";
async function openDurableState(directory) {
  ...
  return {
    ...
    save(key, value) {
      const path = pathFor(key);
      if (value === void 0) return Promise.reject(new Error("State value is required"));
