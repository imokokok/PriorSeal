...
function externalKeyId(publicKeyPem: string): string {
  const der = createPublicKey(publicKeyPem).export({ type: 'spki', format: 'der' });
  return `sha256:${createHash('sha256').update(der).digest('hex')}`;
}

function parseWholeSecondTimestamp(value: string): number | null {
  const seconds = Date.parse(value) / 1000;
  return Number.isSafeInteger(seconds) ? seconds : null;
}

function checkFreshness({ timestamp, authorizationTime, maxAgeSeconds, maxFutureSkewSeconds, invalidCode, futureCode, staleCode }: { timestamp: string; authorizationTime: number; maxAgeSeconds: number; maxFutureSkewSeconds: number; invalidCode: string; futureCode: string; staleCode: string }) {
...
export function verifyBoundaryAttestReceipt(receipt: unknown, publicKeyPem: string) {
