...
function verifyBoundaryAttestReceipt(receipt, publicKeyPem) {
...
async function verifyPair({ externalReceipt, externalPublicKey, priorSealReceipt, trustedIssuerKeys, expected, usedSignedExports = /* @__PURE__ */ new Set(), usedDecisionIds = /* @__PURE__ */ new Set() }) {
...
async function runFixtureChecks({ log = console.log } = {}) {
  ...
  const usedSignedExports = /* @__PURE__ */ new Set([JSON.stringify(["sha256:unrelated-signer", externalReceipt.claim.event_id])]);
  ...
  const usedDecisionIds = /* @__PURE__ */ new Set([externalReceipt.claim.decision_record.decision_id]);
