// Generated from jcs.reference.mts by npm run core:build. Do not edit directly.
function assertWellFormedUnicode(value, path) {
  for (let index = 0; index < value.length; index += 1) {
    const unit = value.charCodeAt(index);
    if (unit >= 55296 && unit <= 56319) {
      const next = value.charCodeAt(index + 1);
      if (!(next >= 56320 && next <= 57343)) throw new TypeError(`${path}: lone high surrogate is not I-JSON`);
      index += 1;
...
function canonicalize(value, path, ancestors) {
  ...
  if (typeof value !== "object") throw new TypeError(`${path}: unsupported non-JSON value`);
