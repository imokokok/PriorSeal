...
function requireHex(value: unknown, bytes: number, label: string) {
	if (
		typeof value !== "string" ||
...
function validateInputShape(bundleValue: unknown, rootsValue: unknown) {
	...
	requireRecord(bundle.draft, "draft");
	requireRecord(bundle.draft.initial, "initial draft");
	requireRecord(bundle.draft.final, "final draft");
	requireRecord(bundle.chain, "chain");
	requireRecord(bundle.derivedCall, "derived call");
	requireRecord(bundle.testRecord, "test record");
	requireRecord(bundle.priorSeal, "PriorSeal evidence");
	requireRecord(bundle.priorSeal.authorization, "authorization");
	requireRecord(bundle.priorSeal.authorization.intent, "intent");
	requireRecord(bundle.priorSeal.acceptance, "acceptance");
	requireRecord(bundle.priorSeal.receipt, "receipt");
	requireRecord(bundle.priorSeal.receipt.execution, "execution");
	requireRecord(bundle.priorSeal.keyRegistry, "key registry");
	if (!Array.isArray(bundle.priorSeal.keyRegistry.keys))
		throw new TypeError("key registry keys must be an array");
	requireRecord(bundle.limits, "limits");
	if (
		typeof bundle.draft.initial.id !== "string" ||
		!/^[0-9a-fA-F-]{36}$/.test(bundle.draft.initial.id)
	)
		throw new TypeError("Invalid draft ID");
	if (
		typeof bundle.draft.initial.amountBaseUnits !== "number" ||
		!Number.isSafeInteger(bundle.draft.initial.amountBaseUnits) ||
		bundle.draft.initial.amountBaseUnits <= 0
	)
		throw new TypeError("Invalid draft amount");
	if (
		typeof bundle.draft.initial.expiresAt !== "string" ||
		!Number.isFinite(Date.parse(bundle.draft.initial.expiresAt))
	)
		throw new TypeError("Invalid draft expiry");
	for (const [label, value] of [
		["draft receiver", bundle.draft.initial.receiverAddress],
		["draft token", bundle.draft.initial.tokenAddress],
		["payer", bundle.derivedCall.payer],
		["root payer", roots.payer],
		["root token", roots.tokenAddress],
	] as const)
		requireHex(value, 20, label);
	for (const [label, value] of [
		["root transaction", roots.txHash],
		["root block", roots.blockHash],
		["calldata hash", bundle.derivedCall.calldataHash],
		["context digest", bundle.derivedCall.contextDigest],
	] as const)
		requireHex(value, 32, label);
	if (
		typeof bundle.derivedCall.data !== "string" ||
		!/^0x(?:[0-9a-fA-F]{2})+$/.test(bundle.derivedCall.data)
	)
		throw new TypeError("Invalid calldata");
	if (
		typeof bundle.derivedCall.nonce !== "string" ||
		!/^(0|[1-9][0-9]*)$/.test(bundle.derivedCall.nonce)
	)
...
check(
	bundle.draft.initial.id === roots.draftId &&
		bundle.draft.final.id === roots.draftId,
	"DRAFT_ID",
);
check(
	bundle.chain.txHash === roots.txHash &&
		same(bundle.draft.final.paidTxId, roots.txHash),
	"TX_HASH",
);
check(
	bundle.chain.chainId === 84532 &&
		bundle.draft.initial.network === "BaseSepolia",
	"CHAIN_ID",
);
check(
	bundle.draft.initial.currency === "Usdc" &&
		bundle.draft.final.status === "Confirmed",
	"SEND21_STATUS",
);
...
check(
	Date.parse(bundle.draft.initial.expiresAt) ===
		Date.parse(bundle.draft.final.expiresAt),
...
check(
	bundle.derivedCall.amountBaseUnits ===
		String(bundle.draft.initial.amountBaseUnits),
	"AMOUNT",
);
...
const expectedData = encodeFunctionData({
	...
	args: [
		bundle.draft.initial.receiverAddress as Address,
...
check(
	same(intent.sender, bundle.derivedCall.payer) &&
		same(authorization.delegate.executor, bundle.derivedCall.payer),
	"INTENT_EXECUTOR",
);
...
check(
	intent.validUntil ===
		Math.floor(Date.parse(bundle.draft.initial.expiresAt) / 1000),
	"INTENT_EXPIRY",
);
check(
	intent.contextCommitments?.length === 1 &&
		intent.contextCommitments[0].namespace === "payment-draft.id.utf8.v1" &&
		intent.contextCommitments[0].algorithm === "sha256" &&
		same(intent.contextCommitments[0].digest, bundle.derivedCall.contextDigest),
	"INTENT_CONTEXT",
);
check(authorization.expiresAt === intent.validUntil, "AUTHORIZATION_EXPIRY");
check(
	bundle.testRecord.authorizationSignedAt <=
		bundle.testRecord.locallyAcceptedAt &&
		bundle.testRecord.locallyAcceptedAt <= bundle.testRecord.broadcastAt,
	"SCRIPT_EVENT_ORDER",
);
...
const key = keyRegistry.keys.find(
	(entry: { keyId: string }) => entry.keyId === receipt.keyId,
);
...
check(
	receiptResult.complianceStatus === "COMPLIANT" &&
		receipt.binding.bound === true,
	"POSITIVE_COMPLIANCE",
);
check(
	same(receipt.execution.txHash, roots.txHash) &&
		same(receipt.execution.calldataHash, bundle.derivedCall.calldataHash),
	"RECEIPT_EXECUTION",
);
...
check(
	bundle.chain.executedAt <= intent.validUntil &&
		bundle.chain.executedAt >= authorization.issuedAt,
	"EXECUTION_WINDOW",
);
check(
	bundle.limits.testnetOnly === true &&
		bundle.limits.localEphemeralIssuer === true &&
		bundle.limits.publicDevelopmentWallet === true &&
		bundle.limits.productionIntegrationClaimed === false,
	"CLAIM_BOUNDARY",
);
...
if (process.argv.includes("--online")) {
	...
	const liveValue: unknown = send21Response.ok
		? await send21Response.json()
		: null;
	const liveDraft =
		liveValue && typeof liveValue === "object" && !Array.isArray(liveValue)
			...
			: null;
	check(
		liveDraft?.status === "Confirmed" &&
			same(liveDraft?.paidTxId, roots.txHash),
		"ONLINE_SEND21_STATUS",
	);
	check(
		chainReceipt.status === "success" &&
			same(chainReceipt.blockHash, roots.blockHash),
		"ONLINE_RECEIPT",
	);
	check(
		same(block.hash, roots.blockHash) &&
			Number(block.timestamp) === bundle.chain.executedAt,
		"ONLINE_BLOCK",
	);
	check(
		same(transaction.from, roots.payer) &&
			same(transaction.to, roots.tokenAddress),
		"ONLINE_PARTIES",
	);
