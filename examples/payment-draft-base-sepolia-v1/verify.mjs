...
import { createPublicClient, decodeFunctionData, encodeFunctionData, http, keccak256 } from "viem";
...
let decoded = null;
try {
  decoded = decodeFunctionData({ abi: TRANSFER_ABI, data: bundle.derivedCall.data });
} catch {
...
const expectedData = encodeFunctionData({
  ...
  args: [bundle.draft.initial.receiverAddress, BigInt(bundle.draft.initial.amountBaseUnits)]
});
...
check(intent.validUntil === Math.floor(Date.parse(bundle.draft.initial.expiresAt) / 1e3), "INTENT_EXPIRY");
check(intent.contextCommitments?.length === 1 && intent.contextCommitments[0].namespace === "payment-draft.id.utf8.v1" && intent.contextCommitments[0].algorithm === "sha256" && same(intent.contextCommitments[0].digest, bundle.derivedCall.contextDigest), "INTENT_CONTEXT");
check(authorization.expiresAt === intent.validUntil, "AUTHORIZATION_EXPIRY");
check(bundle.testRecord.authorizationSignedAt <= bundle.testRecord.locallyAcceptedAt && bundle.testRecord.locallyAcceptedAt <= bundle.testRecord.broadcastAt, "SCRIPT_EVENT_ORDER");
...
const key = keyRegistry.keys.find((entry) => entry.keyId === receipt.keyId);
...
check(bundle.limits.testnetOnly === true && bundle.limits.localEphemeralIssuer === true && bundle.limits.publicDevelopmentWallet === true && bundle.limits.productionIntegrationClaimed === false, "CLAIM_BOUNDARY");
let online;
if (process.argv.includes("--online")) {
  ...
  const [transaction, chainReceipt, block, send21Response] = await Promise.all([
    ...
    fetch(`https://send21.io/api/v1/demo/drafts/${encodeURIComponent(roots.draftId)}`, { signal: AbortSignal.timeout(2e4) })
  ]);
