...
async function runFinalPairChecks({ log = console.log } = {}) {
  ...
  for (const vector of expected.cases) {
    ...
    const subjectBindingCode = "subjectBindingCode" in result ? String(result.subjectBindingCode) : void 0;
    if (code !== vector.expectedCode) {
      throw new Error(`${vector.name}: expected ${vector.expectedCode}, received ${code}`);
    }
    if (vector.expectedSubjectBindingCode && subjectBindingCode !== vector.expectedSubjectBindingCode) {
      throw new Error(
