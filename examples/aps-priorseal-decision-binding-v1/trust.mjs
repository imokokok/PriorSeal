// Generated from trust.mts by npm run core:build. Do not edit directly.
const APS_KEYS = Object.freeze({
  agent: "f80727401f51c1b7e41eeda7004b29aca9c9d7a017144c31f7370725514d6260",
  boundary: "6468a72acb50bf67fc180d0a092e22d1aa44a43268c8e2dc7b6302dc9199126c",
  principal: "d69859e9701161194f0f583a2369d287ce895bf0e96b70db753b9e1e27b2bc94"
});
function resolveApsKey(signer, keyId) {
  if (keyId !== "key-1") return void 0;
  return signer === "did:example:agent" ? APS_KEYS.agent : signer === "did:example:boundary" ? APS_KEYS.boundary : void 0;
}
const PRIORSEAL_KEY = Object.freeze({
  issuer: "priorseal.aps-163-fixture",
  keyId: "aps-163-priorseal-test-key-1",
  algorithm: "Ed25519",
  status: "active",
  validFrom: 1789776e3,
  validUntil: null,
  publicKey: "-----BEGIN PUBLIC KEY-----\nMCowBQYDK2VwAyEA4v4qObcyZkKCfW2C1JdiLNbCl/54Jw6qQ2sB8Ia288s=\n-----END PUBLIC KEY-----\n"
});
const REFERENCE_TIME = "2026-09-19T10:05:00.000Z";
const MANIFEST_SHA256 = "2bf365bc9124ecfc943d5be86c34e8e0cacd51a5929b8d0c906e633a017231f9";
export {
  APS_KEYS,
  MANIFEST_SHA256,
  PRIORSEAL_KEY,
  REFERENCE_TIME,
  resolveApsKey
};
