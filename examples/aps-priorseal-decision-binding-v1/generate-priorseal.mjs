// Generated from generate-priorseal.mts by npm run core:build. Do not edit directly.
...
const issuerKey = createPrivateKey({ key: Buffer.from(`302e020100300506032b657004220420${"71".repeat(32)}`, "hex"), format: "der", type: "pkcs8" }).export({ type: "pkcs8", format: "pem" }).toString();
const hash = (value) => createHash("sha256").update(value).digest("hex");
const reference = Date.parse(REFERENCE_TIME) / 1e3;
async function signTestAuthorization(intent, name, edits = {}) {
  const unsigned = buildAuthorization({
    intent,
    ...
    authorizationNonce: `0x${hash(`authorization:${name}`)}`,
    maxUses: "1",
    audience: "priorseal",
    ...edits
  });
...
function receiptFromAuthorization(authorization, name, executionEdits = {}) {
  const intent = authorization.intent;
  const acceptance = signAuthorizationReceipt(buildAuthorizationReceipt({
    authorization,
    issuer: PRIORSEAL_KEY.issuer,
    keyId: PRIORSEAL_KEY.keyId,
    acceptedAt: authorization.issuedAt
  }), issuerKey);
  const execution = {
    schema: "priorseal.execution-observation.v1",
    chainId: intent.chainId,
    txHash: `0x${hash(`synthetic:${name}`)}`,
    status: "CONFIRMED",
    blockNumber: 163,
    blockHash: `0x${"a".repeat(64)}`,
    executedAt: reference,
    observedAt: reference,
    action: "CONTRACT_CALL",
    nonce: intent.nonce,
    sender: EXECUTOR,
    recipient: intent.callTarget,
    target: intent.callTarget,
    calldataHash: intent.calldataHash,
    asset: intent.asset,
    amount: intent.amount,
    transfers: [],
    transferMatchUnique: false,
    nativeValue: intent.transactionValue,
    tokenValue: null,
    gasUsed: "21000",
    fee: null,
  ...
  return signReceipt(buildAuthorizedReceipt({
    authorization,
    acceptance,
    execution,
    issuer: PRIORSEAL_KEY.issuer,
...
async function makeTestReceipt({
  apsCase = "permit",
  name = apsCase,
  nonce = "7",
