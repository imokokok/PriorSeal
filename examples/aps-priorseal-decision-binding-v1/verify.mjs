...
async function runExample() {
  ...
  const cases = [];
  let permitDecisionRef;
  let permitAuthorizationId;
  let reusedDecisionRef;
  let reusedAuthorizationId;
  for (const [name, apsCase, compliance] of [
    ["permit", "permit", "COMPLIANT"],
    ["narrow", "narrow", "COMPLIANT"],
    ["execution-mismatch", "permit", "NON_COMPLIANT"],
    ["decision-reused", "permit", "COMPLIANT"]
  ]) {
    const receipt = JSON.parse(readFileSync(new URL(`./priorseal-inputs/${name}.json`, import.meta.url), "utf8"));
  ...
  for (const [name, code] of [["deny", "APS_GATE_REJECTED"], ["expired", "APS_EXPIRED_AT_REFERENCE"]]) {
  ...
  return {
    profile: "aps-priorseal-offline-report.v1",
    source,
