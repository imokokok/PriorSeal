...
test('valid generic receipt signatures cannot bypass receipt-type and prev-link rules', () => {
  ...
  for (const edit of [
    (fields) => { fields.receipt_type = 'fixture:unrelated:v1'; },
    (fields) => { fields.prev = 'f'.repeat(64); },
  ]) {
    const input = loadApsCase('permit');
    const { receipt_id, signatures, ...fields } = JSON.parse(input.decisionBytes);
    edit(fields);
...
for (const [name, edit, code] of [
  ...
  ['changed digest', (i) => { i.contextCommitments[0].digest = `0x${'f'.repeat(64)}`; }, 'CONTEXT_COMMITMENT_DIGEST_MISMATCH'],
  ['duplicate namespace', (i) => { i.contextCommitments.push({ ...i.contextCommitments[0], digest: `0x${'f'.repeat(64)}` }); }, 'CONTEXT_COMMITMENT_AMBIGUOUS'],
  ['wrong algorithm', (i) => { i.contextCommitments[0].algorithm = 'keccak256'; }, 'CONTEXT_COMMITMENT_ALGORITHM_MISMATCH'],
