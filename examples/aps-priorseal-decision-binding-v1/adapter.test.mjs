...
for (const [name, code, valid] of [["deny", "APS_GATE_REJECTED", false], ["expired", "APS_EXPIRED_AT_REFERENCE", true]]) {
  test(`${name} rejects before any principal signing callback`, async () => {
    let calls = 0;
    const result = await authorizeFromAps(loadApsCase(name), {
      executor: EXECUTOR,
      nonce: "7",
      intentId: name,
      onAuthorize: () => {
...
test("unknown and incorrect receipt keys fail independently of PriorSeal", async () => {
  ...
  for (const resolveKey of [() => void 0, () => APS_KEYS.agent]) {
...
test("valid generic receipt signatures cannot bypass receipt-type and prev-link rules", () => {
  ...
  for (const edit of [
    (fields) => {
      fields.receipt_type = "fixture:unrelated:v1";
    },
    (fields) => {
      fields.prev = "f".repeat(64);
    }
  ]) {
    const input = loadApsCase("permit");
    const parsed = JSON.parse(input.decisionBytes);
...
const intentMutations = [
  ["missing commitment", (i) => {
    delete i.contextCommitments;
  }, "CONTEXT_COMMITMENT_MISSING"],
  ["changed digest", (i) => {
    assert.ok(i.contextCommitments?.[0]);
    i.contextCommitments[0].digest = `0x${"f".repeat(64)}`;
  }, "CONTEXT_COMMITMENT_DIGEST_MISMATCH"],
  ["duplicate namespace", (i) => {
    assert.ok(i.contextCommitments?.[0]);
    i.contextCommitments.push({ ...i.contextCommitments[0], digest: `0x${"f".repeat(64)}` });
  }, "CONTEXT_COMMITMENT_AMBIGUOUS"],
  ["wrong algorithm", (i) => {
    assert.ok(i.contextCommitments?.[0]);
    i.contextCommitments[0].algorithm = "keccak256";
  }, "CONTEXT_COMMITMENT_ALGORITHM_MISMATCH"],
  ["overlong validity", (i) => {
    i.validUntil += 1;
  }, "PRIORSEAL_WINDOW_REJECTED"],
  ["changed call value", (i) => {
    i.transactionValue = "2";
  }, "EXACT_CALL_MISMATCH"],
  ["changed target", (i) => {
    i.callTarget = "0x3333333333333333333333333333333333333333";
  }, "EXACT_CALL_MISMATCH"],
  ["changed calldata", (i) => {
    i.calldataHash = `0x${"b".repeat(64)}`;
  }, "EXACT_CALL_MISMATCH"],
  ["changed chain", (i) => {
    i.chainId = 1;
    i.asset = "eip155:1/native";
  }, "EXACT_CALL_MISMATCH"]
];
for (const [name, edit, code] of intentMutations) {
  test(`validly signed PriorSeal receipt with ${name} fails the composition boundary`, async () => {
    const receipt = await makeTestReceipt({ name: code, editIntent: (intent) => {
      edit(intent);
      return intent;
    } });
...
test("reference time is required to be canonical and rejects before issuance and at expiry", () => {
  ...
  ]) assert.equal(verifyAps(loadApsCase("permit"), { referenceTime }).code, code);
});
test("an overlong PriorSeal window never invokes signing", async () => {
  let calls = 0;
  const result = await authorizeFromAps(loadApsCase("permit"), {
    executor: EXECUTOR,
    nonce: "7",
    intentId: "window-negative",
    validUntil: 1789812601,
    onAuthorize: () => calls++
  });
